...
struct Stru_TimespanInfo
{
	...
	Stru_TimespanInfo<TimeType>& operator= (const Stru_TimespanInfo<TimeType2>& r)
	{
		start=(TimeType)r.start;
		end=(TimeType)r.end;
		offset=r.offset;
		return *this;
	}
...
public:
	...
	CEasyTimespan(const std::map<TimeType,Stru_TimespanInfo<TimeType>>& timespans)
	{
	...
	template <typename TimeType2>
	CEasyTimespan<TimeType>& operator= (const CEasyTimespan<TimeType2>& r)
	{
		const std::map<TimeType2,Stru_TimespanInfo<TimeType2>>& mapTimespans2=r.getTimespans();
		std::map<TimeType2,Stru_TimespanInfo<TimeType2>>::const_iterator it1;
		Stru_TimespanInfo<TimeType> tmpinfo;
		m_mapTimespans.clear();
		for(it1=mapTimespans2.begin();it1!=mapTimespans2.end();it1++)
		{
			tmpinfo=it1->second;
			m_mapTimespans[(TimeType)(it1->first)]=tmpinfo;
		}

		const std::map<int,TimeType2>& mapOffset2=r.getOffsets();
		std::map<int,TimeType2>::const_iterator it2;
		m_mapOffset.clear();
		for(it2=mapOffset2.begin();it2!=mapOffset2.end();it2++)
		{
			m_mapOffset[it2->first]=(TimeType)(it2->second);
		}
		return *this;
	}
    ...
    bool isInTimespans(TimeType itime) const
    {
		...
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it=m_mapTimespans.lower_bound(itime);
    ...
    bool isTimespanIncluding(TimeType start,TimeType end) const
    {
		...
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it=m_mapTimespans.lower_bound(start);
	...
	const std::map<TimeType,Stru_TimespanInfo<TimeType>>& getTimespans(void) const
    {
        return m_mapTimespans;
    }
	const std::map<int,TimeType>& getOffsets(void) const
    {
    	return m_mapOffset;
    }
    ...
    Stru_TimespanInfo<TimeType> getLastTimespan(void) const
	{
		...
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it=m_mapTimespans.end();
   		it--;
    	return it->second;
	}
    TimeType getTimeByOffset(int offset) const
    {
		typename std::map<int,TimeType>::const_iterator it=m_mapOffset.lower_bound(offset);
        if(it!=m_mapOffset.begin()&&(it==m_mapOffset.end()||it->first>offset))
            it--;
        if(it==m_mapOffset.end()||offset<it->first) return (TimeType)0;

		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it2=m_mapTimespans.find(it->second);
    ...
    int getOffsetByTime2(TimeType itime,TimeType& rltTimespanOffset,TimeType& rltTimespanstart,TimeType& rltTimespanend) const
    {
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it=m_mapTimespans.lower_bound(itime);
        if(it!=m_mapTimespans.begin()&&(it==m_mapTimespans.end()||it->first>itime))
            it--;
        ...
        TimeType deltatime=itime-it->first;
        if(deltatime>it->second.end-it->first) return -1;
		rltTimespanOffset=it->second.offset;
		rltTimespanstart=it->second.start;
		rltTimespanend=it->second.end;
        return it->second.offset+(int)deltatime;
    }
    ...
    void exclude(const CEasyTimespan<TimeType>& Exclusions)
    {
		...
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it;
        for(it=Exclusions.getTimespans().begin();it!=Exclusions.getTimespans().end();it++)
        	exclude_internel(it->first,it->second.end);
        normalize_internal();
    }
    void exclude(const std::map<TimeType,Stru_TimespanInfo<TimeType>>& timespans)
    {
	...
	bool isempty(void) { return m_mapTimespans.size()==0 ? true : false; }
	bool getrange(TimeType& start,TimeType&end)
	{
		if((int)m_mapTimespans.size()==0) return false;
		start=m_mapTimespans.begin()->first;
		typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::const_iterator it=m_mapTimespans.end();
    	it--;
		end=it->second.end;
		return true;
															...
															bool blocaltzdst=true,const Stru_TimezoneDst& tzdst=Stru_TimezoneDst())
	{
		...
		typename std::map<time_t,Stru_TimespanInfo<time_t>>::const_iterator it;
...
private:
    ...
    {
    	typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::iterator it;
        ...
        if(trycombine)
        {
            for(it=m_mapTimespans.begin();it!=m_mapTimespans.end();)
            {
        	    typename std::map<TimeType,Stru_TimespanInfo<TimeType>>::iterator it2=it;
                it2++;
...
private:
	...
	//所有timespan的集合
    std::map<TimeType,Stru_TimespanInfo<TimeType>> m_mapTimespans;
	//每个timespan在总长度上的位置 -> timespan的起始值
    std::map<int,TimeType> m_mapOffset;
};
