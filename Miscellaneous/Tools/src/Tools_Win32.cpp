...
string CTools_Win32::GetMainPath()
{
    char buf[512];
    GetMainPath(buf,sizeof(buf));
    return string(buf);
}

void CTools_Win32::GetFileNameWithMainPath(char *pMainPath,int iSize,const char* pPureFileName)
{
    GetMainPath(pMainPath,iSize);
    int len=strlen(pMainPath);
    strncat(pMainPath+len,pPureFileName,iSize-1-len);
}
...
BOOL CTools_Win32::IsFileUpdate(const char *FileName,struct _stat *pLastFileStat)
{
...
void CTools_Win32::GetClientSize(HWND hWnd,int* prltwid,int* prlthei)
{
	RECT Rect;
	GetClientRect(hWnd,&Rect);
	if(prltwid) *prltwid=Rect.right-Rect.left;
	if(prlthei) *prlthei=Rect.bottom-Rect.top;
}
int CTools_Win32::GetClientWid(HWND hWnd)
{
	RECT Rect;
	GetClientRect(hWnd,&Rect);
	return Rect.right-Rect.left;
}
int CTools_Win32::GetClientHei(HWND hWnd)
{
	RECT Rect;
	GetClientRect(hWnd,&Rect);
...
void CTools_Win32::GetTextoutSize(HDC hDC,const char* pbuf,int buflen,int*prltwid,int*prlthei)
{
	if(!pbuf) return;
	SIZE Sz;
    GetTextExtentPoint32(hDC,pbuf,buflen,&Sz);
	if(prltwid) *prltwid=Sz.cx;
	if(prlthei) *prlthei=Sz.cy;
}
void CTools_Win32::GetTextoutSize2(HDC hDC,int textoutval,int*prltwid,int*prlthei)
{
	char buf[256];
	itoa(textoutval,buf,10);
	GetTextoutSize(hDC,buf,strlen(buf),prltwid,prlthei);
}
SIZE CTools_Win32::GetTextoutSize3(HDC hDC,const char* pbuf,int buflen)
{
	SIZE Sz;
    GetTextExtentPoint32(hDC,pbuf,buflen,&Sz);
	return Sz;
}
SIZE CTools_Win32::GetTextoutSize4(HDC hDC,int textoutval)
{
	char buf[256];
	itoa(textoutval,buf,10);
	return GetTextoutSize3(hDC,buf,strlen(buf));
}

//	停止线程，停止前最多等待timeoutseconds秒
void CTools_Win32::StopThread(HANDLE hThread,int timeoutseconds)
{
	if(!hThread) return;

	if(timeoutseconds>0)
		WaitForSingleObject((HANDLE)hThread,timeoutseconds*1000);
...
string CTools_Win32::GBKToUTF8(const string& strGBK)  
{  
    string strOutUTF8 = "";  
    WCHAR * str1;  
...
string CTools_Win32::UTF8ToGBK(const string& strUTF8)  
{  
...
HWND CTools_Win32::FindDesktopWindowByParent(HWND hParent,const char* ClassName,const char* WindowName)
{
	class CFindDesktopWindowByParent
	{
	public:
		CFindDesktopWindowByParent(const char* windowname,const char* classname,HWND hparent)
		: m_WindowName(windowname),m_ClassName(classname),m_hParent(hparent),m_hWnd(NULL)
		{
		...
		{
			CFindDesktopWindowByParent* pobj=(CFindDesktopWindowByParent*)lParam;
			if(GetParent(hwnd)==pobj->m_hParent)
			{
				char buf1[256]={0};
				char buf2[256]={0};
				GetWindowText(hwnd,buf1,sizeof(buf1)-1);
				GetClassName(hwnd,buf2,sizeof(buf2)-1);
				if(string(buf1)==pobj->m_WindowName&&string(buf2)==pobj->m_ClassName)
				{
					pobj->m_hWnd=hwnd;
					return FALSE;
				}
	...
	CFindDesktopWindowByParent find(WindowName,ClassName,hParent);
	return find.m_hWnd;
}

#ifdef _USE_PSAPI_
//获取指定模块名称的进程ID。这里的名称不包含路径。如果有多个进程，返回第一个
vector<DWORD> CTools_Win32::GetProcessIDByExeFileName(const char* ExeFileName)
{
	vector<DWORD> vecRlt;

	char purefilename[256]={0};
	int Len=strlen(ExeFileName);
	int Off=Len;
	for(;Off>0&&ExeFileName[Off-1]!='\\'&&ExeFileName[Off-1]!=':';Off--);
	if(Len-Off>=sizeof(purefilename))
		return vecRlt;
	memcpy(purefilename,ExeFileName+Off,Len-Off);

	DWORD ProcessIDs[2048];
	DWORD cbNeeded;
	if(!EnumProcesses(ProcessIDs,sizeof(ProcessIDs),&cbNeeded))
		return vecRlt;
	int cntProcessID=cbNeeded/(int)sizeof(DWORD);
	for(int i=0;i<cntProcessID;i++)
	{
		HANDLE hProcess=OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ,FALSE,ProcessIDs[i]);
		if(!hProcess) continue;
		char modulebasename[256];
		int len=GetModuleBaseName(hProcess,NULL,modulebasename,sizeof(modulebasename)-1);
		CloseHandle(hProcess);
		if(len>0)
		{
			modulebasename[len]=0;
			if(strcmp(modulebasename,purefilename)==0) vecRlt.push_back(ProcessIDs[i]);
		}
    }
	return vecRlt;
}
...
#ifdef _USE_PSAPI_
//杀死进程。这里的名称不包含路径
bool CTools_Win32::KillProcess(const char* ExeFileName)
{
	bool brlt=true;
	vector<DWORD> vecProcessID=GetProcessIDByExeFileName(ExeFileName);
	if(vecProcessID.size()==0) return false;
	for(int i=0;i<(int)vecProcessID.size();i++)
	{
		HANDLE hProcess=OpenProcess(PROCESS_TERMINATE,FALSE,vecProcessID[i]);
		if(!hProcess) 
		{
			brlt=false;
			continue;
		}
		BOOL bRlt2=TerminateProcess(hProcess,0);
		if(!bRlt2) brlt=false;
		CloseHandle(hProcess);
	}
	return brlt;
