...
//切分串。串格式为：str11-str12;str21-str22;str31-str32，其中[;]为splitchar1，[-]为splitchar2；过滤分隔符首尾的空格、tab、回车、换行；过滤掉空的子串
                                         ...
                                         COMPARE_LookupInSequenceBuffer CompareMethod)
{
    ...
    while(1)
    {
	    ...
	    {
            //找到了结果, CmpRlt==0，要查找的内容在Mid(Head和Tail之间)
            break;
	    }
    }

    //这时的Head和Tail是可能的上限和下限
    iHead=Head;
    iTail=Tail;
    iMid=Mid;
