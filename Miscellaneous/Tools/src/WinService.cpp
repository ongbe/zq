...
//Advapi32.lib是链接时windows服务api需要的
...
CWinService::eRunMode       CWinService::m_RunMode=CWinService::NotRunning;
...
HANDLE                      CWinService::m_hThread_RunAsyn=NULL;                //RunAsyn模式下的运行线程
								  ...
								  WinService_DELAYFUNC pDelayFunc)
{
	...
	if(argc>=2&&stricmp(argv[1],WinService_STOPSERVICECMD)==0)
	{
		if(argc==2)
        {
            ...
            if(1)
            {
                //检查nowinrunmode.txt存在的话，创建needstopinrunmode.txt文件，指示run mode运行的服务停止
                string      mainpath=CTools_Win32::GetMainPath();
                string      filename=mainpath+string("\\nowinrunmode.txt");
                if(_access(filename.c_str(),0)!=-1)
                {
                    filename=mainpath+string("\\needstopinrunmode.txt");
                    if(_access(filename.c_str(),0)==-1)
                    {
                        HFILE hF=_lcreat(filename.c_str(),0);
                        _lclose(hF);
                    }
                }
            }
	...
	//Run模式，直接运行
    //启动时，先创建nowinrunmode.txt文件，删除needstopinrunmode.txt文件。
    //然后持续监视该文件，一旦发现该文件出现，即退出runmode服务
	if(argc==2&&stricmp(argv[1],WinService_RUNCMD)==0)
	{
        ...
        //创建nowinrunmode.txt文件，删除needstopinrunmode.txt文件
        if(1)
        {
            string      filename=mainpath+string("\\nowinrunmode.txt");
            if(_access(filename.c_str(),0)==-1)
        ...
        CFileOpr::getObj().writelocallog("threadid","TID:\t% 8u\tCWinService::mainService_WorkerThread_RunMode(RUNCMD)",ThreadID);
		...
		while(!m_NeedStopID)
		{
            ...
            //检查needstopinrunmode.txt文件，如果存在则退出
            if(1)
            {
                string filename=mainpath+string("\\needstopinrunmode.txt");
                if(_access(filename.c_str(),0)!=-1)
        ...
        //删除nowinrunmode.txt文件和needstopinrunmode.txt文件
        if(1)
        {
            string      filename=mainpath+string("\\nowinrunmode.txt");
            if(_access(filename.c_str(),0)!=-1)
    ...
    //运行，异步直接运行
	if(argc==2&&stricmp(argv[1],WinService_RUNASYNCMD)==0)
	{
		char		*iargv[2]={"",""};
		DWORD		ThreadID;
		...
		m_RunningID_RunMode=1;
		m_hThread_RunAsyn=CreateThread(NULL,65536,(LPTHREAD_START_ROUTINE)mainService_WorkerThread_RunMode,NULL,0,&ThreadID);
        CFileOpr::getObj().writelocallog("threadid","TID:\t% 8u\tCWinService::mainService_WorkerThread_RunMode(RUNASYNCMD)",ThreadID);


		return 0;
	}
																				//下面执行服务指定的任务
    m_RunMode=RunInService;
...
void CWinService::Stop_WinService(void)
{
    ...
    //对应异步直接运行的，需要通知并等待服务线程结束
    if(m_RunMode==RunInAsyn&&m_hThread_RunAsyn!=NULL)
    {
		DWORD		ExitCode;
		m_RunningID_RunMode=0;
		WaitForSingleObject((HANDLE)m_hThread_RunAsyn,8000);
		if(GetExitCodeThread((HANDLE)m_hThread_RunAsyn,&ExitCode)!=0&&ExitCode==STILL_ACTIVE)
			TerminateThread((HANDLE)m_hThread_RunAsyn,0);
        m_hThread_RunAsyn=NULL;
