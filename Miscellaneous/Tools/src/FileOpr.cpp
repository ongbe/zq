...
bool CFileOpr::writelocallog(const char* logname,const char* fmt,...)
{
	...
	return writelocallog_internal("log",logname,logbuf,false,false,true,true,false,false,false);
}
//write log to local file.
//the log file exist in the exe file path,
//and date will be added to tail of logname
bool CFileOpr::writelocallog4(const char* logname,const char* fmt,...)
{
	va_list ap;
	char logbuf[20480];
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal("log",logname,logbuf,false,false,true,true,false,true,false);
}
...
//the log file exist in the exe file path,
//and date will be added to tail of logname
bool CFileOpr::writelocallog3(const char* logfilenamewithpath,const char* fmt,...)
{
	va_list ap;
	char logbuf[20480];
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(NULL,logfilenamewithpath,logbuf,false,false,false,true,false,false,true);
}
...
bool CFileOpr::writelocallog2(const char* subpathname,const char* logname,bool bAppendExtName,bool bInsertTime,bool bDelFileFirst,const char* fmt,...)
{
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(subpathname,logname,logbuf,false,false,bAppendExtName,bInsertTime,bDelFileFirst,false);
}
...
bool CFileOpr::writelocallogDaily(const char* subpathname,const char* logname,const char* fmt,...)
{
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(subpathname,logname,logbuf,true,false,true,true,false,false);
}
bool CFileOpr::writelocallogDaily2(bool bEncrypt,const char* subpathname,const char* logname,const char* fmt,...)
{
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(subpathname,logname,logbuf,true,bEncrypt,true,true,false,false);
}
...
bool CFileOpr::writelocallogDaily3(const char* subpathname,const char* logname,const char* fmt,...)
{
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(subpathname,logname,logbuf,true,false,true,true,false,true);
}
bool CFileOpr::writelocallog4(const char* subpathname,const char* logname,bool bDaily,bool bEncrypt,bool bAppendExtName,bool bInsertTime,bool bDelFileFirst,const char* fmt,...)
{
	va_list ap;
	char logbuf[1024];
    ...
    CeasymutexGuard guard(m_mutex);
	return writelocallog_internal(subpathname,logname,logbuf,bDaily,bEncrypt,bAppendExtName,bInsertTime,bDelFileFirst,false);
}

//bDaily: 是否每天一个日志文件
bool CFileOpr::writelocallog_internal(const char* subpathname,const char* logname,const char* logbuf,bool bDaily,bool bEncrypt,bool bAppendExtName,bool bInsertTime,bool bDelFileFirst,bool bPrintf,bool blognameincludepath)
{
	char filename[512];
	char pathname[512];
	char strdate[64];
	...
	else
	{
		...
		if(len+32+strlen(logname)>=sizeof(filename))
			return false;
		if(!subpathname||subpathname[0]==0) sprintf(pathname+strlen(pathname),"%clog",seperator);
...
{
    return getcurrentexepath2(buf,bufsize,false);
}
//get exe path of current process, include tail of '\' or '/'
bool CFileOpr::getcurrentexepath2(char* buf,int bufsize,bool bTailIsSeperator)
{
	int len=0;
