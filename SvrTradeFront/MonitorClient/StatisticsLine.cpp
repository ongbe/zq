...
IMPLEMENT_DYNAMIC(CStatisticsLine, CWnd)

CStatisticsLine::CStatisticsLine()
{
...
//	m_pFData = NULL;
	...
	m_clLineColor[0] = RGB(120, 150, 100);//白
	...
	m_clLineColor[6] = RGB(200, 200, 200);//大红
	m_IsDelay = false;
}
...
BEGIN_MESSAGE_MAP(CStatisticsLine, CWnd)
	ON_WM_PAINT()
	//ON_WM_MOUSEMOVE()
	ON_WM_MOUSEHOVER()
	ON_WM_MOUSELEAVE()
...
BOOL CStatisticsLine::Create(CRect rtClient, CWnd* pParentWnd, UINT nID)
{
	return	CWnd::Create(NULL,  NULL, WS_CHILD|WS_VISIBLE, rtClient, pParentWnd, nID, NULL);
}
void CStatisticsLine::OnPaint() 
{
	CPaintDC dc(this); 
...
void CStatisticsLine::DrawCommonLast(CDC* pDC)
{
...
void CStatisticsLine::DrawBitmap_TLine(CDC *pDC)
{
	CRect rtClient,rect;
...
void CStatisticsLine::DrawHead(CDC *pDC)
{
	CRect rtClient;
	GetClientRect(&rtClient);
...
//	pt.y = rtClient.top - sz.cy; 
	...
	{	
		{
			CPen penLine(PS_SOLID, 1, m_clLineColor[0]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("上行接收数/分");
		...
		{	
			pt.x = rt2.right +1;
			CPen penLine(PS_SOLID, 1, m_clLineColor[1]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("上行发送数/分");
			sz = pDC->GetTextExtent(strLineName);	
			pt.x = pt.x ;
			pt.y = rtClient.top -sz.cy; 	
			rt2.left   = pt.x;
			...
			rt2.right  = rt2.left + sz.cx;
			rt2.bottom = rt2.top + sz.cy;
			pDC->SetTextColor(m_clLineColor[1]);
			pDC->DrawText(strLineName, rt2, DT_LEFT|DT_TOP);
			pDC->SelectObject(pOld);
		}
		{	
			pt.x = rt2.right +1;
			CPen penLine(PS_SOLID, 1, m_clLineColor[2]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("下行接收数/分");
			sz = pDC->GetTextExtent(strLineName);	
			pt.x = pt.x ;
			pt.y = rtClient.top -sz.cy; 	
			rt2.left   = pt.x;
			...
			rt2.right  = rt2.left + sz.cx;
			rt2.bottom = rt2.top + sz.cy;
			pDC->SetTextColor(m_clLineColor[2]);
			pDC->DrawText(strLineName, rt2, DT_LEFT|DT_TOP);
			pDC->SelectObject(pOld);
		}
		{	
			pt.x = rt2.right +1;
			CPen penLine(PS_SOLID, 1, m_clLineColor[3]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("下行发送数/分");
			sz = pDC->GetTextExtent(strLineName);	
			pt.x = pt.x ;
			pt.y = rtClient.top -sz.cy; 	
			rt2.left   = pt.x;
			...
			rt2.right  = rt2.left + sz.cx;
			rt2.bottom = rt2.top + sz.cy;
			pDC->SetTextColor(m_clLineColor[3]);
			pDC->DrawText(strLineName, rt2, DT_LEFT|DT_TOP);
			pDC->SelectObject(pOld);
		}
	}
	...
	{
		{				
			CPen penLine(PS_SOLID, 1, m_clLineColor[4]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("上行延迟毫秒/分");
			sz = pDC->GetTextExtent(strLineName);	
			pt.x = pt.x ;
			pt.y = rtClient.top -sz.cy; 	
			rt2.left   = pt.x;
			...
			rt2.right  = rt2.left + sz.cx;
			rt2.bottom = rt2.top + sz.cy;
			pDC->SetTextColor(m_clLineColor[4]);
			pDC->DrawText(strLineName, rt2, DT_LEFT|DT_TOP);
			pDC->SelectObject(pOld);
		}
		{	
			pt.x = rt2.right +1;
			CPen penLine(PS_SOLID, 1, m_clLineColor[5]);	
			CPen* pOld = pDC->SelectObject(&penLine);
			pOld = pDC->SelectObject(&penLine);
			CString strLineName = _T("下行延迟毫秒/分");
			sz = pDC->GetTextExtent(strLineName);	
			pt.x = pt.x ;
			pt.y = rtClient.top -sz.cy; 	
			rt2.left   = pt.x;
			...
			rt2.right  = rt2.left + sz.cx;
			rt2.bottom = rt2.top + sz.cy;
			pDC->SetTextColor(m_clLineColor[5]);
			pDC->DrawText(strLineName, rt2, DT_LEFT|DT_TOP);
			pDC->SelectObject(pOld);
		}
	}
...
void CStatisticsLine::DrawOneItemData(CDC *pDC,  CRect rtClient)
{
	...
	{
		StatData pTLine = m_vecTLine[m_nCurSel];
...
//	dc.DrawText(strText, pt);
	...
	if(m_nCurSel >=0 && m_nCurSel < (int)m_vecTLine.size())
	{
		...
		strText = Double2String( pTLine.dwUpRecv, m_nNumDigits);
		SetTextColor(pDC, pTLine.dwUpRecv);
	}
	else 
		strText = _T("----");
	sz = pDC->GetTextExtent(strText);
	...
	if(m_nCurSel >=0 && m_nCurSel < (int)m_vecTLine.size())
	{
		StatData pTLineBegin = m_vecTLine[m_nBegin];
		...
		{
			StatData pTLineEnd = m_vecTLine[i];
...
//	dc.DrawText(strText, pt);
	...
	if(m_nCurSel >=0 && m_nCurSel < (int)m_vecTLine.size())
	{
		...
		strText = Double2String( pTLine.dwUpRecv - m_fLastClosePrice, m_nNumDigits);
		SetTextColor(pDC, pTLine.dwUpRecv);
	...
	if(m_nCurSel >=0 && m_nCurSel < (int)m_vecTLine.size())
	{
		...
		{
			strText.Format(_T("%.2f%%"), 100*(pTLine.dwUpRecv - m_fLastClosePrice)/m_fLastClosePrice);	
			SetTextColor(pDC, pTLine.dwUpRecv);
		}				
	}
	else 
...
*/
void CStatisticsLine::DrawCurSelLine(CDC *pDC)
{	
	SHitTest hitTest;
...
void CStatisticsLine::DrawBorder(CDC *pDC)
{
	CRect rtClient;
	GetClientRect(&rtClient);
...
void CStatisticsLine::DrawScales(CDC* pDC)
{
...
//		pDC->DrawText(strText, rt, DT_LEFT|DT_TOP);
	...
	strText.Format(_T("%.0f"), m_fMaxValue);
	sz = pDC->GetTextExtent(strText);	
	pt.x = rtClient.left - sz.cx-4;
	pt.y = m_rtTLine.top - sz.cy/2 ; 
	rt.left   = pt.x;
	rt.top    = pt.y;
	rt.right  = rt.left + sz.cx;
	rt.bottom = rt.top + sz.cy;
...
void CStatisticsLine::DrawUpRecev(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
	...
	StatData pTLineEnd1 = m_vecTLine[m_nEnd];
	for( int i = m_nEnd+1; i<= m_nBegin; i++ )
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwUpRecv);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwUpRecv);
	...
	//画开始
/*	StatData pTLineTemp = m_vecTLine[m_nBegin];
	if(difftime(mktime(&m_vecTimeScales[0]), mktime(&pTLineTemp.dwTime)) < 0 )
	{
		...
		int nYEnd = JiaGe2GaoDu(pTLineTemp.dwUpRecv);
		pDC->MoveTo(nXBegin, nYBegin);
...
void CStatisticsLine::DrawUpSend(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
	...
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwUpSend);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwUpSend);
...
void CStatisticsLine::DrawDownRecev(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
	...
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwDownRecv);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwDownRecv);
...
void CStatisticsLine::DrawDownSend(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
	...
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwDownSend);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwDownSend);
...
void CStatisticsLine::DrawUpDelay(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
	...
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwUpDelay);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwUpDelay);
...
void CStatisticsLine::DrawDownDelay(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
	...
	{
		int nXBegin = Time2X(pTLineEnd1.dwTime);
		int nYBegin = JiaGe2GaoDu(pTLineEnd1.dwDownDelay);
		StatData pTLineBegin1 = m_vecTLine[i];
		int nXEnd = Time2X(pTLineBegin1.dwTime);
		int nYEnd = JiaGe2GaoDu(pTLineBegin1.dwDownDelay);
...
bool CStatisticsLine::DrawPriceLines_NoData(CDC* pDC, StatData& pTLineEnd1, StatData& pTLineBegin1)//end时间近
{	
...
void CStatisticsLine::SetTextColor(CDC* pDC, double dbClosePrice)
{
...
void CStatisticsLine::GetBeginEnd()
{
	if(m_vecTimeScales.size() == 0)
		return;
...
void CStatisticsLine::SetMaxMinValue(double& dbMaxValue, double& dbMinValue)
{
...
void CStatisticsLine::CalcMaxMin()
{
	...
	{		
		...
		if(!m_IsDelay)
		{
			if(pTLine.dwUpRecv > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwUpRecv;
			}
			if(pTLine.dwUpRecv < m_fMinValue)
			{
				m_fMinValue = pTLine.dwUpRecv;
			}

			if(pTLine.dwUpSend > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwUpSend;
			}
			if(pTLine.dwUpSend < m_fMinValue)
			{
				m_fMinValue = pTLine.dwUpSend;
			}

			if(pTLine.dwDownRecv > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwDownRecv;
			}
			if(pTLine.dwDownRecv < m_fMinValue)
			{
				m_fMinValue = pTLine.dwDownRecv;
			}
			if(pTLine.dwDownSend > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwDownSend;
			}
			if(pTLine.dwDownSend < m_fMinValue)
			{
				m_fMinValue = pTLine.dwDownSend;
			}
		}
		else
		{
			if(pTLine.dwUpDelay > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwUpDelay;
			}
			if(pTLine.dwUpDelay < m_fMinValue)
			{
				m_fMinValue = pTLine.dwUpDelay;
			}

			if(pTLine.dwDownDelay > m_fMaxValue)
			{
				m_fMaxValue = pTLine.dwDownDelay;
			}
			if(pTLine.dwDownDelay < m_fMinValue)
			{
				m_fMinValue = pTLine.dwDownDelay;
			}
		}
	}
...
int	CStatisticsLine::JiaGe2GaoDu(double fJiaGe)
{
	CRect rtClient ;
	rtClient = m_rtTLine;
/*	GetClientRect(&rtClient);
...
double CStatisticsLine::GaoDu2JiaGe(int nHeight)
{
	CRect rtClient ;
	rtClient = m_rtTLine;
/*	GetClientRect(&rtClient);
...
int CStatisticsLine::Time2X(struct tm tmTime)
{
	if(m_vecTimeScales.size() == 0)
		return -1;
	tmTime.tm_sec = 0;
...
struct tm CStatisticsLine::X2Time(int nx)
{
	struct tm tTime;
	tTime.tm_year = 0;
...
void CStatisticsLine::DateTime2Tm(COleDateTime dt ,struct tm& tmDt)
{
...
void CStatisticsLine::SetFuture(std::string ExchangeName,std::string ContractName)
{
	m_ExchangeName = ExchangeName;
	m_ContractName = ContractName;	
}
BOOL CStatisticsLine::SetVectorData(std::vector<StatData>& vec)
{
	m_vecTLine.clear();
	m_vecTLine = vec;	
	GetBeginEnd();
...
void CStatisticsLine::SetScales(std::vector<struct tm>& vecScales)
{
	if(vecScales.size() < 2)
		return;
...
void CStatisticsLine::SetLastClosePrice(double dbPrice)
{
	m_fLastClosePrice = dbPrice;	
}
void CStatisticsLine::SetVolumeMultiple(double dbVolumeMultiple)
{
	m_dbVolumeMultiple = dbVolumeMultiple;
}
int	CStatisticsLine::GetDrawScalesCount(CDC* pDC)
{
	int nCount = 0;
...
/*void CStatisticsLine::DrawAveragePriceLines(CDC* pDC)
{
	if(m_nBegin < 0 || m_nBegin >=  (int)m_vecTLine.size()
		|| m_nEnd < 0 || m_nEnd >=  (int)m_vecTLine.size())
		return;
...
int  CStatisticsLine::WeiZhi2XuHao(int nx)
{
...
int  CStatisticsLine::XuHao2WeiZhi(int nItem)
{
	if(nItem > m_nBegin || nItem < m_nEnd)
		return -1;
	StatData pTLine = m_vecTLine[nItem];
	int nLeft = Time2X(pTLine.dwTime);
	return nLeft;
}
void CStatisticsLine::OnMouseMove(UINT nFlags, CPoint point)
{
	SHitTest hitTest;
...
void CStatisticsLine::OnMouseHover(UINT nFlags, CPoint point)
{
	CWnd::OnMouseHover(nFlags, point);
}
void CStatisticsLine::OnMouseLeave()
{
	m_bMouseIn = false;
	Invalidate();
...
void CStatisticsLine::OnLButtonDown(UINT nFlags, CPoint point) 
{
	SetFocus();
	SetCapture();
	m_bMouseIn = true;
...
void CStatisticsLine::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_bLbuttonDown = false;
...
/*
void  CStatisticsLine::SItemGetValue(bool	bIsMouse, bool bIsKeyBoard)
{
...
void CStatisticsLine::DrawTips(CDC* pDC)
{
	SHitTest hitTest;
	HitTest(m_ptMouseMoveLast, hitTest);
	if(hitTest.eType == enumSplitBar)
		...
		if(m_bShowHeight)
		{
			...
			if(m_bIsKeyBoard)
			{
				if(m_nCurSel < m_nEnd || m_nCurSel >= (int)m_vecTLine.size())
					return;
				StatData pKLine = m_vecTLine[m_nCurSel];
				dbPrice = pKLine.dwUpRecv;
			}
			if(m_bIsMouse)
...
void CStatisticsLine::DrawTips_Time(CDC* pDC, CRect rtClient, int nX, bool bIsMouse)
{
...
LRESULT CStatisticsLine::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if(message == WM_KEYDOWN  )
	{
		...
		{	
			...
			if(m_nCurSel== m_vecTLine.size()-1)
			{
				...
				{
					StatData pKLine = m_vecTLine[m_nCurSel];
					double fClosePrice = pKLine.dwUpRecv;
			...
			{
				StatData pTLine = m_vecTLine[m_nCurSel];
				double fClosePrice = pTLine.dwUpRecv;
				int nHeight = JiaGe2GaoDu(fClosePrice);				
				m_ptMouseMoveLast = CPoint(nLeft, nHeight);
			}
			//SItemGetValue(false, true);
...
BOOL CStatisticsLine::PreTranslateMessage(MSG* pMsg) 
{
	return CWnd::PreTranslateMessage(pMsg);
}
void CStatisticsLine::OnSize(UINT nType, int cx, int cy)
{
	if(m_nTViewCount == 1)
		OnView1();
...
void CStatisticsLine::DrawBitmap_TVolume(CDC *pDC)
{	
	DrawScales_TVolume(pDC);
	DrawVolumeLine_TVolume(pDC);
}
void CStatisticsLine::DrawScales_TVolume(CDC* pDC)
{
	if(m_vecTimeScales.size() == 0)
		return;
...
void CStatisticsLine::CalcMaxMin_TVolume()
{/*
	m_fMaxValue_TVolume  = MIN_VALUE_DEFAULT;
	m_fMinValue_TVolume  = 0;
	for(int i = m_nBegin; i< (int)m_vecTLine.size() && i>= m_nEnd  && i>=0; i--)
...
int	CStatisticsLine::JiaGe2GaoDu_TVolume(double fJiaGe)
{
	CRect rtClient ;
	rtClient = m_rtTVolume;
...
double	CStatisticsLine::GaoDu2JiaGe_TVolume(int nHeight)
{
	CRect rtClient ;
	rtClient = m_rtTVolume;
...
void CStatisticsLine::DrawVolumeLine_TVolume(CDC* pDC)
{/*
	if(m_vecTLine.size() == 0
		|| m_nEnd < 0 || m_nEnd >= (int)m_vecTLine.size() 
...
void CStatisticsLine::DrawBitmap_TScales(CDC* pDC)
{
	if(m_vecTimeScales.size() == 0)
		return;
	...
	{  		
		...
		if(i%2 != 0 && i!= m_vecTimeScales.size()-1)
			pt.x = nX -cz.cx/2;
		else
			pt.x = nX -cz.cx;
...
struct tm CStatisticsLine::GetDrawTime(std::vector<struct tm>& vecTimeScales, std::vector<DWORD>& vecTMWidthSpan, DWORD dwSpan)
{
	struct tm tmReturn;
	tmReturn.tm_year = 0;
...
bool  CStatisticsLine::HitTest(CPoint pt, SHitTest& hitTest)
{
	CRect rtClient;
	GetClientRect(&rtClient);
...
BOOL CStatisticsLine::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CStatisticsLine::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	int n = 0;
	CWnd::OnLButtonDblClk(nFlags, point);
}
/*
BOOL CStatisticsLine::SetInTimeData(CFData& fData)
{
	SetMaxMinValue(fData.m_fMaxValue, fData.m_fMinValue);
	std::std::vector<StatData> vec;
	fData.GetVecTLine(vec);
	SetVectorData(vec);	
	
	return true;
}*/
void CStatisticsLine::OnRButtonUp(UINT nFlags, CPoint point)
{
/*	CMenu menu1;
...
void CStatisticsLine::OnToKLine()
{
	int n = 0;
