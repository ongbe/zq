...
#define LOG_INFO(fmt, ...) \
	...
	int CPatsApiSvr::GetPositionCombs2(const std::string& strTradeID, std::string& strInstrument,std::vector<PlatformStru_PositionComb>& outData)
	{
	...
	int CPatsApiSvr::GetPositionCombInfo(long lFTID, PlatformStru_PositionComb& outData)
	{
	...
	bool CPatsApiSvr::GetMACurrentAccount()
	{
		return m_bMACurrent;
	}
	void CPatsApiSvr::SetMACurrentAccount(bool bCur)
	{
		m_bMACurrent=bCur;
	}
	bool CPatsApiSvr::GetQryFinished()
	{
		return m_QryFinished;
	}
	void CPatsApiSvr::SetQryFinished(bool bFinished)
	{
