...
, m_pAccountGroupMgr( NULL )
, m_pMultiAccountMgr( NULL )
...
void CfgMgr::CreateCfgManagers()
{
	...
	m_pAccountGroupMgr = new AccountGroupMgr;
	m_pMultiAccountMgr = new MultiAccountMgr;
}
//读取配置文件
//1表示成功
...
bool CfgMgr::InitFromCFG( std::string strFilePath, bool bFromUserCfg )
{
	...
	m_pMultiAccountMgr->InitCfg(root, bFromUserCfg);
...
void CfgMgr::ClearCfg()
{
	...
	if ( NULL != m_pAccountGroupMgr )
	{
		m_pAccountGroupMgr->ClearCfg();
	}
	...
	if ( NULL != m_pMultiAccountMgr )
	{
		m_pMultiAccountMgr->ClearCfg();
	}
...
bool CfgMgr::Save()
{
	...
	if ( !m_pMultiAccountMgr->SaveCfg( root ))
	{
		return false;
	}
...
void CfgMgr::ReleaseCfgManegers()
{
	...
	SAFE_DELETE(m_pAccountGroupMgr);
	SAFE_DELETE(m_pMultiAccountMgr);
    m_bInit = false;
}
...
void CfgMgr::CalcUsableShortCutKeys()
{
	...
	m_pAccountGroupMgr->CalcUsableShortCutKeys(m_pShortCutKeysMgr);
}
...
bool CfgMgr::IsAccountGroupExist( int nGroupID )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return false;
	...
	return m_pAccountGroupMgr->IsGroupExist(nGroupID);
}

int CfgMgr::GetNextAccountGroupID()
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return 0;
	...
	return m_pAccountGroupMgr->GetNextGroupID();
}

bool CfgMgr::AddAccountGroup( int nGroupID, std::string strGroupName )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return false;
	...
	return m_pAccountGroupMgr->AddGroup(nGroupID, strGroupName);
}

bool CfgMgr::AddAccount( int nGroupID, int nAccountID, std::string strAccountName )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return false;
	...
	return m_pAccountGroupMgr->AddAccount(nGroupID, nAccountID, strAccountName);
}

bool CfgMgr::DelAccount( int nAccountID )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return false;
	...
	return m_pAccountGroupMgr->DelAccount(nAccountID);
}

void CfgMgr::GetAccountGroup(std::vector<AccountGroup>& accountGroup)
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return;
	...
	accountGroup = m_pAccountGroupMgr->GetAccountGroup();
}

void CfgMgr::SetAccountGroup( const std::vector<AccountGroup>& accountGroup )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return;
	...
	m_pAccountGroupMgr->SetAccountGroup(accountGroup);
}

void CfgMgr::SetCurAccountGroupID( int nGroupID )
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return;
	...
	m_pAccountGroupMgr->SetCurGroupID(nGroupID);
}

int CfgMgr::GetCurAccountGroupID()
{
	if ( NULL == m_pAccountGroupMgr )
	{
		return -1;
	...
	return m_pAccountGroupMgr->GetCurGroupID();
}

MULTIACCOUNT* CfgMgr::GetMultiAccountCfg()
{
	if ( NULL == m_pMultiAccountMgr )
	{
		return NULL;
	...
	return m_pMultiAccountMgr->GetMultiAccountSet();
}

void CfgMgr::GetContracts( OrderStrategy eStrategy, std::vector<std::string>& vContracts)
{
	if ( NULL == m_pMultiAccountMgr )
	{
		return;
	...
	m_pMultiAccountMgr->GetContracts(eStrategy, vContracts);
}

void CfgMgr::SetOrderStrategyData( OrderStrategy eStrategy, const std::vector<std::string>& vContracts, const std::vector<OrderStrategyRowCfg>& vStrategyData )
{
	if ( NULL == m_pMultiAccountMgr )
	{
		return;
	...
	m_pMultiAccountMgr->SetOrderStrategyData(eStrategy, vContracts, vStrategyData);
}

bool CfgMgr::GetOrderStrategyData( OrderStrategy eStrategy, const std::string strAccount, OrderStrategyRowCfg& rowCfg )
{
	if ( NULL == m_pMultiAccountMgr )
	{
		return false;
	...
	return m_pMultiAccountMgr->GetOrderStrategyData(eStrategy, strAccount, rowCfg);
}

int CfgMgr::GetOrderStrategyContractVolume( OrderStrategy eStrategy, const std::string strAccount, 
										   const std::string strContract, const std::string strContractType )
{
	...
	return m_pMultiAccountMgr->GetOrderStrategyContractVolume(eStrategy, strAccount, strContract, strContractType);
}
