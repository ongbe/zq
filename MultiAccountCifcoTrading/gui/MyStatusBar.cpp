...
m_LinkStatusDlg(NULL)
{
	...
	for(it;it!=exchnames.end();++it)
	{
        ...
        if(pSvr)
		    pStatic->SetToolTip((pSvr->ExchangeID2String(it->c_str())).c_str());
		field.m_static=pStatic;
		strTime="";
		CurTickCount=0;
        if(pSvr)
		    CurTickCount=pSvr->GetExchangeTime(it->c_str(),strTime);
...
{  
    ...
    //显示连接状态
	IPlatformSingleSvr* pSvr=g_pPlatformMgr->GetCurrentPlatformSvr();
	if(pSvr) pSvr->GetLinkStatus(&hostlink,&pricelink);
	if(hostlink<0||pricelink<0)
	{
		...
		if(pSvr) pSvr->GetCurrentStatus(&quot_status,&trader_status);
...
{
	IPlatformSingleSvr* pSvr=g_pPlatformMgr->GetCurrentPlatformSvr();
	if(pSvr==NULL) return 0;
	string strTime;
    pSvr->GetExchangeIDs(outData);//交易所数量
    vector<string>::iterator it;
    for(it=outData.begin();it!=outData.end();)
    {
        if(pSvr->GetExchangeTime(it->c_str(),strTime)==0)
            it=outData.erase(it);
        else
            ++it;
    }
