...
#include "../inc/Module-QuotFrame/QuotFrame.h"    //报价表
...
wxWindow* MainFrame::GetModuleWindow(int ModuleID)
{
	...
	case GID_QUERYLOCAL_ORDER:
		...
		break;
	case GID_MULTI_ACCOUNT://多账号模块
		pPanel=new MultiAccountPanel(m_notebook,GID_MULTI_ACCOUNT);
		break;	
	case GID_MULTI_POSITION://持仓汇总模块
	...
	case Cmd_MA_ModifyPwd_Rsp:
		{
			if ( pRecvData->head.userdata1 == OPERATOR_SUCCESS)
			{
				...
				wxMessageBox("修改密码成功", LOADSTRING(USERLOGINDLG_INFORMATION));
			}
			else
			{
				wxMessageBox(wxString::Format("修改密码失败！\n%s",(char*)(pRecvData->pData)),LOADSTRING(USERLOGINDLG_INFORMATION));
			}
		}
	case Cmd_MA_ForceCloseClient_Rsp:
		{
			wxMessageBox("此账户已被停用，您被迫下线！\n如有问题，请联系客服。", LOADSTRING(USERLOGINDLG_INFORMATION));
			m_bForceClose = true;
