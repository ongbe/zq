...
:ConfigPanelBase(parent,winid,pos,size,style,name)
{
    ...
    m_pBtnQuery = new wxButton(this, ID_BTN_QUERY, LOADSTRING(BTN_QP_BUTTONQUERY), wxPoint(320, 14+offset), wxSize(75, 25), 0);
	...
	new wxStaticText( this, wxID_STATIC,"用户选择", wxPoint(175,19+offset), wxDefaultSize, 0 );
	new wxChoice( this, ID_ChoiceUser, wxPoint(230,16+offset), wxSize(80,-1), MA_CORE()->GetAccountArray() );
	...
	m_pList->InsertColumn(index++,LOADSTRING(BANK_CODE));
	m_pList->InsertColumn(index++,LOADSTRING(SUMMARY));
}
void CJ_ChuRuJinDlg::OnUserChoice(wxCommandEvent& event)
{
	...
	if(pChoice)
	{
		...
		//订阅请求查询转帐流水响应
		if(m_pSvr) 
			m_pSvr->SubscribeBusinessData(BID_RspQryTransferSerial, GID_CHURUJIN, rspOnTransferCallBackFunc, false);
	}
...
CJ_ChuRuJinDlg::~CJ_ChuRuJinDlg()
{
	...
	for (int i=0;i<strArr.size();i++)
	{
		...
		if(m_pSvr) 
			m_pSvr->UnSubscribeBusinessData(BID_RspQryTransferSerial, GID_CHURUJIN);
	}
}
