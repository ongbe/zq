								   ...
								   m_treeBase(NULL)
{
	...
	if(CURRENT_SVR()&&CURRENT_SVR()->ContractCountIsFit())
	{
    ...
    if(CURRENT_SVR()) CURRENT_SVR()->GetInstrumentList(m_SysGroupInfo);
    Init();
    ...
    string str;
    int grpid=0;
    ((MainFrame*)GETTOPWINDOW())->GetCurInstrumentGrpInfo_QuotPanel(str,grpid);
...
void J_ZiXuanHeYueDlg::OnAdd(wxCommandEvent& event)
{
	if(m_pListSysGroup)
	{
		...
		{ 
			...
			{
				m_bChanged = true;
				if(m_pListSelfGroup->GetNumberRows()>=CURRENT_SVR()->GetGroupMaxContractNum())
				{
...
void J_ZiXuanHeYueDlg::OnTreeItemActived(wxTreeEvent& event)
{
	...
	{
		int nRow = m_pListSelfGroup->GetNumberRows();
		if( nRow >=CURRENT_SVR()->GetGroupMaxContractNum())
		{
			wxMessageBox(LOADSTRING(OVER_LIMIT_OF_GROUP),LOADSTRING(USERLOGINDLG_ERROR),wxOK|wxICON_QUESTION);
			return;
		}
...
void J_ZiXuanHeYueDlg::Init()
{
	if(m_pComSysGroup&&m_pListSysGroup)
	{
		...
		if(CURRENT_SVR()&&!CURRENT_SVR()->ContractNameIsID())   
