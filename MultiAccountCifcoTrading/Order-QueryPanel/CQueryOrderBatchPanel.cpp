...
bool CQueryOrderBatchPanel::Init()
{
	...
	nColCount = pCfgMgr->GetListColCount(GID_QUERY_ORDER_BATCH);
...
void CQueryOrderBatchPanel::ShowOrderBatchs()
{
	...
	COrderServiceThread::MAPBATCHSUBORDER mapBatchSub = m_poOrderServiceThread->GetOrderBatchSubMap();

	m_poOrderServiceThread->UnlockObject();

	COrderServiceThread::MAPBATCHSUBORDERITEM it = mapBatchSub.begin();
...
bool CQueryOrderBatchPanel::GetOrderInfoByRowNo(int RowNo, DWORD& RowID, COrderServiceThread::BATCHSUBORDER& tBatchItem)
{
	bool bHave=false;
	std::map<DWORD,int>::iterator it_RowID_Orderkey;
	std::map<int,COrderServiceThread::BATCHSUBORDER>::iterator it_OrderKey_OrderInfo;
...
//增加一条记录
//end--
int CQueryOrderBatchPanel::LoadOneOrderList(const COrderServiceThread::BATCHSUBORDER& tBatchOrder)
{
	...
	if(iRowNo<0) {	//一条新记录
		...
		m_MapOrderKey_OrderInfo.insert(std::pair<int,COrderServiceThread::BATCHSUBORDER>(tBatchOrder.nNum,tBatchOrder));
		LeaveCriticalSection(&m_CS);
...
void CQueryOrderBatchPanel::OnContext(wxContextMenuEvent& evt)
{
	...
	if(menu.GetMenuItemCount()>0)
	{
		...
		if(retcmd - ID_MENU_ADDTOMYSELF >= 0) {
			...
			COrderServiceThread::BATCHSUBORDER tOrder;
...
void CQueryOrderBatchPanel::OnRequery(wxCommandEvent& evt)
{
	...
	COrderServiceThread::MAPBATCHSUBORDERITEM it = mapBatchSub.find((int)evt.GetExtraLong());
