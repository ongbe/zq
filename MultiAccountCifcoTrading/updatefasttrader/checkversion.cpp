...
{
	bool bNeed = false;
    if( GetClientVersion(m_localVersion) && GetServerVersion(m_remoteVersion))
    {
...
//得到配置文件路径
void CheckVersion::GetPath(std::string &filePath)
{
    char localPath[256];
...
{	
	...
	GetFileName(ConfigFilename, strFileName);
	wxString strInfo;
	strInfo.Printf(wxT("下载:%s..."), strFileName);
...
bool CheckVersion::DownLoadOneFile(SFileInfo *p, string strExePath, string strLoaclPath, string strRemotePath)
{
	...
	if(p->strFilePath != "")
	{
		...
		strAppendPathSrc.Printf(_T("%s%s\\"), strLoaclPath.c_str(), p->strFilePath.c_str());//下载下来升级文件的临时路径
		CreateDir(strAppendPathSrc.c_str());
	}
	p->strFullPathExe = wxString::Format(_T("%s%s"), strExe, p->strFileName.c_str()); //已安装版本升级文件的全路径
	p->strFullPathUpdate = wxString::Format(_T("%s%s"), strAppendPathSrc, p->strFileName.c_str());//下载下来升级文件的全路径
/*这个代码先去掉，md5值通过这个函数getFileMd5Value去得有时候得到的值不对，为空。反正目前逻辑没用到，所以索性去掉
	string str;
	MD5Sum md5;
	md5.getFileMd5Value(p->strFullPathExe.c_str(), str);
	if(str == p->strFileMD5)
	{
		p->bSameMD5 = true;//MD5值一样就不下载
		wxString str = wxString::Format(_T("The value of md5 file are the same, do not need to download file [%s] to [%s]!\n"), p->strFullPathRemote.c_str(), p->strFullPathExe.c_str());
		OutputDebugString(str.c_str());		
		return true;
	}
*/

	if (Internet::InternetGetURL(p->strFullPathRemote.c_str(), p->strFullPathUpdate.c_str(), NULL, NULL)
		!= Internet::INTERNET_SUCCESS)
	{
		wxString str = wxString::Format(_T("DownLoad file from [%s] to [%s] error!\n"), p->strFullPathRemote.c_str(), p->strFullPathUpdate.c_str());
		OutputDebugString(str.c_str());			
		return false;
	}
...
/*
	...
	wxString str;
	if(bNeed)
		str.Printf("中国国际期货交易终端有新版本，版本号为：%s(本地版本号为：%s)\n因为本地安装的程序版本过低，升级后，您所做过的配置文件将丢失。\n是否进行替代安装?\n\n(备注：安装前会关闭正在运行的交易终端)\n\n%s",m_remoteVersion.c_str(),m_localVersion.c_str(), m_strVersionInfo);
	else
		str.Printf("中国国际期货交易终端有新版本，版本号为：%s(本地版本号为：%s)\n是否进行替代安装?\n\n(备注：安装前会关闭正在运行的交易终端)\n\n%s",m_remoteVersion.c_str(),m_localVersion.c_str(), m_strVersionInfo);
	iret=wxMessageBox(str,"提示",wxYES_NO);
	if(iret==wxNO)
		return false;		
*/
	int errcode=0;    
	...
	/*	DWORD dwProcID = atol(wxGetApp().argv[1]);
		HANDLE handle = ::OpenProcess( PROCESS_TERMINATE, FALSE, dwProcID );
		if ( NULL != handle )
		{
			::TerminateProcess(handle, 0);
            CloseHandle(handle);
		}
		//::wxKill(atol(wxGetApp().argv[1]));

        Sleep(4000);

        handle = ::OpenProcess( PROCESS_TERMINATE, FALSE, dwProcID );
        if(handle!=NULL)
        {
            CloseHandle(handle);
            wxMessageBox("无法关闭正在运行的交易终端，请关闭交易终端，下载安装程序，手动安装！","自动升级失败",wxOK);
            return false;
        }*/
	}
	...
	strInfo = wxT("升级成功完成!");
	ProcessMsg(strInfo, 1);
//	::MessageBox(NULL,  _T("升级成功"), _T("提示"),0);
	return true;
}
