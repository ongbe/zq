...
void zqAuiNotebook::TabAddPage(vector<wxWindow*>& parentvec)
{
	...
	{
		int stockindex=pTabGroup[index];
		if(stockindex>0)
		{
			wxWindow* win=NULL;
			wxString caption;	
...
{
//	wxWindow* pParent=FindWindowById(GID_QUOT_TABLE,this);
//	wxCommandEvent evt(wxEVT_PLUGIN_NEWQUOTE);
//    evt.SetString(AppID.c_str());
//	evt.SetInt(1);
    return ((MainFrame*)GETTOPWINDOW())->Plugin_New_Del_Panel_QuotFrame(1);

//	pParent->ProcessEvent(evt);
//	return (wxWindow*)evt.GetExtraLong();
}
...
//	evt.SetInt(2);
//	pParent->ProcessEvent(evt);
    ((MainFrame*)GETTOPWINDOW())->Plugin_New_Del_Panel_QuotFrame(2);
    return true;
}
