...
void FundAccountPats::OnSubscrible(wxCommandEvent& evt)
{
	if(evt.GetInt())
	{
		...
		if(DEFAULT_SVR()) DEFAULT_SVR()->SubscribeBusinessData(BID_RspOrderAction1, GID_ACCOUNT, AllCallBackFunc);
		if(DEFAULT_SVR()) DEFAULT_SVR()->SubscribeBusinessData(BID_RspOrderAction2, GID_ACCOUNT, AllCallBackFunc);
	...
	else
	{
		...
		if(DEFAULT_SVR()) DEFAULT_SVR()->UnSubscribeBusinessData(BID_RspOrderAction1, GID_ACCOUNT);
		if(DEFAULT_SVR()) DEFAULT_SVR()->UnSubscribeBusinessData(BID_RspOrderAction2, GID_ACCOUNT);
...
void FundAccountPats::OnAllCallBack(wxCommandEvent& event)
{
	...
	PlatformStru_TradingAccountInfo NewAccountInfo={0};
