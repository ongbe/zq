...
//	WXLOG_INFO( wxT("0x%lx MouseHandyInputPaneX::OnButton ID %d"), long(this), event.GetId());
	...
	// 获取合约列表
	vector<GroupInfo>::iterator itgroup;
	vector<InstrumentInfo> vec;
	vector<InstrumentInfo>::iterator itvec;
	GroupInfo tGroup;
	InstrumentInfo tInfo;
...
void MouseHandyInputPaneX::MatchInstrumentByCode( const std::string &strCode, bool bExactMatch, std::set<std::string>& setShowText )
{
	...
	for(unsigned int i=0; i < m_poGroup->size(); i++) 
	{
		GroupInfo tGroup = (*m_poGroup)[i];
		vector<InstrumentInfo> vec = tGroup.InstrVec;
		for(unsigned int j=0; j < vec.size(); j++) 
		{
			InstrumentInfo tInfo = vec[j];
...
void MouseHandyInputPaneX::SetCodeGroup( vector<GroupInfo>* poGroup )
{
	m_poGroup = poGroup;
	for(unsigned int i=0; i < m_poGroup->size(); i++) 
	{
