...
CTraderSpi::CTraderSpi(CPlatFormService& RefPlatFormService,CQryQueue* RefQryQueue)
:
...
/*
        PlatformStru_QryCFMMCTradingAccountKey req;
        memset(&req,0,sizeof(req));
        strcpy(req.Thost.BrokerID, m_PlatformParam.BrokerID.c_str());
        strcpy(req.Thost.InvestorID, m_PlatformParam.InvestorID.c_str());

        int iRequestID=m_PlatformParam.RequestID++;
        int iRlt = m_RefTraderApi.ReqQryCFMMCTradingAccountKey(&req.Thost, iRequestID);

        LOG_INFO("OnRspSettlementInfoConfirm(投资者结算结果确认响应)\n"
                        "\t\t\t --->ReqQryCFMMCTradingAccountKey(请求查询保证金监管系统经纪公司资金账户密钥):ret=[%d],iRequestID=[%d],\n"
                        "\t\t\t BrokerID=[%s],\t InvestorID=[%s]",
                        iRlt, iRequestID, req.Thost.BrokerID, req.Thost.InvestorID); 
*/
    }
...
//	    ShellExecute(NULL, _T("open"), _T("explorer.exe"), Urlbuf, NULL, SW_SHOW);
	    ShellExecute(NULL, _T("open"), Urlbuf, "",NULL, SW_SHOW);
...
{
    if(bIsLast&&m_pQryQueue)  m_pQryQueue->ClearbInQry();
...
void CTraderSpi::OnRspQryParkedOrderAction(CThostFtdcParkedOrderActionField *pParkedOrderAction, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {
        LOG_INFO("OnRspQryParkedOrderAction(请求查询预埋撤单响应) : ErrorID=[%d],ErrorMsg=[%s],nRequestID=[%d],bIsLast=[%d],\n"
			"\t\t\t BrokerID=[%s],\t InvestorID=[%s],\t OrderActionRef=[%],\t OrderRef=[%s],\t RequestID=[%d],\n"
...
void CTraderSpi::OnRspQryOrder(CThostFtdcOrderField *pOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {
		CopyOrderField(tmp.OrderField,*pOrder);
    }
...
void CTraderSpi::OnRspQryTrade(CThostFtdcTradeField *pTrade, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {    
        CopyTradeRecordField( tmp.TradeField, *pTrade);
    }
...
void CTraderSpi::OnRspQryInstrumentMarginRate(CThostFtdcInstrumentMarginRateField *pInstrumentMarginRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {
        tmp.InstrumentMarginRateField = *((PlatformStru_InstrumentMarginRate*)pInstrumentMarginRate);
    }
...
void CTraderSpi::OnRspQryInstrumentCommissionRate(CThostFtdcInstrumentCommissionRateField *pInstrumentCommissionRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {
        tmp.InstrumentCommissionRateField = *((PlatformStru_InstrumentCommissionRate*)pInstrumentCommissionRate);
    }
...
//修复大商所 组合 合约乘数为0的问题
    ...
    tmp.nRequestID = nRequestID;
    tmp.bIsLast    = bIsLast;
	CopyInstrumentField( tmp.InstrumentField, *pInstrument);
    if(pRspInfo)
        tmp.RspInfoField = *pRspInfo;
...
//    {
//		std::string strID=tmp.InstrumentField.InstrumentID;
//		int idpos=strID.find(" ");
//		if(idpos>=0)
//		{
//			strID=strID.substr(idpos+1);
//			idpos=strID.find("&");
//			if(idpos>=0) strID.replace(idpos,1,"/");
//		}
//		memcpy(tmp.InstrumentField.InstrumentID,strID.c_str(),strID.size()+1);
//    }	
...
void CTraderSpi::OnRspQryInvestorPositionCombineDetail(CThostFtdcInvestorPositionCombineDetailField *pInvestorPositionCombineDetail, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspQryInvestorPositionComb tmp;
    tmp.nRequestID = nRequestID;
    tmp.bIsLast    = bIsLast;    
    if(pRspInfo)
        tmp.RspInfoField = *pRspInfo;
    ...
    {
        LOG_INFO("OnRspQryInvestorPositionCombineDetail(请求查询投资者持仓组合合约明细响应),pInvestorPositionCombineDetail is NULL");
        if(tmp.RspInfoField.ErrorID==0)
        {
            memset(&tmp.RspInfoField, 0, sizeof(tmp.RspInfoField));
            tmp.RspInfoField.ErrorID=CustomErrorID;
    ...
    {
		CopyPositionCombField( tmp.InvestorPositionCombineDetailField, *pInvestorPositionCombineDetail);
    }

    m_RefPackageCache.SaveDataTo(BID_RspQryInvestorPositionComb, 0, 0, 0, sizeof(tmp), &tmp); 

    //log
    if(tmp.RspInfoField.ErrorID!=0)
    {
        LOG_INFO("OnRspQryInvestorPositionCombineDetail(请求查询投资者持仓组合合约明细响应) : ErrorID=[%d],ErrorMsg=[%s],nRequestID=[%d],bIsLast=[%d]",
                         tmp.RspInfoField.ErrorID, tmp.RspInfoField.ErrorMsg,nRequestID,bIsLast
                         );
    }
    else
    {
		...
		LOG_INFO("OnRspQryInvestorPositionCombineDetail(请求查询投资者持仓组合合约明细响应) : ErrorID=[%d],ErrorMsg=[%s],RequestID=[%d],bIsLast=[%d],\n"
    ...
    LOG_INFO("OnRspQryInvestorPositionCombineDetail(请求查询投资者组合持仓明细响应)");
}
...
{
    if(bIsLast&&m_pQryQueue)  m_pQryQueue->ClearbInQry();
    DataRspQryNotice tmp;
    tmp.nRequestID=nRequestID;
    tmp.bIsLast=bIsLast;
    if(pNotice) tmp.NoticeField=*pNotice;
    if(pRspInfo) tmp.RspInfoField=*pRspInfo;
...
void CTraderSpi::OnRspQrySettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	m_PlatformParam.TradeStatus=CPlatFormParam::CONNSTATUS_Connected;
    DataRspQrySettlementInfoConfirm tmp;
	tmp.nRequestID=nRequestID;
	tmp.bIsLast=bIsLast;
	if(pSettlementInfoConfirm) tmp.SettlementInfoConfirm=*pSettlementInfoConfirm;
    if(pRspInfo) tmp.RspInfoField=*pRspInfo;
...
void CTraderSpi::OnRspQryAccountregister(CThostFtdcAccountregisterField *pAccountregister, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    if(!pAccountregister)
    {
        LOG_INFO("OnRspQryAccountregister(请求查询银期签约关系响应),pAccountregister is NULL ErrorID=%d bIsLast=%d",
            tmp.RspInfoField.ErrorID,bIsLast);
        if(tmp.RspInfoField.ErrorID==0)
        {
            memset(&tmp.RspInfoField, 0, sizeof(tmp.RspInfoField));
            tmp.RspInfoField.ErrorID=CustomErrorID;
    ...
    {
        LOG_INFO("OnRspQryAccountregister(请求查询银期签约关系响应),bIsLast=%d\n"
            "\t\t\tTradeDay=%s BankID=%s BankBranchID=%s BankAccount=%s",
            bIsLast,pAccountregister->TradeDay,pAccountregister->BankID,pAccountregister->BankBranchID,pAccountregister->BankAccount);
...
void CTraderSpi::CopyPositionCombField( PlatformStru_PositionComb& data, const CThostFtdcInvestorPositionCombineDetailField& positionCombField)
{
