...
//	oldfont.SetWeight(wxBOLD);
	...
	m_listBox = new wxListCtrl(this, ID_PopupList, 
		wxPoint(0,0), wxSize(nBWidth*3, 20*((m_nCodeHeadsCount+2)/3)),wxLC_REPORT|wxLC_NO_HEADER|wxLC_SINGLE_SEL);
	...
	if(DEFAULT_SVR()->IsMultipleAccounts())
		...
		pSvr = DEFAULT_SVR();
	if(pSvr!=NULL) {
		pSvr->GetInstrumentList(outData);
	...
	for(int i=0; i<m_nCodeHeadsCount; i++) {
		new wxButton(this, ID_PopupButton+i, wxT(m_ptCodeHeads[i].strCodeName), 
				wxPoint(1+(i%3)*nBWidth, 1+(i/3)*20), wxSize(nBWidth-1, 20));
	}
