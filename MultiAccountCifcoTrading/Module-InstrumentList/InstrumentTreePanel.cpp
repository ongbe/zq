...
BEGIN_EVENT_TABLE(InstrumentTreePanel,wxPanel)
	...
	EVT_COMMAND(wxID_ANY, wxEVT_INSTRUMENT_LIST_COMMISSIONRATE_UPDATE,  OnUpdateForProduct)
...
void InstrumentTreePanel::InitializeItem()
{
	...
	for(int exch_index=0;exch_index<(int)exchangeIDset.size();exch_index++)
	{
		...
		ProductIdMap.clear();
		if(DEFAULT_SVR()) DEFAULT_SVR()->GetProductID(ProductIdMap,exchangeIDset[exch_index]);
...
void InstrumentTreePanel::OnUpdateForProduct(wxCommandEvent& evt)
{
	...
	int ret=DEFAULT_SVR()?DEFAULT_SVR()->GetInstrumentListByProductID(EvtProduct,InstrumentList):-1;
	if(ret>0)
	{//是合约品种
...
void InstrumentTreePanel::UpdateForInstrument(std::string EvtParamStr)
{
...
//    wxString strInstrumentID = evt.GetString();
	...
	PlatformStru_InstrumentMarginRate margField;
	memset(&margField,0,sizeof(margField));
    int ret_Comm=DEFAULT_SVR()?DEFAULT_SVR()->GetCommissionRate(strInstrumentID.c_str(), commField):-1;
    int ret_Marg=DEFAULT_SVR()?DEFAULT_SVR()->GetMarginRate(strInstrumentID.c_str(),margField):-1;
	CfgMgr *pCfgMgr = CFG_MGR_DEFAULT();  
	wxASSERT(pCfgMgr);
	int nColCount = pCfgMgr->GetListVisibleColCount(GID_INSTRUMENT);
	for(int i = 0; i < nColCount; i++)           //设置各列的信息
