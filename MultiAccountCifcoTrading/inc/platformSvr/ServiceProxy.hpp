...
#include "QryQueue.h"
#ifdef _DEBUG
	#pragma comment(lib, "platformSvrD.lib")
#else
	#pragma comment(lib, "platformSvr.lib")
#endif
...
public:
	PTYPE GetPlatformType(){return PTYPE_CTP;}
    ///启动平台
    int Start();
	...
	//判断是否支持银期转账
	bool HaveBankFutureTransfer(){return true;}
...
public:
 //   static CServiceProxy& GetSingleInstance();
    ...
    CServiceProxy();
	CQryQueue* GetQryQueue(){return m_pQryQueue;}
