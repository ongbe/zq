...
public:
    ...
    void NotifyStartStopInitQry(bool bStart) { m_bDoingInitQry=bStart?true:false; }


private:

    CPackageCache *m_pReqQueue;                             //优先查询队列。队列按先进先出顺序查询
    CPackageCache *m_pReqQueue2;                            //次级查询队列。队列按先进先出顺序查询    
	...
	//解决ACCOUNTS-140
	char m_SendQryCmdBuf[1*1024*1024];
    CPlatFormService *m_pPlatFormService;

    bool m_bInQry;                                   //是否在查询途中
    bool m_bDoingInitQry;                            //正在进行初始化查询，此时不能查费率
