...
public:
	...
	static DWORD TradeWorkThreadFunc( void* pData );
	static DWORD QuotWorkThreadFunc( void* pData );
	void SetQryPrior( int QryPrior );
	void OnRtnTrade( const std::string& strBrokerID,const std::string& strInvestorID,
		const std::string& strInstrumentID,bool bPositionComb,int RecordCountOfPositionDetail );
	void CheckOnTime();
private:
    ...
    HANDLE					m_hTradeWorkThread; //交易工作线程
    bool 					m_bTradeWorkThreadExit; //交易工作线程
    HANDLE		            m_hQuotWorkThread;  //行情工作线程
    bool 					m_bQuotWorkThreadExit; //交易工作线程
	...
	CReqOnRtnTrade*			m_ReqOnRtnTrade;
