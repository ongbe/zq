...
BEGIN_EVENT_TABLE(CKTechIndex, CKCtrl)
	...
	//EVT_LEFT_DOWN(CKTechIndex::OnLeftDown)
	//EVT_LEFT_UP(CKTechIndex::OnLeftUp)
	//EVT_RIGHT_DOWN(CKTechIndex::OnRightDown)
	//EVT_RIGHT_UP(CKTechIndex::OnRightUp)
END_EVENT_TABLE()
...
/*
void CKTechIndex::OnLeftDown(wxMouseEvent& event)
{
	SetFocus();
...
void CKTechIndex::OnLeftUp(wxMouseEvent& event)
{
	wxPoint pt = wxGetMousePosition(); 
	ScreenToClient(&pt.x, &pt.y);
...
void CKTechIndex::OnRightDown(wxMouseEvent& event)
{
	SetFocus();
	wxPoint pt = wxGetMousePosition(); 
	ScreenToClient(&pt.x, &pt.y);
...
void CKTechIndex::OnRightUp(wxMouseEvent& event)
{
	SPoint *pSel = NULL;
	MAPsmark::iterator it = m_mapMarks.begin();
	while(it != m_mapMarks.end())
