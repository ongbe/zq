...
void CTradeInfoDlg::SetMessage( const wxString& title, const wxString& info, int id)
{
	if(id<0 || m_setSysID.find(id)==m_setSysID.end()) {
		m_TitleArray.Add(title);
		m_InfoArray.Add(info);
		WxStaticText1->SetLabel(info);
...
void CTradeInfoDlg::ShowTradeMsg(const wxString& strInfo, int nInfoId, const wxString& strTitle, int nTitleId, BOOL bIsShowDlg)
{
	...
	if(bIsShowDlg) {
		//SetInfo(strTempInfo, nInfoId);
		//SetTitle(strTitle, nTitleId);
		SetMessage(strTitle, strTempInfo, nInfoId);
		ShowCheckConfig();
		Show(true);
	}
}
