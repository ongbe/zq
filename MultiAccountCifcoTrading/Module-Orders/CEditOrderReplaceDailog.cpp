									  ...
									  PlatformStru_InputOrder& tftdcOrderInputField)
{
	...
	memcpy(tftdcOrderInputField.strAccount, tftdcOrderField.szAccount, sizeof(tftdcOrderInputField.strAccount));					// 交易账号
...
void CEditOrderReplaceDailog::ShowOrderReq()
{
	...
	strValue.Printf("%s", m_tftdcOrderField.szAccount);
...
void CEditOrderReplaceDailog::OnOk(wxCommandEvent& event)
{
	...
	strText = GetTraderAccount();
	strncpy(m_tftdcOrderField.szAccount, strText.c_str(), strText.size()<20 ? strText.size() : 20);
