	...
	m_bIsPause(FALSE)
{
	DIALOG_CREATE_LOG();
    AUTO_SETSIZE();
	COrderServiceThread::sm_nBatchOrderNum++;
	m_nGroupNum = COrderServiceThread::sm_nBatchOrderNum;
...
void COrderBatchDailog::OnTimer(wxTimerEvent& event)
{
	...
	//tmpData.InputOrderField=m_OrderReq;
//	CThostFtdcInputOrderField& req=tmpData.InputOrderField;
	...
	if(m_nTimerOut<=0) {
		...
		if(!bPriceIsInvalid) {
			...
			COrderServiceThread::BATCHSUBORDER tOrder={0};
			...
			if(m_poOrderServiceThread) {
				...
				m_poOrderServiceThread->OrderBatchSubAdd(tOrder);
