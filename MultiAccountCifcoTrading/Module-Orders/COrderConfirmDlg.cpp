...
void COrderConfirmDlg::CreateGUIControls()
{
	...
	//if(PLATFORM_SVR().HaveOrderType(UIOT_ACCOUNT)) {
...
bool COrderConfirmDlg::Show(bool show)
{
	if(show) {
		if(m_nCmbOrderCount>0) {
			CreateGUIControls();
			//m_pStaticCancelInfo->SetLabel(m_strCancelInfo);
			//for(int i=0; i<m_nDataRspOrderInsertCount; i++) {
			//	CThostFtdcInputOrderField& ftdcField=m_pDataRspOrderInserts[i]->InputOrderField;
			//    m_poOrderConfirmPanels[i] = new COrderConfirmPanel(
			//			this, wxID_ANY, wxPoint(0, 100+22*i), wxSize(520, 22));
			//	m_poOrderConfirmPanels[i]->SetPriceTick(m_fPriceTick);
			//	m_poOrderConfirmPanels[i]->SetIsAuto(m_bIsAuto);
			//	m_poOrderConfirmPanels[i]->ShowOrder(ftdcField);
			//	//m_pBookCtrl->AddPage(m_poOrderConfirmPanels[i], 
			//	//		wxString::Format("%s", ftdcField.CombOffsetFlag[0]==THOST_FTDC_OF_Open ? 
			//	//		_("开仓") : 
