...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if ( IsFocused() || IsDropHighlighted() )
    {
        ...
        if (pGrid->GetFrameFocusCell() && pGrid->GetShowFocusRect())
        {
            ...
            TRY 
            {
				...
				CBrush brush((~GetBackClr())&0xFFFFFF);
    ...
    else if ((GetState() & GVIS_SELECTED))
    {
        ...
        pDC->FillSolidRect(rect, RGB(192, 192, 255));
        rect.right--; rect.bottom--;
        //pDC->SetTextColor(::GetSysColor(COLOR_HIGHLIGHTTEXT));
		pDC->SetTextColor(GetTextClr());
    }
    else
    {
        if (bEraseBkgnd)
