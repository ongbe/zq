...
LRESULT CGridCtrl::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CGridCtrl::DrawRect(CDC* pDc, CRect& rect, CPen& pen)
{
...
void CGridCtrl::OnNcPaint()
{
	...
	if(pWinDC) 
	{
		...
		CPen penCurr;
		penCurr.CreatePen(PS_SOLID, 1, RGB(192, 192, 192));
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
    ...
    {
    case SB_LINERIGHT:
		TRACE("CGridCtrl::OnHScroll() SB_LINERIGHT: %d\n", ::GetTickCount());
    ...
    case SB_LINELEFT:
		TRACE("CGridCtrl::OnHScroll() SB_LINELEFT: %d\n", ::GetTickCount());
    ...
    case SB_PAGERIGHT:
		TRACE("CGridCtrl::OnHScroll() SB_PAGERIGHT: %d\n", ::GetTickCount());
        if (scrollPos < m_nHScrollMax)
        {
            rect.left = GetFixedColumnWidth();
    ...
    case SB_PAGELEFT:
		TRACE("CGridCtrl::OnHScroll() SB_PAGELEFT: %d\n", ::GetTickCount());
        if (scrollPos > 0)
        {
            rect.left = GetFixedColumnWidth();
            int offset = -rect.Width();
    ...
    case SB_THUMBTRACK:
		TRACE("CGridCtrl::OnHScroll() SB_THUMBPOSITION/SB_THUMBTRACK: %d\n", ::GetTickCount());
        {
    ...
    case SB_LEFT:
		TRACE("CGridCtrl::OnHScroll() SB_LEFT: %d\n", ::GetTickCount());
        if (scrollPos > 0)
        {
            SetScrollPos32(SB_HORZ, 0);
            Invalidate();
    ...
    case SB_RIGHT:
		TRACE("CGridCtrl::OnHScroll() SB_RIGHT: %d\n", ::GetTickCount());
        if (scrollPos < m_nHScrollMax)
        {
            SetScrollPos32(SB_HORZ, m_nHScrollMax);
            Invalidate();
    ...
    default: 
		TRACE("CGridCtrl::OnHScroll() default: %d\n", ::GetTickCount());
        break;
    }
}
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
    ...
    {
    case SB_LINEDOWN:
		TRACE("CGridCtrl::OnVScroll() SB_LINEDOWN: %d\n", ::GetTickCount());
    ...
    case SB_LINEUP:
 		TRACE("CGridCtrl::OnVScroll() SB_LINEUP: %d\n", ::GetTickCount());
    ...
    case SB_PAGEDOWN:
 		TRACE("CGridCtrl::OnVScroll() SB_PAGEDOWN: %d\n", ::GetTickCount());
        if (scrollPos < m_nVScrollMax)
        {
            rect.top = GetFixedRowHeight();
    ...
    case SB_PAGEUP:
 		TRACE("CGridCtrl::OnVScroll() SB_PAGEUP: %d\n", ::GetTickCount());
        if (scrollPos > 0)
        {
            rect.top = GetFixedRowHeight();
    ...
    case SB_THUMBPOSITION:
    case SB_THUMBTRACK:
 		TRACE("CGridCtrl::OnVScroll() SB_THUMBTRACK/SB_THUMBPOSITION: %d\n", ::GetTickCount());
       {
    ...
    case SB_TOP:
		TRACE("CGridCtrl::OnVScroll() SB_TOP: %d\n", ::GetTickCount());
        if (scrollPos > 0)
        {
    ...
    case SB_BOTTOM:
		TRACE("CGridCtrl::OnVScroll() SB_BOTTOM: %d\n", ::GetTickCount());
        if (scrollPos < m_nVScrollMax)
        {
    ...
    default: 
		TRACE("CGridCtrl::OnVScroll() default: %d\n", ::GetTickCount());
        break;
    }
}
...
// 2012.3.12，杨桂明改进，添加一个只针对数值排序的函数
// Sorts on a given column using the cell numeric
BOOL CGridCtrl::SortNumericItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
...
// 2012.3.13，杨桂明改进，数值比较有整形转换成双精度浮点型
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	//else
	//	return 1;
	double fValue1 = _tstof(pCell1->GetText());
	double fValue2 = _tstof(pCell2->GetText());
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    if( !IsValid( cell) )
    {
        SendMessageToParent(-1, -1, NM_DBLCLK);
        //ASSERT(FALSE);
        return;
    }
