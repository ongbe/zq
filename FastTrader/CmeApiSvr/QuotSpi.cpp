...
void CQuotSpi::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    if(pRspInfo->ErrorID==0)
    {
        ...
        //todo 根据合约定制行情
        std::set<std::string>::iterator iter=m_RefCPlatFormService.GetMarketDataInstrumentID().begin();
        for(iter;iter!=m_RefCPlatFormService.GetMarketDataInstrumentID().end();++iter)
        {
