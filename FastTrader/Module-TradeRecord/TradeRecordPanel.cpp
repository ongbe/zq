...
void TradeRecordPanel::OnSubscrible(wxCommandEvent& evt)
{
    ...
    for(int i=0;i<(int)pSvrs.size();i++)
    {
	    if(evt.GetInt())
	    {
		    pSvrs[i]->SubscribeBusinessData(BID_RspQryTrade, GID_THRADE_RECORD, RspQryTradeCallBackFunc, true);
		    pSvrs[i]->SubscribeBusinessData(BID_RspQryInstrumentCommissionRate, GID_THRADE_RECORD, GetCommissionRateCallBackFunc);
		    pSvrs[i]->SubscribeBusinessData(BID_RtnTrade, GID_THRADE_RECORD, RtnTradeCallBackFunc, true);
	    }
	    else
	    {
		    pSvrs[i]->UnSubscribeBusinessData(BID_RspQryTrade, GID_THRADE_RECORD);
		    pSvrs[i]->UnSubscribeBusinessData(BID_RspQryInstrumentCommissionRate, GID_THRADE_RECORD);
		    pSvrs[i]->UnSubscribeBusinessData(BID_RtnTrade, GID_THRADE_RECORD);
	    }
    }
}
...
void TradeRecordPanel::OnGetCommissionRate(wxCommandEvent& evt)
{
    ...
    vector<vector<PlatformStru_TradeTotalInfo>> vecTradeTotals;
    vector<vector<PlatformStru_TradeInfo>> vecTrades;
	int iret;
	...
	iret=DEFAULT_SVR()->GetInstrumentListByProductID(strInstru,setInstruments);
	...
	{//是品种代码
        ...
        {
            vecTrades.clear();
            vecTradeTotals.clear();
            for(int i=0;i<(int)pSvrs.size();i++)
            {
                vecTrades.push_back(vector<PlatformStru_TradeInfo>());
                vecTradeTotals.push_back(vector<PlatformStru_TradeTotalInfo>());

                pSvrs[i]->GetTradesOfInstrument(*it,vecTrades[i]);
                pSvrs[i]->GetTradeTotalOfInstrument(*it,vecTradeTotals[i]);
            }

            for(int i=1;i<(int)vecTrades.size();i++)
                vecTrades[0].insert(vecTrades[0].end(),vecTrades[i].begin(),vecTrades[i].end());
            for(int i=1;i<(int)vecTradeTotals.size();i++)
                vecTradeTotals[0].insert(vecTradeTotals[0].end(),vecTradeTotals[i].begin(),vecTradeTotals[i].end());

            if(m_pListDetail&&vecTrades.size()>0)  m_pListDetail->UpdateInstrumentItems3(vecTrades[0]);
            if(m_pListTotal&&vecTradeTotals.size()>0)  m_pListTotal->UpdateInstrumentItems3(vecTradeTotals[0]);
	...
	{//是合约ID
        ...
        for(int i=0;i<(int)pSvrs.size();i++)
        {
            ...
            pSvrs[i]->GetTradesOfInstrument(strInstru,vecTrades[i]);
            pSvrs[i]->GetTradeTotalOfInstrument(strInstru,vecTradeTotals[i]);
        }
        ...
        if(m_pListDetail&&vecTrades[0].size()>0)  m_pListDetail->UpdateInstrumentItems3(vecTrades[0]);
        if(m_pListTotal&&vecTradeTotals[0].size()>0)  m_pListTotal->UpdateInstrumentItems3(vecTradeTotals[0]);
...
void TradeRecordPanel::OnRcvTrade(wxCommandEvent& evt)
{
    ...
    TradeKey tmpTradeKey(RtnTrade.TradeField);
    ...
    std::string strInstrument(RtnTrade.TradeField.InstrumentID);
    std::vector<PlatformStru_TradeTotalInfo> vecTradeTotal;

    if(pSvr->GetTradeTotalOfInstrument(strInstrument,vecTradeTotal)>0)
        if(m_pListTotal) m_pListTotal->UpdateInstrumentItems3(vecTradeTotal);
}
...
{
    vector<vector<PlatformStru_TradeInfo>> AllTrades;
    vector<vector<PlatformStru_TradeTotalInfo>> AllTradeTotals;
    ...
    for(int i=0;i<(int)pSvrs.size();i++)
    {
        AllTrades.push_back(vector<PlatformStru_TradeInfo>());
        AllTradeTotals.push_back(vector<PlatformStru_TradeTotalInfo>());

        pSvrs[i]->GetAllTrades(AllTrades[i]);
        pSvrs[i]->GetAllTradeTotals(AllTradeTotals[i]);
    }

    for(int i=1;i<(int)AllTrades.size();i++)
        AllTrades[0].insert(AllTrades[0].end(),AllTrades[i].begin(),AllTrades[i].end());
    for(int i=1;i<(int)AllTradeTotals.size();i++)
        AllTradeTotals[0].insert(AllTradeTotals[0].end(),AllTradeTotals[i].begin(),AllTradeTotals[i].end());

    if(m_pListDetail&&AllTrades.size()>0) m_pListDetail->UpdateAllItems(AllTrades[0]);
    if(m_pListTotal&&AllTradeTotals.size()>0) m_pListTotal->UpdateAllItems(AllTradeTotals[0]);
}
...
void TradeRecordPanel::OnReConnectQry(wxCommandEvent& evt)
{
	if(evt.GetInt()==1)//获取需要刷新的总数
	{
        ...
        int cnt=0;
        vector<PlatformStru_TradeInfo> AllTrades;
        for(int i=0;i<(int)pSvrs.size();i++)
        {
            pSvrs[i]->GetAllTrades(AllTrades);
            cnt+=AllTrades.size();
        }
		evt.SetExtraLong(cnt);
	}
	else
	{
		OnReQry(evt);
...
#define UpdateCol_Price2(FieldName)  \
            if(!bnewline&&pOld->FieldName==pNew->FieldName) \
                return false;\
            if(pNew->FieldName==util::GetIntInvalidValue()) outItem.SetText("-");\
            else \
            {\
			    ...
			    if(DEFAULT_SVR() != NULL && DEFAULT_SVR()->GetInstrumentInfo(string(pNew->InstrumentID), instInfo)==0)\
                    outItem.SetText(Price2String(pNew->FieldName,instInfo.PriceTick));\
                else outItem.SetText(Price2String(pNew->FieldName,0.01));\
            }\
            outItem.SetdColData(pNew->FieldName);
