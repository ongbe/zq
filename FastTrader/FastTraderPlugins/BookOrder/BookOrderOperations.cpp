...
void CBookOrderOperations::QueryTradeByInstrument(CString& strInstrumentID)
{
	m_mapTradeNumByPrice_Buy.clear();
	m_mapTradeNumByPrice_Sell.clear();
	PlusinAPI* poPlusinApi = GetPlusinAPI();
	if(poPlusinApi!=NULL) 
	{
		int nSeqNum = CSequenceManager::GetNextSeqNum();
		...
		objSeqMng.RegisterSeqNum(nSeqNum, this);
		UnlockGlobalObject();
		poPlusinApi->Send(CMDID_TRADEINFOBRIEF_LIST_QUERY, (LPVOID)(LPCSTR)strInstrumentID, strInstrumentID.GetLength()+1, nSeqNum);
	}
}
...
void CBookOrderOperations::NewOrder(CString& strInstrumentID, BOOL bDirection, int nOCMode, 
									double fLimitPrice, int nVolume, BOOL bIsMarket)
{
	PlusinAPI* poPlusinApi = GetPlusinAPI();
	if(poPlusinApi!=NULL) 
	{
		...
		MakeReq_Order(tInputOrder, strInstrumentID, m_strCurrSubAccount, OrderType_FT_Immediately, 
				bDirection, nOCMode, nVolume, fLimitPrice, 0, FALSE, bIsMarket, FALSE);
		int nSeqNum = CSequenceManager::GetNextSeqNum();
		LockGlobalObject();
		CSequenceManager& objSeqMng = GetSequenceManager();
		objSeqMng.RegisterSeqNum(nSeqNum, this);
...
void CBookOrderOperations::CancelAllOrder(CString& strInstrumentID)
{
	...
	if(poPlusinApi!=NULL) 
	{
		...
		PlusinAcctInstrumentKey tKey;
		ZeroMemory(&tKey, sizeof(tKey));
		strncpy(tKey.szAccountID, m_strCurrAccount, sizeof(tKey.szAccountID)-1);
		strncpy(tKey.szSubAccountID, m_strCurrSubAccount, sizeof(tKey.szSubAccountID)-1);
		strncpy(tKey.szInstrumentID, strInstrumentID, sizeof(tKey.szInstrumentID)-1);
		LockGlobalObject();
		CSequenceManager& objSeqMng = GetSequenceManager();
		objSeqMng.RegisterSeqNum(nSeqNum, this);
		UnlockGlobalObject();
