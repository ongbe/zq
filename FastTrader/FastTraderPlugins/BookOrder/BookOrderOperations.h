...
public:
	...
	enum {	conColBuyTrade, conColBuyCondition, conColBuyVolumeWait, conColBidVolume, conColPrice, 
			conColAskVolume, conColSellVolumeWait, conColSellCondition, conColSellTrade, conPriceColCount };
	enum {	conActionNothing, conActionOpen, conActionCloseToday, conActionClose };
	enum {	conPositionColCount = 7 };
...
protected:
	...
	void QueryTradeByInstrument(CString& strInstrumentID);
	...
	void NewOrder(CString& strInstrumentID, BOOL bDirection, int nOCMode, double fLimitPrice, int nVolume, BOOL bIsMarket);
	...
	CString GetNowVolume() {
		CString strText;
		if(m_nNowVolume>=0)
			strText.Format("%d", m_nNowVolume);
		return strText;
	};
...
protected:
	...
	std::map<string, int>			m_mapTradeNumByPrice_Buy;
	std::map<string, int>			m_mapTradeNumByPrice_Sell;
