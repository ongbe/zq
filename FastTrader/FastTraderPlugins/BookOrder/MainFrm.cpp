...
UINT nITFCMDARRIVE = ::RegisterWindowMessage("Modlule-BookOrder ApiMsg Arrive");
...
int CMainFrame::CallbackRecvApiMsg(int nCmdId, int nErrorCode, void* pData, int nDataLen, int nRequestId)
{
	...
	else 
	{
		...
		switch(nCmdId) 
		{
			...
			case CMDID_PRODUCT_ALLINFO_RSP:
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	poPlusinApi->Init("BookOrder", "123", CallbackRecvApiMsg, GetTargetHWnd(), NEED_NO_POS_NO_BIND, GetSafeHwnd());
	poPlusinApi->Logon();
	return 0;
}
...
void CMainFrame::ParseCmd(int nCmdId, int nErrorCode, void* pData, int nDataLen, int nSeqNum)
{
	if(nCmdId >= CMDID_CONSOLE_MIN && nCmdId <= CMDID_CONSOLE_MAX)
	{
		...
		switch(nCmdId) {
			...
			case CMDID_CONFIG_INPUT_VOLUMES_RSP:
				{
					CBookOrderDialog* pDlg = NULL;
					LockGlobalObject();
					CSequenceManager& objSeqMng = GetSequenceManager();
					pDlg = (CBookOrderDialog*)objSeqMng.GetSeqNumObject(nSeqNum);
					...
					if(bIsSuccess)
					{
						...
						switch(nCmdId)
						{
							case CMDID_ORDER_ORDER_STATUS_CHANGE:
								...
								strncpy(szInstrumentID, pOrderData->InstrumentID, sizeof(InstrumentIDType));
								}
								break;
							case CMDID_ORDER_TRADE_RTN:
								{
								...
								ZeroMemory(szInstrumentID, sizeof(szInstrumentID));
								strncpy(szInstrumentID, pTradeData->InstrumentID, sizeof(InstrumentIDType));
								}
								break;
							case CMDID_QUOTE_SEND_QUOTE:
								{
								...
								ZeroMemory(szInstrumentID, sizeof(szInstrumentID));
								strncpy(szInstrumentID, pDepthData->InstrumentID, sizeof(InstrumentIDType));
								}
								break;
						};
						...
						if(nCmdId==CMDID_QUOTE_SEND_QUOTE) {
							PlatformStru_DepthMarketData* pDepthData = (PlatformStru_DepthMarketData*)pData;
							OnQuoteForLocalConds(*pDepthData);
						}
...
void CMainFrame::ParseCmd_BaseBusiness(int nCmdId, int nErrorCode, void* pData, int nDataLen, int nSeqNum)
{
	switch(nCmdId) {
		...
		case CMDID_PRODUCT_ALLINFO_RSP:
			{
				if(nErrorCode == SUCCESS && nDataLen%sizeof(PlatformStru_ProductInfo)==0 && pData!=NULL)
				{
					for (int i=0; i<nDataLen/sizeof(PlatformStru_ProductInfo); i++)
					{
						PlatformStru_ProductInfo* pInfo = ((PlatformStru_ProductInfo*)pData) + i;
						LockGlobalObject();
						GetProduct2ExchangeList().insert(std::make_pair(pInfo->ProductID, pInfo->ExchangeID));
						UnlockGlobalObject();
					}
				}
				else 
...
void CMainFrame::QueryProductInfos()
{
	PlusinAPI* poPlusinApi = GetPlusinAPI();
	if(poPlusinApi!=NULL) 
		poPlusinApi->Send(CMDID_PRODUCT_ALLINFO_QUERY, NULL, 0, 
				CSequenceManager::GetNextSeqNum());
}
...
void CMainFrame::ShowNewBookOrderDialog(CString& strInstrumentID)
{
	...
	BOOL bIsFound = FALSE;
	CBookOrderDialog* pDlg = NULL;
	CBookOrderDialog* pFoundDlg = NULL;
	CBookOrderDialog* pEmptyDlg = NULL;
	...
	if(pEmptyDlg==NULL)
	{
		pDlg = new CBookOrderDialog(NULL);
		pDlg->Create(CBookOrderDialog::IDD);
	}
	else
	{
		pDlg = pEmptyDlg;
