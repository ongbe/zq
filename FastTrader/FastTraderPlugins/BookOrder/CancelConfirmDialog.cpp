...
BEGIN_MESSAGE_MAP(CCancelConfirmDialog, CDialog)
	...
	ON_BN_CLICKED(ID_CANCELSELECTALL, &CCancelConfirmDialog::OnBnClickedCancelselectall)
	ON_BN_CLICKED(ID_SELECTALL, &CCancelConfirmDialog::OnBnClickedSelectall)
END_MESSAGE_MAP()
...
HBRUSH CCancelConfirmDialog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	//else if(pWnd->GetDlgCtrlID()==IDOK)
	//{
 //       //pDC->SetBkMode(TRANSPARENT);
	//	pDC->SetBkColor(RGB(255, 0, 0));
	//	return m_pDlgBrushBk->operator HBRUSH();
	//}
...
BOOL CCancelConfirmDialog::OnInitDialog()
{
	...
	//m_btnOk.SetBGColor(RGB(255, 0, 0));
	//m_btnCancel.SetBGColor(RGB(0, 0, 255));
	...
	if(m_mapCanCancelOrder.size()<=(UINT)nRealItem)
...
void CCancelConfirmDialog::CancelAllOrder(std::map<int, PlatformStru_OrderInfo>& mapCanCancelOrder)
{
	std::map<int, PlatformStru_OrderInfo>::iterator it;

	for(it = mapCanCancelOrder.begin(); it != mapCanCancelOrder.end(); it++) {
		CancelOrder(it->second);
	}
}
...
void CCancelConfirmDialog::OnBnClickedCancelselectall()
{
	// TODO: Add your control notification handler code here
...
void CCancelConfirmDialog::OnBnClickedSelectall()
{
	// TODO: Add your control notification handler code here
	for(int i=0; i<m_listOrders.GetItemCount(); i++)
