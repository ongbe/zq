...
HANDLE g_hMutexOverTime = CreateMutex(NULL, FALSE, NULL);//确保同一时间只有一个对超时容器操作
HANDLE g_hMutexSingle = CreateMutex(NULL, FALSE, NULL);
...
BOOL LockObjectOverTime() {
    ::WaitForSingleObject(g_hMutexOverTime, INFINITE);
    return TRUE;
};
BOOL UnLockObjectOverTime() {    
    ::ReleaseMutex(g_hMutexOverTime);
    return TRUE;
};

BOOL LockObjectSingle() {
    ::WaitForSingleObject(g_hMutexSingle, INFINITE);
    return TRUE;
};
BOOL UnLockObjectSingle() {    
    ::ReleaseMutex(g_hMutexSingle);
    return TRUE;
};

void ReleaseObjectOverTime()
{
...
UINT g_nBookOrderCloseMSGID = ::RegisterWindowMessage("AlgoAutoOrder Dialog Close");
...
void LoadConfig()
{
	...
	g_nColorStyle = ::GetPrivateProfileInt("AlgoAutoOrderDlg", "ColorStyle", 1, strIniFile);
	g_dlgSize.cx = ::GetPrivateProfileInt("AlgoAutoOrderDlg", "DlgWidth", 0, strIniFile);
	g_dlgSize.cy = ::GetPrivateProfileInt("AlgoAutoOrderDlg", "DlgHeight", 0, strIniFile);
}
