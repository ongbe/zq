...
13．公式： 32 
*/
bool operator < (const _POINT& lhs,const _POINT& rhs)  
{  
    if(lhs.x>rhs.x)  
    {  
...
bool operator ==( _POINT& lhs,_POINT rhs )  
{  
...
double dist(_POINT p1,_POINT p2)                // 返回两点之间欧氏距离 
{ 
    return( sqrt( (p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y) ) ); 
} 

bool equal_point(_POINT p1,_POINT p2)           // 判断两个点是否重合  
{ 
...
r<0：ep在矢量opsp的顺时针方向 
*******************************************************************************/ 
double multiply(_POINT sp,_POINT ep,_POINT op) 
{ 
    double dd = ((sp.x-op.x)*(ep.y-op.y)-(ep.x-op.x)*(sp.y-op.y)); 
    return (dd<0.000001)?0:dd;
} 
/* 
...
r>0：两矢量夹角为锐角 
*******************************************************************************/ 
double dotmultiply(_POINT p1,_POINT p2,_POINT p0) 
{ 
...
*******************************************************************************/ 
bool online(_LINESEG l,_POINT p) 
{ 
    return( (multiply(l.e,p,l.s)==0) &&( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) ); 
} 
// 返回点p以点o为圆心逆时针旋转alpha(单位：弧度)后所在的位置 
_POINT rotate(_POINT o,double alpha,_POINT p) 
{ 
    _POINT tp; 
    p.x-=o.x; 
    p.y-=o.y; 
...
-1<r<1 && r'<=0 angle = -arccos(r)
*/ 
double angle(_POINT o,_POINT s,_POINT e) 
{ 
...
*/ 
double relation(_POINT p,_LINESEG l) 
{ 
    _LINESEG tl; 
    tl.s=l.s; 
    tl.e=p; 
...
// 求点C到线段AB所在直线的垂足 P 
_POINT perpendicular(_POINT p,_LINESEG l) 
{ 
...
double ptolinesegdist(_POINT p,_LINESEG l,_POINT &np) 
{ 
    double r=relation(p,l); 
    if(r<0) 
    { 
...
double ptoldist(_POINT p,_LINESEG l) 
{ 
    return abs(multiply(p,l.e,l.s))/dist(l.s,l.e); 
} 
/* 计算点到折线集的最近距离,并返回最近点. 
注意：调用的是ptolineseg()函数 */ 
double ptopointset(int vcount,_POINT pointset[],_POINT p,_POINT &q) 
{ 
    int i; 
...
bool CircleInsidePolygon(int vcount,_POINT center,double radius,_POINT polygon[]) 
{ 
    _POINT q; 
    double d; 
    q.x=0; 
...
double cosine(_LINESEG l1,_LINESEG l2) 
{ 
...
double lsangle(_LINESEG l1,_LINESEG l2) 
{ 
    _POINT o,s,e; 
    o.x=o.y=0; 
    s.x=l1.e.x-l1.s.x; 
...
bool intersect(_LINESEG u,_LINESEG v) 
{ 
...
bool intersect_A(_LINESEG u,_LINESEG v) 
{ 
...
bool intersect_l(_LINESEG u,_LINESEG v) 
{ 
    return multiply(u.s,v.e,v.s)*multiply(v.e,u.e,v.s)>=0; 
} 
// 根据已知两点坐标，求过这两点的直线解析方程： a*x+b*y+c = 0  (a >= 0)  
_LINE makeline(_POINT p1,_POINT p2) 
{ 
    _LINE tl; 
    int sign = 1; 
    tl.a=p2.y-p1.y; 
...
// 求点p关于直线l的对称点  
_POINT symmetry(_LINE l,_POINT p) 
{ 
    _POINT tp; 
...
bool lineintersect(_LINE l1,_LINE l2,_POINT &p) // 是 L1，L2 
{ 
...
bool intersection(_LINESEG l1,_LINESEG l2,_POINT &inter) 
{ 
    _LINE ll1,ll2; 
...
本程序默认第一个条件已经满足 
*/ 
bool issimple(int vcount,_POINT polygon[]) 
{ 
    int i,cn; 
    _LINESEG l1,l2; 
    for(i=0;i<vcount;i++) 
...
void checkconvex(int vcount,_POINT polygon[],bool bc[]) 
{ 
    int i,index=0; 
    _POINT tp=polygon[0]; 
...
bool isconvex(int vcount,_POINT polygon[]) 
{ 
    bool bc[MAXV]; 
...
double area_of_polygon(int vcount,_POINT polygon[]) 
{ 
    int i; 
    double s; 
    if (vcount<3) 
...
// 如果输入顶点按逆时针排列，返回true 
bool isconterclock(int vcount,_POINT polygon[]) 
{ 
    return area_of_polygon(vcount,polygon)>0; 
} 
// 另一种判断多边形顶点排列方向的方法  
bool isccwize(int vcount,_POINT polygon[]) 
{ 
    int i,index; 
    _POINT a,b,v; 
    v=polygon[0]; 
...
如果点在多边形外： 返回2 
*********************************************************************************************/ 
int insidepolygon(int vcount,_POINT Polygon[],_POINT q) 
{ 
    int c=0,i,n; 
    _LINESEG l1,l2; 
...
bool InsideConvexPolygon(int vcount,_POINT polygon[],_POINT q) // 可用于三角形！ 
{ 
    _POINT p; 
    _LINESEG l; 
    int i; 
...
len为输出的凸包上的点的个数 
**********************************************/ 
void Graham_scan(_POINT PointSet[],_POINT ch[],int n,int &len) 
{ 
    int i,j,k=0,top=2; 
    _POINT tmp; 
...
void ConvexClosure(_POINT PointSet[],_POINT ch[],int n,int &len) 
{ 
...
**********************************************************************************************/ 
bool LinesegInsidePolygon(int vcount,_POINT polygon[],_LINESEG l) 
{ 
    // 判断线端l的端点是否不都在多边形内 
    if(!insidepolygon(vcount,polygon,l.s)||!insidepolygon(vcount,polygon,l.e)) 
        return false; 
    int top=0,i,j; 
    _POINT PointSet[MAXV],tmp; 
    _LINESEG s; 
...
_POINT cg_simple(int vcount,_POINT polygon[]) 
{ 
...
_POINT gravitycenter(int vcount,_POINT polygon[]) 
{ 
    _POINT tp; 
...
y坐标最大，最小的点肯定是凸顶点            
************************************************/ 
_POINT a_point_insidepoly(int vcount,_POINT polygon[]) 
{ 
    _POINT v,a,b,r; 
    int i,index; 
    v=polygon[0]; 
...
/***********************************************************************************************/ 
void pointtangentpoly(int vcount,_POINT polygon[],_POINT p,_POINT &rp,_POINT &lp) 
{ 
    _LINESEG ep,en; 
    bool blp,bln; 
    rp=polygon[0]; 
...
bool core_exist(int vcount,_POINT polygon[],_POINT &p) 
{ 
    int i,j,k; 
    _LINESEG l; 
    _LINE lineset[MAXV]; 
...
只需要逐一判断点是否在圆内即可。 
*******************************************************************************/ 
bool point_in_circle(_POINT o,double r,_POINT p) 
{ 
...
*******************************************************************************/ 
bool cocircle(_POINT p1,_POINT p2,_POINT p3,_POINT &q,double &r) 
{ 
...
int line_circle(_LINE l,_POINT o,double r,_POINT &p1,_POINT &p2) 
{ 
    return true; 
} 
...
_POINT rect4th(_POINT a,_POINT b,_POINT c) 
{ 
    _POINT d; 
...
内含： return 5； 
*/ 
int CircleRelation(_POINT p1, double r1, _POINT p2, double r2) 
{ 
...
// 调用ptoldist函数，在第4页 
bool CircleRecRelation(_POINT pc, double r, _POINT pr1, _POINT pr2, _POINT pr3, _POINT pr4) 
{ 
    if( pr1.x < pc.x && pc.x < pr2.x && pr3.y < pc.y && pc.y < pr2.y ) 
    { 
        _LINESEG line1(pr1, pr2); 
        _LINESEG line2(pr2, pr3); 
        _LINESEG line3(pr3, pr4); 
        _LINESEG line4(pr4, pr1); 
...
//点是否在直线同侧：
//两个点是否在直线同侧，是则返回true 
bool SameSide(_POINT p1, _POINT p2, _LINE line) 
{ 
...
//两圆交点： 
// 两圆已经相交（相切） 
void  c2point(_POINT p1,double r1,_POINT p2,double r2,_POINT &rp1,_POINT &rp2) 
{ 
    double a,b,r; 
    a=p2.x-p1.x; 
    b=p2.y-p1.y; 
...
//两圆公共面积：
// 必须保证相交 
double c2area(_POINT p1,double r1,_POINT p2,double r2) 
{ 
    _POINT rp1,rp2; 
    ...
    double s=0; 
    if(rr<r2)//相交弧为优弧 
        s=r1*r1*(PAI-sita1/2+sin(sita1)/2)+r2*r2*(sita2-sin(sita2))/2; 
    else//相交弧为劣弧 
...
//圆和直线关系： 
//0----相离 1----相切 2----相交 
int clpoint(_POINT p,double r,double a,double b,double c,_POINT &rp1,_POINT &rp2) 
{ 
    int res=0; 
...
//内切圆： 
void incircle(_POINT p1,_POINT p2,_POINT p3,_POINT &rp,double &r) 
{ 
...
void cutpoint(_POINT p,double r,_POINT sp,_POINT &rp1,_POINT &rp2) 
{ 
    _POINT p2; 
    p2.x=(p.x+sp.x)/2; 
    p2.y=(p.y+sp.y)/2; 
...
返回 –1 ： 左旋； 
*/ 
int rotat(_LINESEG l1,_LINESEG l2) 
{ 
