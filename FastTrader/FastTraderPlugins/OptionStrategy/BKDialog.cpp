...
BK_COLOR_MODE CBKDialog::m_ColorMode = Color_Light;
...
void CBKDialog::FreeResources(BOOL bCheckForNULL)
{
...
void CBKDialog::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// EasyMove mode
...
void CBKDialog::OnPostEraseBkgnd(CDC* pDC)
{
...
//
#ifndef UNDER_CE
HRGN CBKDialog::ScanRegion(HBITMAP hBitmap, BYTE byTransR, BYTE byTransG, BYTE byTransB)
{
...
//
#ifndef UNDER_CE
LPBYTE CBKDialog::Get24BitPixels(HBITMAP hBitmap, LPDWORD lpdwWidth, LPDWORD lpdwHeight)
{
...
//
DWORD CBKDialog::ActivateEasyMoveMode(BOOL bActivate)
{
...
//
DWORD CBKDialog::SetBitmap(int nBitmap, COLORREF crTransColor)
{
...
//			Failed creating region
//
DWORD CBKDialog::SetBitmap(HBITMAP hBitmap, COLORREF crTransColor)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//			Invalid mode.
//
DWORD CBKDialog::SetMode(BYTE byMode, BOOL bRepaint)
{
...
#ifndef UNDER_CE
DWORD CBKDialog::ShrinkToFit(BOOL bRepaint)
{
	CRect	rWnd;
	CRect	rClient;
	DWORD	dwDiffCX;
...
void CBKDialog::SetBKColorMode(BK_COLOR_MODE mode)
{
    m_ColorMode = mode;
}
