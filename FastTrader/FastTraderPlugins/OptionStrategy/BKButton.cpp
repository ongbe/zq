...
BEGIN_MESSAGE_MAP(CBKButton, CButton)
    //{{AFX_MSG_MAP(CBKButton)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CBKButton::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CBKButton::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CBKButton::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
HBRUSH CBKButton::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CBKButton::OnSysColorChange() 
{
...
LRESULT CBKButton::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBKButton::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBKButton::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CBKButton::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
	if (CBKMenu::IsMenu(pMenu))
		lResult = CBKMenu::FindKeyboardShortcut(nChar, nFlags, pMenu);
	else
...
#ifdef	BTNST_USE_BCMENU
void CBKButton::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
	if (lpMeasureItemStruct->CtlType == ODT_MENU)
	{
		if (IsMenu((HMENU)lpMeasureItemStruct->itemID) && CBKMenu::IsMenu((HMENU)lpMeasureItemStruct->itemID))
		{
...
void CBKButton::OnEnable(BOOL bEnable) 
{
...
void CBKButton::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CBKButton::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CBKButton::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CBKButton::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CBKButton::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CBKButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CBKButton::OnClicked() 
{	
	SetFocus();
...
#ifdef	BTNST_USE_BCMENU
			CBKMenu* psub = (CBKMenu*)m_menuPopup.GetSubMenu(0);
...
void CBKButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CBKButton::PaintBk(CDC* pDC)
{
...
HBITMAP CBKButton::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CBKButton::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CBKButton::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CBKButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CBKButton::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HBITMAP CBKButton::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
...
//		If the function fails, the return value is NULL.
//
HBITMAP CBKButton::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
	HBITMAP		hGrayBitmap = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON CBKButton::CreateGrayscaleIcon(HICON hIcon)
{
...
//		If the function fails, the return value is NULL.
//
HICON CBKButton::CreateDarkerIcon(HICON hIcon)
{
	HICON		hGrayIcon = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	BITMAP		bmp;
...
COLORREF CBKButton::DarkenColor(COLORREF crColor, double dFactor)
{
...
//
DWORD CBKButton::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CBKButton::SetIcon(int nIconIn, int nIconOut)
{
...
//			Failed loading the specified resource.
//
DWORD CBKButton::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CBKButton::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CBKButton::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
...
//				By default, CBKButton buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CBKButton::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CBKButton buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CBKButton::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CBKButton buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CBKButton::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CBKButton::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CBKButton::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CBKButton::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CBKButton::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CBKButton::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CBKButton::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CBKButton::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CBKButton::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CBKButton::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CBKButton::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CBKButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CBKButton::ActivateTooltip(BOOL bActivate)
{
...
//			Function executed successfully.
//
DWORD CBKButton::EnableBalloonTooltip()
{
...
//				If TRUE the control will be repainted.
//
void CBKButton::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CBKButton::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CBKButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
// This function associates a menu to the button.
// The menu will be displayed clicking the button.
// The menu will be handled by the CBKMenu class.
//
// Parameters:
//		[IN]	nMenu
//				ID number of the menu resource.
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CBKButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
//		Where:
//				[IN]	wParam
//						If support for CBKMenu is enabled: a pointer to CBKMenu
...
//			Function executed successfully.
//
DWORD CBKButton::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
#ifdef	BTNST_USE_SOUND
DWORD CBKButton::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
// This is a virtual function that can be rewritten in CBKButton-derived classes
...
//			Function executed successfully.
//
DWORD CBKButton::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CBKButton::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	// Draw pressed button
	if (m_bIsPressed)
	{
