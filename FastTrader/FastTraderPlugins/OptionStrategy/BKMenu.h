...
// BKMenuData class. Fill this class structure to define a single menu item:
class BKMenuData
{
	wchar_t *m_szMenuText;
public:
	BKMenuData () {menuIconNormal=-1;xoffset=-1;bitmap=NULL;pContext=NULL;
...
public:
	...
	CBKMenu* AppendODPopupMenuA(LPCSTR lpstrText);
	CBKMenu* AppendODPopupMenuW(wchar_t *lpstrText);
	...
	CBKMenu* GetSubBCMenu(char* lpszSubMenuName);
	CBKMenu* GetSubBCMenu(wchar_t* lpszSubMenuName);
	...
	static void SetMenuDrawMode(UINT mode){
		CBKMenu::original_drawmode=mode;
		CBKMenu::xp_drawmode=mode;
	};
	...
	static void SetSelectDisableMode(BOOL mode){
		CBKMenu::original_select_disabled=mode;
		CBKMenu::xp_select_disabled=mode;
	};
	static int CBKMenu::GetMenuDrawMode(void);
	static BOOL CBKMenu::GetSelectDisableMode(void);

	// how the bitmaps are drawn in XP Luna mode
	static void SetXPBitmap3D(BOOL val){
		CBKMenu::xp_draw_3D_bitmaps=val;
	};
	static BOOL GetXPBitmap3D(void){return CBKMenu::xp_draw_3D_bitmaps;}
...
protected:
	...
	CBKMenu *FindMenuOption(int nId,int& nLoc);
	CBKMenu *FindAnotherMenuOption(int nId,int& nLoc,CArray<CBKMenu*,CBKMenu*>&bcsubs,
								  CArray<int,int&>&bclocs);
	BKMenuData *FindMenuOption(wchar_t *lpstrText);
	void InsertSpaces(void);
	void DrawCheckMark(CDC *pDC,int x,int y,COLORREF color);
	void DrawRadioDot(CDC *pDC,int x,int y,COLORREF color);
	BKMenuData *NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string);
...
// Member Variables
protected:
	CTypedPtrArray<CPtrArray, BKMenuData*> m_MenuList;  // Stores list of menu items 
	// When loading an owner-drawn menu using a Resource, CBKMenu must keep track of
	// the popup menu's that it creates. Warning, this list *MUST* be destroyed
	// last item first :)
	CTypedPtrArray<CPtrArray, HMENU>  m_SubMenus;  // Stores list of sub-menus 
	// Stores a list of all CBKMenu's ever created 
