...
#include "BKMenu.h"        // BKMenu class declaration
...
UINT CBKMenu::original_drawmode=BCMENU_DRAWMODE_ORIGINAL;
BOOL CBKMenu::original_select_disabled=TRUE;
// how the menu's are drawn in winXP
UINT CBKMenu::xp_drawmode=BCMENU_DRAWMODE_XP;
BOOL CBKMenu::xp_select_disabled=FALSE;
BOOL CBKMenu::xp_draw_3D_bitmaps=TRUE;

CImageList CBKMenu::m_AllImages;
CArray<int,int&> CBKMenu::m_AllImagesID;
...
void BKMenuData::SetAnsiString(LPCSTR szAnsiString)
{
	USES_CONVERSION;
	SetWideString(A2W(szAnsiString));  //SK:  see MFC Tech Note 059
}

CString BKMenuData::GetString(void)//returns the menu text in ANSI or UNICODE
...
CTypedPtrArray<CPtrArray, HMENU> CBKMenu::m_AllSubMenus;  // Stores list of all sub-menus

IMPLEMENT_DYNAMIC( CBKMenu, CMenu )
...
BOOL CBKMenu::IsNewShell ()
{
	return (g_Shell>=Win95);
}

BOOL CBKMenu::IsWinXPLuna()
{
	if(g_Shell==WinXP){
...
void BKMenuData::SetWideString(const wchar_t *szWideString)
{
	delete[] m_szMenuText;//Need not check for NULL because ANSI X3J16 allows "delete NULL"
...
BOOL CBKMenu::IsMenu(CMenu *submenu)
{
	int m;
...
BOOL CBKMenu::IsMenu(HMENU submenu)
{
	int m;
	int numSubMenus = m_AllSubMenus.GetUpperBound();
	for(m=0;m<=numSubMenus;++m){
...
BOOL CBKMenu::DestroyMenu()
{
	...
	for(m = numSubMenus; m >= 0; m--){
		...
		BOOL flag=ptr->IsKindOf(RUNTIME_CLASS( CBKMenu ));
		if(flag)delete((CBKMenu *)ptr);
	}
...
int CBKMenu::GetMenuDrawMode(void)
{
	if(IsWinXPLuna())return(xp_drawmode);
	return(original_drawmode);
}

BOOL CBKMenu::GetSelectDisableMode(void)
{
...
/*
==========================================================================
void CBKMenu::DrawItem(LPDRAWITEMSTRUCT)
---------------------------------------
...
void CBKMenu::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
...
void CBKMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
	CRect rect;
	UINT state = (((BKMenuData*)(lpDIS->itemData))->nFlags);
	...
	if(state & MF_SEPARATOR){
		...
		if(lpDIS->itemData != NULL){
			nIconNormal = (((BKMenuData*)(lpDIS->itemData))->menuIconNormal);
			xoffset = (((BKMenuData*)(lpDIS->itemData))->xoffset);
			global_offset = (((BKMenuData*)(lpDIS->itemData))->global_offset);
			bitmap = (((BKMenuData*)(lpDIS->itemData))->bitmap);
			strText = ((BKMenuData*) (lpDIS->itemData))->GetString();
...
COLORREF CBKMenu::LightenColor(COLORREF col,double factor)
{
...
COLORREF CBKMenu::DarkenColor(COLORREF col,double factor)
{
	if(factor>0.0&&factor<=1.0){
		BYTE red,green,blue,lightred,lightgreen,lightblue;
		red = GetRValue(col);
...
void CBKMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
...
BOOL CBKMenu::GetBitmapFromImageList(CDC* pDC,CImageList *imglist,int nIndex,CBitmap &bmp)
{
...
/*
==========================================================================
void CBKMenu::MeasureItem(LPMEASUREITEMSTRUCT)
---------------------------------------------
...
void CBKMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	UINT state = (((BKMenuData*)(lpMIS->itemData))->nFlags);
	int BCMENU_PAD=4;
	...
	if(state & MF_SEPARATOR){
		...
		const wchar_t *lpstrText = ((BKMenuData*)(lpMIS->itemData))->GetWideString();//SK: we use const to prevent misuse
...
void CBKMenu::SetIconSize (int width, int height)
{
	m_iconX = width;
	m_iconY = height;
}

BOOL CBKMenu::AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CBKMenu::AppendODMenuW(wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	...
	BKMenuData *mdata = new BKMenuData;
...
BOOL CBKMenu::InsertODMenuA(UINT nPosition,LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CBKMenu::InsertODMenuW(UINT nPosition,wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	if(!(nFlags & MF_BYPOSITION)){
		int iPosition =0;
		CBKMenu* pMenu = FindMenuOption(nPosition,iPosition);
		if(pMenu){
...
BOOL CBKMenu::ModifyODMenuA(const char * lpstrText,UINT nID,int nIconNormal)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,nIconNormal);//SK: see MFC Tech Note 059
}

BOOL CBKMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	int nLoc;
	BKMenuData *mdata;
	CArray<CBKMenu*,CBKMenu*>bcsubs;
	CArray<int,int&>bclocs;
	
	// Find the old BKMenuData structure:
	CBKMenu *psubmenu = FindMenuOption(nID,nLoc);
	do{
		if(psubmenu && nLoc>=0)mdata = psubmenu->m_MenuList[nLoc];
		else{
			// Create a new BKMenuData structure:
			mdata = new BKMenuData;
			m_MenuList.Add(mdata);
		}
...
BOOL CBKMenu::ModifyODMenuA(const char * lpstrText,UINT nID,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,il,xoffset);
}

BOOL CBKMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CImageList *il,int xoffset)
{
	int nLoc;
	BKMenuData *mdata;
	CArray<CBKMenu*,CBKMenu*>bcsubs;
...
BOOL CBKMenu::ModifyODMenuA(const char * lpstrText,UINT nID,CBitmap *bmp)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,bmp);
}

BOOL CBKMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
...
BOOL CBKMenu::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,fill,border,hatchstyle,pSize);
}

BOOL CBKMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
...
BOOL CBKMenu::ModifyODMenuA(const char *lpstrText,const char *OptionText,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CBKMenu::ModifyODMenuW(wchar_t *lpstrText,wchar_t *OptionText,
                           int nIconNormal)
{
	BKMenuData *mdata;
...
BKMenuData *CBKMenu::NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string)
{
	BKMenuData *mdata;
...
BOOL CBKMenu::LoadToolbars(const UINT *arID,int n)
{
	ASSERT(arID);
...
BOOL CBKMenu::LoadToolbar(UINT nToolBar)
{
	UINT nID,nStyle;
...
BOOL CBKMenu::LoadFromToolBar(UINT nID,UINT nToolBar,int& xoffset)
{
	int xset,offset;
	UINT nStyle;
	BOOL returnflag=FALSE;
...
// O.S.
BKMenuData *CBKMenu::FindMenuItem(UINT nID)
{
	BKMenuData *pData = NULL;
	int i;
	...
	if (!pData){
		int loc;
		CBKMenu *pMenu = FindMenuOption(nID, loc);
		ASSERT(pMenu != this);
		if (loc >= 0){
...
CBKMenu *CBKMenu::FindAnotherMenuOption(int nId,int& nLoc,CArray<CBKMenu*,CBKMenu*>&bcsubs,
									  CArray<int,int&>&bclocs)
{
	int i,numsubs,j;
	CBKMenu *psubmenu,*pgoodmenu;
	BOOL foundflag;
	
	for(i=0;i<(int)(GetMenuItemCount());++i){
#ifdef _CPPRTTI 
		psubmenu=dynamic_cast<CBKMenu *>(GetSubMenu(i));
#else
		psubmenu=(CBKMenu *)GetSubMenu(i);
#endif
		if(psubmenu){
...
CBKMenu *CBKMenu::FindMenuOption(int nId,int& nLoc)
{
	int i;
	CBKMenu *psubmenu,*pgoodmenu;
...
BKMenuData *CBKMenu::FindMenuOption(wchar_t *lpstrText)
{
	int i,j;
	CBKMenu *psubmenu;
	BKMenuData *pmenulist;
...
BOOL CBKMenu::LoadMenu(int nResource)
{
	return(CBKMenu::LoadMenu(MAKEINTRESOURCE(nResource)));
};

BOOL CBKMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	...
	CTypedPtrArray<CPtrArray, CBKMenu*>  m_Stack;    // Popup menu stack
...
void CBKMenu::RemoveTopLevelOwnerDraw(void)
{
	CString str;
	int i=0,j=0;
...
void CBKMenu::InsertSpaces(void)
{
...
void CBKMenu::LoadCheckmarkBitmap(int unselect, int select)
{
...
//--------------------------------------------------------------------------
//[18.06.99 rj]
BOOL CBKMenu::GetMenuText(UINT id, CString& string, UINT nFlags/*= MF_BYPOSITION*/)
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags){
		...
		int uiLoc;
		CBKMenu* pMenu = FindMenuOption(id,uiLoc);
...
void CBKMenu::DrawRadioDot(CDC *pDC,int x,int y,COLORREF color)
{
...
void CBKMenu::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color)
{
	CPen m_penBack;
...
BKMenuData *CBKMenu::FindMenuList(UINT nID)
{
...
void CBKMenu::InitializeMenuList(int value)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i)
		m_MenuList[i]->syncflag=value;
}

void CBKMenu::DeleteMenuList(void)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i){
...
void CBKMenu::SynchronizeMenu(void)
{
	CTypedPtrArray<CPtrArray, BKMenuData*> temp;
	BKMenuData *mdata;
	CString string;
...
void CBKMenu::UpdateMenu(CMenu *pmenu)
{
#ifdef _CPPRTTI 
	CBKMenu *psubmenu = dynamic_cast<CBKMenu *>(pmenu);
#else
	CBKMenu *psubmenu = (CBKMenu *)pmenu;
#endif
	if(psubmenu)psubmenu->SynchronizeMenu();
}

LRESULT CBKMenu::FindKeyboardShortcut(UINT nChar, UINT nFlags,
                                     CMenu *pMenu)
{
#ifdef _CPPRTTI 
	CBKMenu *pBCMenu = dynamic_cast<CBKMenu *>(pMenu);
#else
	CBKMenu *pBCMenu = (CBKMenu *)pMenu;
#endif
...
void CBKMenu::DitherBlt (HDC hdcDest, int nXDest, int nYDest, int nWidth, 
...
void CBKMenu::GetFadedBitmap(CBitmap &bmp)
{
	CDC ddc;
	COLORREF bgcol,col;
	BITMAP BitMap;
...
void CBKMenu::GetShadowBitmap(CBitmap &bmp)
{
	CDC ddc;
...
BOOL CBKMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	BOOL bReturn=FALSE;
...
void CBKMenu::SetBitmapBackground(COLORREF color)
{
	m_bitmapBackground=color;
	m_bitmapBackgroundFlag=TRUE;
}

void CBKMenu::UnSetBitmapBackground(void)
{
	m_bitmapBackgroundFlag=FALSE;
}
...
void CBKMenu::AddFromToolBar(CToolBar* pToolBar, int nResourceID)
{
...
BOOL CBKMenu::Draw3DCheckmark(CDC *dc, const CRect& rc,
...
BOOL CBKMenu::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout)
{
	CBrush brushin;
...
void CBKMenu::DitherBlt2(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
void CBKMenu::SetDisableOldStyle(void)
{
	disable_old_style=TRUE;
}

void CBKMenu::UnSetDisableOldStyle(void)
{
	disable_old_style=FALSE;
}

BOOL CBKMenu::GetDisableOldStyle(void)
{
	return(disable_old_style);
}


WORD CBKMenu::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
HBITMAP CBKMenu::LoadSysColorBitmap(int nResourceId)
{
	HINSTANCE hInst = 
...
BOOL CBKMenu::RemoveMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			CBKMenu* pSubMenu = (CBKMenu*) GetSubMenu(uiId);
			if(NULL==pSubMenu){
		...
		int iPosition =0;
		CBKMenu* pMenu = FindMenuOption(uiId,iPosition);
...
BOOL CBKMenu::DeleteMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		UINT uint = GetMenuState(uiId,MF_BYPOSITION);
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
...
BOOL CBKMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CBKMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CBKMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CBKMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CBKMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CBKMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
BOOL CBKMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CBKMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CBKMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CBKMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CBKMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CBKMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CBKMenu* CBKMenu::AppendODPopupMenuW(wchar_t *lpstrText)
{
	CBKMenu* pSubMenu = new CBKMenu;
	pSubMenu->m_unselectcheck=m_unselectcheck;
	pSubMenu->m_selectcheck=m_selectcheck;
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CBKMenu* CBKMenu::AppendODPopupMenuA(LPCSTR lpstrText)
{
	USES_CONVERSION;
	return AppendODPopupMenuW(A2W(lpstrText));
}

BOOL CBKMenu::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
...
CMenu* CBKMenu::GetSubMenu(int nPos)
{
	return CMenu::GetSubMenu (nPos);
}

CMenu* CBKMenu::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
CBKMenu* CBKMenu::GetSubBCMenu(char* pText)
{
	USES_CONVERSION;
	return GetSubBCMenu(A2W(pText));
}

CBKMenu* CBKMenu::GetSubBCMenu(wchar_t* lpszSubMenuName)
{
	BKMenuData *mdata;
...
int CBKMenu::GetMenuPosition(char* pText)
{
	USES_CONVERSION;
	return GetMenuPosition(A2W(pText));
}

int CBKMenu::GetMenuPosition(wchar_t* pText)
{
	int i,j;
	CBKMenu* psubmenu;
	for(i=0;i<(int)(GetMenuItemCount());++i)
...
int CBKMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int CBKMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
...
int CBKMenu::DeleteMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return DeleteMenu(A2W(pText), sPos);
}

int CBKMenu::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
	int nPos = GetMenuPosition(pText);
	if(nPos != -1)
	{
...
// Larry Antram
BOOL CBKMenu::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
...
// courtesy of Warren Stevens
void CBKMenu::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	CDC bmpdc;
	COLORREF m_newclrBack;
...
BOOL CBKMenu::IsWindowsClassicTheme(void)
{
...
int CBKMenu::GlobalImageListOffset(int nID)
{
...
BOOL CBKMenu::CanDraw3DImageList(int offset)
{
	BOOL retflag=FALSE;
	int numcurrent=m_AllImagesID.GetSize();
...
int CBKMenu::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	int loc = -1;
