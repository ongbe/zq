...
void CFData::SetDataInTime(PlatformStru_DepthMarketData* pData)
{
	...
	{
		double dbMaxSpan  = abs(pData->LastPrice - m_PreSettlementPrice);
		double dbMaxValue = m_PreSettlementPrice + dbMaxSpan;
		double dbMinValue = m_PreSettlementPrice - dbMaxSpan;
	...
	case PHRASE_5MIN:
		{
			if(bShow)
			{				
				SetInTimeData_MINS(m_vecKLineMin1, pTemp, 300);//5	
			}	
		}
		break;
	case PHRASE_15MIN:
		{
			if(bShow)
			{				
				SetInTimeData_MINS(m_vecKLineMin1, pTemp, 900);//5	
			}	
		}
		break;
	case PHRASE_30MIN:
		{
			if(bShow)
			{				
				SetInTimeData_MINS(m_vecKLineMin1, pTemp, 1800);//5	
			}	
		}
		break;
	case PHRASE_60MIN:
		{
			if(bShow)
			{				
				SetInTimeData_MINS(m_vecKLineMin1, pTemp, 3600);//5	
			}	
		}
		break;
	case PHRASE_DAY:
	...
	case PHRASE_WEEK:
		{
			...
			{				
				...
				pTemp.dwTime.tm_min  = 0;
				pTemp.dwTime.tm_sec  = 0;
				SetInTimeData_Week(m_vecKLineDay, pTemp);	
			}	
		}
		break;
	case PHRASE_MONTH:
		{
			...
			{				
				...
				pTemp.dwTime.tm_min  = 0;
				pTemp.dwTime.tm_sec  = 0;
				SetInTimeData_Month(m_vecKLineDay, pTemp);	
			}	
		}
		break;
	}
...
bool CFData::AppendHistoryTLineData(std::vector<TimeLineData>& vecTimeLineData)
{
	...
	{
		TimeLineData& data = vecTimeLineData[i];
		std::string strDay = data.szDate;
		std::string strTime = data.szTime;
		STLine sTLine;
		sTLine.dwTime.tm_year = atoi(strDay.substr(0, 4).c_str()) - 1900;
		sTLine.dwTime.tm_mon  = atoi(strDay.substr(5, 2).c_str()) - 1;
		sTLine.dwTime.tm_mday = atoi(strDay.substr(8, 2).c_str());
		sTLine.dwTime.tm_hour = atoi(strTime.substr(0, 2).c_str());
		sTLine.dwTime.tm_min  = atoi(strTime.substr(3, 2).c_str());
		sTLine.dwTime.tm_sec  = atoi(strTime.substr(6, 2).c_str());

		sTLine.dwVolume = data.Volume;
		sTLine.dwHoldVolume = data.OpenInterest;
		sTLine.dwVolumeMoney = data.Turnover;
...
bool CFData::AppendHistoryKLineData_Min1(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	...
	for(int i = vecData.size()-1; i>=0; i--)
	{
		...
		sKLine.fOpenPrice = data.OpenPrice;
		sKLine.fHighPrice = data.HighestPrice;
		sKLine.fLowPrice  = data.LowestPrice;
		sKLine.fClosePrice = data.ClosePrice;
		sKLine.dJieSuan    = data.SettlementPrice;
		sKLine.dwHoldVolume =data.OpenInterest;
		sKLine.dwVolume     = data.IncreaseVolume;
		sKLine.fSumTradeVolume = data.Turnover;
...
bool CFData::AppendHistoryKLineData_Min5(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
...
bool CFData::AppendHistoryKLineData_Min15(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
...
bool CFData::AppendHistoryKLineData_Min30(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
...
bool CFData::AppendHistoryKLineData_Min60(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
...
bool CFData::AppendHistoryKLineData_Day(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
...
bool CFData::AppendHistoryKLineData_Week(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
	for(int i = vecData.size()-1; i>=0; i--)
	{
		...
		sKLine.dwTime.tm_mon  = atoi(strDay.substr(4, 2).c_str()) - 1;
		sKLine.dwTime.tm_mday = atoi(strDay.substr(6, 2).c_str());
	/*	if(i == vecData.size()-1  && strTime.length() >=8)
		{//最后一天的数据为实时的
			sKLine.dwTime.tm_hour = atoi(strTime.substr(0, 2).c_str());
			sKLine.dwTime.tm_min  = atoi(strTime.substr(3, 2).c_str());
			sKLine.dwTime.tm_sec  = atoi(strTime.substr(6, 2).c_str());
...
bool CFData::AppendHistoryKLineData_Month(std::vector<KLineData>& vecData)
{
	if(vecData.size() ==0)
		return true;
	std::vector<SKLine> vecKLine;
