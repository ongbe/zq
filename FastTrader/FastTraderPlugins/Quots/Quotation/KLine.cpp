...
void CKLine::SetCursel(SItem& sItem)
{
	...
	/* 鼠标跟键盘走，这个在k线缩到很小的时候，会出现走不动的问题，原因是hook捕获了SetCursorPos，重新设置了m_nCurSel的值
			POINT		pt;
			pt.x = m_ptMouseMoveLast.x;
			pt.y = m_ptMouseMoveLast.y;
			::ClientToScreen((HWND)GetHWND(),&pt);
			SetCursorPos(pt.x, pt.y);//键盘左右移动的时候，鼠标跟着走
			*/
		}
	}
	Refresh(false);
...
bool CKLine::InitCfg(TiXmlElement *root)
{
	...
	ListItem = pColor->FirstChild("sar");
	if(ListItem == NULL)
		return false;
	strValue = ListItem->ToElement()->GetText();
	value = ColorStr2Long(strValue);
...
void CKLine::SetSARName(unsigned long lValue)
{
	m_clSAR = lValue;
}
...
void CKLine::DoMouseMove(wxPoint mousePos)
{
...
void CKLine::DrawSAR(wxDC& dc)
{
	if(m_strTechIndexName != SAR)
		return;
	if(m_pTechIndex == NULL)
		return;
	...
	if(m_pTechIndex->m_vecData == NULL)
		return;
	if( m_pTechIndex->m_vecData[0].size() == 0)
		return;
	dc.SetClippingRegion(rtClient);

	
	wxPen penBorder(m_clSAR, 1, wxSOLID);
	dc.SetPen(penBorder);
	dc.SetBrush( *wxTRANSPARENT_BRUSH );
	for(int i = m_nBegin; i> m_nEnd; i--)
	{
		if(i>= (int)m_pTechIndex->m_vecData[0].size() )
			continue;
		int nLeftBegin =0, nRightBegin =0;
		XuHao2WeiZhi(i, nLeftBegin, nRightBegin);
		int x = (nLeftBegin + nRightBegin)/2;
		int y = JiaGe2GaoDu(m_pTechIndex->m_vecData[0][i].fValue);		
