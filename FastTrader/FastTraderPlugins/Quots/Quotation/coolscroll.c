...
#include "../../../inc/Quotation/userdefs.h"
#include "../../../inc/Quotation/coolsb_internal.h"
...
static int DrawScrollArrow(SCROLLBAR *sbar, HDC hdc, RECT *rect, UINT arrow, BOOL fMouseDown, BOOL fMouseOver)
{
	...
	if(sbar->fFlatScrollbar != CSBS_NORMAL)
	{
		...
		//	SetBkColor(hdcmem2, GetSysColor(COLOR_BTNTEXT));
			...
			SetTextColor(hdc, RGB(94, 94, 94));//这个颜色为点击后的背景色，箭头按钮处
...
void DrawBlankButton(HDC hdc, const RECT *rect, UINT drawflag)
{
	...
	HBRUSH hbrushBackground = CreateSolidBrush(RGB(100,100,100));
	RECT rc = *rect;
...
static LRESULT NCDrawHScrollbar(SCROLLBAR *sb, HWND hwnd, HDC hdc, const RECT *rect, UINT uDrawFlags)
{
	...
	COLORREF crCheck1   = RGB(10,10,10);//GetSBForeColor();
	COLORREF crCheck2   = RGB(10,10,10);// GetSBBackColor();
	COLORREF crInverse1 = RGB(10,10,10);//InvertCOLORREF(crCheck1);
	COLORREF crInverse2 = RGB(10,10,10);//InvertCOLORREF(crCheck2);
	...
	if(scrollwidth > butwidth*2)
	{
		...
		{	
			...
			else
			{
				...
				{	
					COLORREF oldcol = RGB(66,66,66);
					PaintRect(hdc, &thumb, oldcol);
				}
				else
#endif
				{
