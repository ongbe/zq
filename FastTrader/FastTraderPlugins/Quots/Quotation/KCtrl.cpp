			...
			const wxString& name )
{
	...
	RegisterHotKey(0x79,wxMOD_WIN ,VK_F10);	
	return bReturn;
}
...
float CKCtrl::XuHao2WeiZhi(int nItem, int& nLeft, int& nRight)
{
	...
	{
		nLeft = rtClient.GetLeft() + nKaiShi * (m_dbDayWidth +m_nDaySpan);
		nRight = nLeft + m_dbDayWidth;
	}
	return nLeft;
}
...
void CKCtrl::RefreshTip()
{
	wxRect rtClient2 = GetClientRect();
	rtClient2.SetLeft(rtClient2.GetLeft() + m_RectOffSet.GetLeft());	
	rtClient2.SetRight(rtClient2.GetRight() - m_RectOffSet.GetWidth() -m_RectOffSet.GetLeft());
	rtClient2.SetBottom(rtClient2.GetTop() +m_RectOffSet.GetTop());
	Refresh(true, &rtClient2);
...
void CKCtrl::DrawTips_Time(wxDC& dc, wxRect rtClient, int nX)
{
	...
	strTextBottom.Printf(_T("%4d/%2d/%2d"), pKLine->dwTime.tm_year+1900, pKLine->dwTime.tm_mon+1, pKLine->dwTime.tm_mday);
...
void   CKCtrl::SItemGetValue(bool	bIsMouse, bool bIsKeyBoard)
{
	...
	m_sItem.nDayWidth	= m_dbDayWidth;
...
void CKCtrl::OnLeftUp(wxMouseEvent& event)
{
	...
	if(nCurSel >=0 && nCurSel< (int)m_vecKLine.size())
	{
		...
		{
			map<int, SMark *>::iterator it = pMarks->find(m_enumMark);
			if(it != pMarks->end())
			{
			...
			else
			{
				...
				sp->dwTime		 = pKLine->dwTime;
				pTemp->vecPt.push_back(sp);
				pMarks->insert(MAPsmark::value_type(m_enumMark,  pTemp));						
			}	
			wxWindow* window= GetParent();
			wxCommandEvent myEvent(wxEVT_KCtrl_MARKSCHANGE);
		//	myEvent.SetClientData(&m_mapMarks);
...
void CKCtrl::OnConfigChange(wxCommandEvent& event)
{
	...
	if(m_pFData)
	{
		...
		sTCPara.enumPhrase = m_enumPhrase;
		sTCPara.m_VolumeMultiple = m_pFData->m_VolumeMultiple;
		m_pFData->CalcKTechIndex(m_vecKLine,"", m_strTechIndexName.c_str(), sTCPara, m_pTechIndex, &vecCofig);
	}
...
wxString CKCtrl::GetTechIndexName()
{
	return m_strTechIndexName;
}