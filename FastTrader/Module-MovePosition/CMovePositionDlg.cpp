	...
	for (;iter != outData.end(); ++iter)
	{
		...
		//	strcmp(strItem.c_str(), m_tPositionData.InstrumentID) != 0)
		{
			...
			if (0 == i)
			{
				...
				if(DEFAULT_SVR()) DEFAULT_SVR()->SetSubscribeStatus(m_strInstrumentIDBackup.c_str(),GID_DLG_MOVEPOSITION);
...
void CMovePositionDlg::SendRequest(bool bIsRedo)
{
    ...
    tSrcPos.nHedgeStatus=m_tPositionData.HedgeFlag-THOST_FTDC_HF_Speculation;
...
//		if(strcmp(outData.ExchangeID, "SHFE")==0 && NeedSimulateMarketPrice()) 
    ...
    tDestPos.nHedgeStatus=m_tPositionData.HedgeFlag-THOST_FTDC_HF_Speculation;
...
void CMovePositionDlg::OnComboboxSelectChange(wxCommandEvent& event)
{
	...
	case ID_WXCOMBOBOX1:
		if (m_pComboBox1 != NULL)
		{
			...
			if(DEFAULT_SVR()) DEFAULT_SVR()->SetSubscribeStatus(m_strInstrumentIDBackup.c_str(),GID_DLG_MOVEPOSITION,eUnsubscribeMarketData);
			if(DEFAULT_SVR()) DEFAULT_SVR()->SetSubscribeStatus(event.GetString().c_str(),GID_DLG_MOVEPOSITION);
