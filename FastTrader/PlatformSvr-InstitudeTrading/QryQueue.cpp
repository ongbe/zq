...
//bool CQryQueue::m_bDoingInitQry=false;                        //正在进行初始化查询，此时不能查费率
...
//        ReqRlt=-999;
        bDataErr=false;
        bDataIngore=false;
        //提前将m_bInQry置为true，避免服务端处理太快，已经得到结果后，再执行m_bInQry=true，从而导致超时的情况
        m_bInQry=true;
        switch(PackageHead.CmdID)
        {
            case QryID_ReqQryInstrument:                    //查询合约信息
                ...
                LOG_INFO("CQryQueue::SendQryCmd : (查询合约信息)ReqQryInstrument : InstrumentID=%s Rlt=%d",((PlatformStru_QryInstrument*)Buf)->Thost.InstrumentID,ReqRlt);
                break;
...
//                        ReqRlt=-1;                          //已经有了，不用查了
                    ...
                    else 
                    {
                        ...
                        else
                        {
                            ...
                            LOG_INFO("CQryQueue::SendQryCmd : (查询合约手续费率)ReqQryInstrumentCommissionRate，InstrumentID=%s Rlt=%d",((CThostFtdcQryInstrumentCommissionRateField*)Buf)->InstrumentID,ReqRlt);
                            if(ReqRlt==0)
                            {
...
//                        ReqRlt=-1;                          //已经有了，不用查了
                    ...
                    else 
                    {
                        ...
                        else
                        {
                            ...
                            LOG_INFO("CQryQueue::SendQryCmd : (查询合约保证金率)ReqQryInstrumentMarginRate，InstrumentID=%s Rlt=%d",((CThostFtdcQryInstrumentMarginRateField*)Buf)->InstrumentID,ReqRlt);
                            if(ReqRlt==0)
                            {
