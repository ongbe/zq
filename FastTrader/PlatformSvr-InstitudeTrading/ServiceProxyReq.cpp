...
int CServiceProxy::ReqOrderAction(PlatformStru_InputOrderAction &InputOrderAction, int nRequestID)
{
	...
	// 禁止撤销强平报单
	OrderKey inOrderKey(InputOrderAction.Thost.InvestorID, InputOrderAction.Thost.InstrumentID, 
			InputOrderAction.Thost.FrontID, InputOrderAction.Thost.SessionID, 
			InputOrderAction.Thost.OrderRef);
	PlatformStru_OrderInfo outData;
    if(GetOrder(inOrderKey, outData) && outData.UserForceClose)
		return -1;
