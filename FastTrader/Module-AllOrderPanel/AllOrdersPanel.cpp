...
void CAllOrdersPanel::OnSubscrible(wxCommandEvent& evt)
{
    ...
    for(int i=0;i<(int)Svrs.size();i++)
    {
	    if(evt.GetInt())
	    {
            ...
            {
		        Svrs[i]->SubscribeBusinessData(BID_RspOrderInsert, m_nGID, OrderInsertBackFunc1);	//订阅报单录入回执
		        Svrs[i]->SubscribeBusinessData(BID_ErrRtnOrderInsert, m_nGID, OrderInsertBackFunc2);	//订阅报单录入回执
		        ...
		        Svrs[i]->SubscribeBusinessData(BID_RtnTrade, m_nGID, TradeCallBackFunc);	//订阅成交回报
		        Svrs[i]->SubscribeBusinessData(BID_RspForQuoteInsert, m_nGID, RspForQuoteInsertCallBackFunc);	//订阅成交回报
		        Svrs[i]->SubscribeBusinessData(BID_ErrRtnForQuoteInsert, m_nGID, ErrRtnForQuoteInsertCallBackFunc);	//订阅成交回报
		        Svrs[i]->SubscribeBusinessData(BID_RtnExecOrder, m_nGID, RtnExecOrderCallBackFunc);	//订阅成交回报
		        Svrs[i]->SubscribeBusinessData(BID_RspExecOrderInsert, m_nGID, RspExecOrderInsertCallBackFunc);	//订阅成交回报
		        Svrs[i]->SubscribeBusinessData(BID_ErrRtnExecOrderInsert, m_nGID, ErrRtnExecOrderInsertCallBackFunc);	//订阅成交回报
            ...
            if(m_nViewStyle==conAllOrderStyle||m_nViewStyle==conOpenOrderStyle)
            {
		        ...
		        Svrs[i]->SubscribeBusinessData(BID_RtnOrder, m_nGID, RtnOrderCallBackFunc);	//订阅报单回报
            }
	    }
	    else
	    {
            if(m_nViewStyle==conAllOrderStyle)
            {
			    Svrs[i]->UnSubscribeBusinessData(BID_RspOrderInsert, m_nGID);	//订阅报单录入回执
			    Svrs[i]->UnSubscribeBusinessData(BID_ErrRtnOrderInsert, m_nGID);	//订阅报单录入回执
			    Svrs[i]->UnSubscribeBusinessData(BID_RspOrderAction, m_nGID);
			    Svrs[i]->UnSubscribeBusinessData(BID_ErrRtnOrderAction, m_nGID);
			    Svrs[i]->UnSubscribeBusinessData(BID_RtnTrade, m_nGID);	//订阅成交回报
 			    Svrs[i]->UnSubscribeBusinessData(BID_RspForQuoteInsert, m_nGID);	//订阅成交回报
			    Svrs[i]->UnSubscribeBusinessData(BID_ErrRtnForQuoteInsert, m_nGID);	//订阅成交回报
 			    Svrs[i]->UnSubscribeBusinessData(BID_RtnExecOrder, m_nGID);	//订阅成交回报
			    Svrs[i]->UnSubscribeBusinessData(BID_RspExecOrderInsert, m_nGID);	//订阅成交回报
			    Svrs[i]->UnSubscribeBusinessData(BID_ErrRtnExecOrderInsert, m_nGID);	//订阅成交回报
			    //Svrs[i]->UnSubscribeBusinessData(BID_RspExecOrderAction, m_nGID);	//订阅成交回报
			    //Svrs[i]->UnSubscribeBusinessData(BID_ErrRtnExecOrderAction, m_nGID);	//订阅成交回报
           }
            if(m_nViewStyle==conAllOrderStyle||m_nViewStyle==conOpenOrderStyle)
            {
			    ...
			    Svrs[i]->SubscribeBusinessData(BID_RspQryDepthMarketData, m_nGID, QryDepthMarketDataCallBackFunc);
			    Svrs[i]->UnSubscribeBusinessData(BID_RtnOrder, m_nGID);	//报单回报(报单录入与报单操作都会产生该回报)	
            }
	    }
    }
}
...
//组合报单计算冻结时需要行情的昨结算价，需要查询。此处处理行情查询结果
void CAllOrdersPanel::OnRspQryDepthMarketData(wxCommandEvent& evt)
{
    ...
    CFileOpr::getObj().writelocallog("报单冻结UI",buf);
}
...
void CAllOrdersPanel::OnRspForQuoteInsert(wxCommandEvent& evt)
{
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CONMENO_FORMAT,"%s\n%s"),//文本必须包含%s,%s,%s,%s,%s,%s,%s
				rspForQuoteInsert.InputForQuoteField.InstrumentID,
				strStatusMsg.c_str());
	        m_pTradeInfoDlg->ShowTradeMsg(info, -1, LOADSTRING(询价回报), -1, NeedOrderSuccessConfirm());
...
void CAllOrdersPanel::OnErrRtnForQuoteInsert(wxCommandEvent& evt)
{
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CANCEL_ORDER_FORMAT,"合约: %s\n错误信息: \n%s"),//文本必须包含%s,%s,%s,%s,%s,%s,%s
				errRtnForQuoteInsert.InputForQuoteField.InstrumentID,
				strStatusMsg.c_str());
	        m_pTradeInfoDlg->ShowTradeMsg(info, -1, LOADSTRING(询价失败), -1, NeedOrderFailConfirm());

			LOG_DATA* pData = new LOG_DATA;
	        pData->strTitle = "询价失败";
			pData->strFormat = "合约: %s, 错误信息: %s";
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CANCEL_ORDER_FORMAT,"合约: %s\n行权: %s\n数量: %d\n状态信息: \n%s"),
				...
				rtnExecOrder.ExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃",
				rtnExecOrder.ExecOrderField.Volume,
				strStatusMsg.c_str());
	        m_pTradeInfoDlg->ShowTradeMsg(info, -1, LOADSTRING(期权执行回报), -1, NeedOrderSuccessConfirm());

			LOG_DATA* pData = new LOG_DATA;
	        pData->strTitle = "期权执行回报";
			pData->strFormat = "合约: %s\n行权: %s\n数量: %d\n状态信息: \n%s";
			pData->vParam.push_back(LOG_PARAM(rtnExecOrder.ExecOrderField.InstrumentID));
			pData->vParam.push_back(LOG_PARAM(rtnExecOrder.ExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃"));
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CANCEL_ORDER_FORMAT,"合约: %s\n行权: %s\n数量: %d\n状态信息: \n%s"),
				...
				rspExecOrderInsert.InputExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃",
				rspExecOrderInsert.InputExecOrderField.Volume, 
				strStatusMsg.c_str());
	        m_pTradeInfoDlg->ShowTradeMsg(info, -1, LOADSTRING(期权执行错误), -1, NeedOrderFailConfirm());

			LOG_DATA* pData = new LOG_DATA;
	        pData->strTitle = "期权执行错误";
			pData->strFormat = "合约: %s\n行权: %s\n数量: %d\n状态信息: \n%s";
			pData->vParam.push_back(LOG_PARAM(rspExecOrderInsert.InputExecOrderField.InstrumentID));
			pData->vParam.push_back(LOG_PARAM(rspExecOrderInsert.InputExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃"));
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CANCEL_ORDER_FORMAT,"合约: %s\n行权: %s\n数量: %d\n状态信息: \n%s"),
				...
				errRtnExecOrderInsert.InputExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃",
			...
			pData->vParam.push_back(LOG_PARAM(errRtnExecOrderInsert.InputExecOrderField.ActionType==THOST_FTDC_ACTP_Exec ? "执行" : "放弃"));
    ...
    if(m_nViewStyle==conAllOrderStyle) 
    {
	    ...
	    if(m_pTradeInfoDlg)
        {
			...
			info.Printf(LOADFORMATSTRING(CANCEL_ORDER_FORMAT,"合约: %s\n交易所: %s, 行权编号: %s\n错误信息: \n%s"),
				...
				strStatusMsg.c_str());
	        m_pTradeInfoDlg->ShowTradeMsg(info, -1, LOADSTRING(期权行权撤销错误), -1, NeedOrderFailConfirm());

			LOG_DATA* pData = new LOG_DATA;
	        pData->strTitle = "期权行权撤销错误";
			pData->strFormat = "合约: %s\n交易所: %s, 行权编号: %s\n错误信息: \n%s";
