...
COrderInsertEntity* COrderInsertEntity::CreateOrderInsert(ORIORDERREQ& tOriReq)
{
	...
	if(tOrderReqBreak.bIsOpen) {
		pEntity->InsertOrder(strInstrumentID, strAccount, tOriReq.bDirection, 0, 
				...
				tOriReq.nHedgeStatus, tOriReq.bIsMarketPrice, tOriReq.bSimulate, 
				tOriReq.bIsParketOrder, tOriReq.nOrderType, tOriReq.nMinVolume);
	}
...
BOOL COrderInsertEntity::GetHoldingVolume(string strCode, BOOL bIsBuy, int nHedgeStatus, 
										  int& nSumVolume, int& nSumTodayVolume, string strAccount)
{
	char cDirect = bIsBuy ? THOST_FTDC_PD_Long : THOST_FTDC_PD_Short;
	char cHedge = nHedgeStatus + THOST_FTDC_HF_Speculation;
...
BOOL COrderInsertEntity::GetAskCloseVolume(string strCode, BOOL bIsBuy, int nHedgeStatus, 
...
BOOL COrderInsertEntity::GetPreVolume(string strCode, BOOL bIsBuy, int nHedgeStatus, 
									  int& nPreVolume, int& nPreTodayVolume, string strAccount)
{
	int nSumVolume = 0, nSumTodayVolume = 0;
	int nAskVolume = 0, nAskTodayVolume = 0;

	if(!GetHoldingVolume(strCode, bIsBuy, nHedgeStatus, nSumVolume, nSumTodayVolume, strAccount)) return FALSE;
	if(!GetAskCloseVolume(strCode, !bIsBuy, nHedgeStatus, nAskVolume, nAskTodayVolume, strAccount)) return FALSE;
...
BOOL COrderInsertEntity::AnalyseOrderReq(ORIORDERREQ& tOriReq, ORDERREQBREAK& tOrderReqBreak)
{
	...
	if(!tOriReq.bIsAuto) {
		...
		GetHoldingVolume(strInstrumentID.c_str(), !tOriReq.bDirection, tOriReq.nHedgeStatus, nSumVolume, nSumTodayVolume, string(tOriReq.strAccount));
...
BOOL COrderInsertEntity::InsertOrder(wxString& strInstrumentID,  wxString& strAccount, 
									 ...
									 int nHedgeStatus, BOOL bIsMarketPrice, BOOL bSimulate, 
									 BOOL bIsParketOrder, int nOrderType, int nMinVolume)
{
	...
	if(bIsParketOrder)
		MakeReq_Parked(m_tRspOrderReqs[n].order.parked, strName, strAcct, nOrderType, 
				bDirection, nOCMode, nVolume, fPrice, 0, nHedgeStatus, bIsMarketPrice, FALSE, nMinVolume);
	else
		MakeReq_Order(m_tRspOrderReqs[n].order.insert, strName, strAcct, nOrderType, 
				bDirection, nOCMode, nVolume, fPrice, 0, nHedgeStatus, bIsMarketPrice, FALSE, nMinVolume);
	m_nRspOrderReqCount++;
...
BOOL COrderInsertEntity::DoOrder(int& nErrPos, wxString& strRetMsg)
{
	...
	for(int i=0; i<m_nRspOrderReqCount; ++i) {
        ...
        {
			nRet = VerifyOrder(m_tRspOrderReqs[i].order.insert);
        }
		if(nRet!=0) {
			nErrPos = i;
		...
		for(i=0;i<count;i++) 
        {
			...
			if(strInstrumentID.compare(field.InstrumentID)==0 && 
					...
					(field.OrderStatus==THOST_FTDC_OST_PartTradedQueueing || 
					 ...
					 field.OrderStatus==THOST_FTDC_OST_Unknown)) {

				CThostFtdcInputOrderActionField ReqData={0};
...
BOOL COrderInsertEntity::FindRspOrderAction(DataErrRtnOrderAction& act)
{
	wxString strTmpSysID;
