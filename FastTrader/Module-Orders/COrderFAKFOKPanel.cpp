...
wxPanel(parent,winid,pos,size,style,name)
{
	...
	ctlSz.Set(nFW*7, nFH);
	wxRadioButton* itemRadioBox1 = new wxRadioButton( this, ID_FAKFOK_RADIOFAK, wxEmptyString, ctlPos, ctlSz);//输入价//, wxRB_GROUP);
	itemRadioBox1->SetValue(true);
	ctlPos = wxPoint(posInit.x+nFW*7+nFW/2, posInit.y+4);
	ctlSz.Set(nFW*7, nFH);
	wxRadioButton* itemRadioBox2 = new wxRadioButton( this, ID_FAKFOK_RADIOFOK, wxEmptyString, ctlPos, ctlSz);//最新//, wxRB_GROUP);

	ctlPos = wxPoint(posInit.x, posInit.y+nVH+2);
	ctlSz.Set(nFW*10, nFH);
	wxStaticText* itemStaticText1 = new wxStaticText( this, ID_FAKFOK_STATICE1, wxEmptyString, ctlPos, ctlSz, wxALIGN_RIGHT);//每批手数
