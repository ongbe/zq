...
bool CQueryOrderParkedPanel::Init()
{
	...
	nColCount = pCfgMgr->GetListVisibleColCount(GID_QUERY_ORDER_PARKED);
...
void CQueryOrderParkedPanel::ShowLocalParkeds(int& nRowID, int nShowMode, BOOL bIsShowDelete)
{
	...
	CLocalOrderService::MAPPARKEDORDER mapParked = m_poLocalOrderService->GetOrderParkedMap();
	m_poLocalOrderService->UnlockObject();
	CLocalOrderService::MAPPARKEDORDERITEM it = mapParked.begin();

	wxString strKey;
	string strkeytmp;
	for(; it!=mapParked.end(); ++it) {
		CLocalOrderService::PARKEDORDER& tParkedOrder = it->second;
...
void CQueryOrderParkedPanel::OnTimer( wxTimerEvent& event )
{
	if(m_bNeedQueryParked && ::GetTickCount() - m_nLastStatusTime > 1000) {
		int nRet = 0;
...
//	};
	...
	case conTypeParkedLocal:
	{
		m_poLocalOrderService->LockObject();
		CLocalOrderService::MAPPARKEDORDER& mapParked = m_poLocalOrderService->GetOrderParkedMap();
		CLocalOrderService::MAPPARKEDORDERITEM it = mapParked.find(nNum);

		BOOL bFound = (it != mapParked.end());
		CLocalOrderService::PARKEDORDER tParkedOrder={0};
		if(bFound) 
		...
		if(bFound) 	{
			if(tParkedOrder.nState < conParkedDelete) {
				...
				if(pDlg->ShowModal()==wxID_OK) {
					...
					if(it != mapParked.end()) {
						m_poLocalOrderService->OrderParkedSet(nNum, tParkedOrder);
...
void CQueryOrderParkedPanel::OnButtonDelete(wxCommandEvent& evt)
{
	...
	case conTypeParkedLocal:
	{
		...
		if(bFound) {
			if(tParkedOrder.nState < conParkedDelete) {
				m_poLocalOrderService->LockObject();
				BOOL bDelete = m_poLocalOrderService->OrderParkedDelete(nNum);
				m_poLocalOrderService->UnlockObject();
				if(bDelete) {
...
void CQueryOrderParkedPanel::OnButtonDoNow(wxCommandEvent& evt)
{
	...
	case conTypeParkedLocal:
	{
		...
		if(bFound) {
			...
			m_poLocalOrderService->LockObject();
			bRet = m_poLocalOrderService->OrderParkedDoNow(nNum);
			m_poLocalOrderService->UnlockObject();
			//it = mapParked.find(nNum);
...
void CQueryOrderParkedPanel::OnButtonClear(wxCommandEvent& evt)
{
	...
	// 删除所有本地预埋单 
	{
		CLocalOrderService::MAPPARKEDORDERITEM it;
		...
		for(int i=0; i<(int)arrDelete.size(); i++) {
			m_poLocalOrderService->OrderParkedDelete(arrDelete[i]);
		}
		arrDelete.clear();
...
void CQueryOrderParkedPanel::OnContext(wxContextMenuEvent& evt)
{
	...
	if(menu.GetMenuItemCount()>0)
	{
		...
		if(retcmd - ID_MENU_ADDTOMYSELF >= 0 && retcmd - ID_MENU_ADDTOMYSELF < 50) {
			...
			//char strTemp[1024];
			string strSysID;
			CLocalOrderService::PARKEDORDER tOrder;
					...
					case conTypeParkedLocal:
						{
							...
							CLocalOrderService::MAPPARKEDORDER& mapParkedSub = m_poLocalOrderService->GetOrderParkedMap();
							CLocalOrderService::MAPPARKEDORDERITEM it=mapParkedSub.find(nNum);
...
//#define THOST_FTDC_IS_Closed '6'
    ...
    {
		m_bNeedQueryParked = true;
		m_nLastStatusTime = ::GetTickCount();
	}
	delete pStatus;
