	...
	m_fPriceTick(1.0f)
{
	...
	ZeroMemory(&m_tConditionOrder, sizeof(m_tConditionOrder));
...
void CEditOrderConditionDailog::ShowOrderReq()
{
	...
	if(DEFAULT_SVR()&&DEFAULT_SVR()->GetQuotInfo(m_tConditionOrder.InstrumentID,field)!=0) {
    }
	...
	CLocalOrderService::ConvertLocal2ReqOrder(m_tConditionOrder, req);
	m_poOrderInputPanel->ShowOrder(req);
}
...
void CEditOrderConditionDailog::ShowOrderCondition()
{
	...
	m_poOrderConditionPanel->SetPriceType(m_tConditionOrder.cond.nPriceType);
	m_poOrderConditionPanel->SetConditionType(m_tConditionOrder.cond.nConditionType);
	m_poOrderConditionPanel->SetConditionPrice(m_tConditionOrder.cond.fConditionPrice);
}
...
bool CEditOrderConditionDailog::Show( bool show )
{
	...
	ShowOrderCondition();
	wxString strText;
	strText.Printf(LOADFORMATSTRING(EOCD_ECLCO,"%d"), m_tConditionOrder.FTID);//文本必须包含%d
...
{
	m_tConditionOrder.cond.nPriceType = m_poOrderConditionPanel->GetPriceType();
	m_tConditionOrder.cond.nConditionType = m_poOrderConditionPanel->GetConditionType();
	m_tConditionOrder.cond.fConditionPrice = m_poOrderConditionPanel->GetConditionPrice();
	...
	if(CheckSumPrice(m_tConditionOrder.cond.fConditionPrice, m_fPriceTick)) {
	...
	m_tConditionOrder.orderSrc.nOCMode = m_poOrderInputPanel->GetOpenClose();
	m_tConditionOrder.orderSrc.bIsBuy = m_poOrderInputPanel->GetDirection();
	m_tConditionOrder.orderSrc.bIsMarket = 
			stricmp(m_poOrderInputPanel->GetPriceText().c_str(), LOADSTRING(OLS_MARKET))==0;
	m_tConditionOrder.orderSrc.fPrice = m_poOrderInputPanel->GetPrice();
	m_tConditionOrder.orderSrc.nVolume = m_poOrderInputPanel->GetQty();
	m_tConditionOrder.orderSrc.nHedgeStatus = m_poOrderInputPanel->GetHedge();

	USERLOG_INFO("用户编辑条件单，对话框中单击[确认]确认\n%s%s", 
