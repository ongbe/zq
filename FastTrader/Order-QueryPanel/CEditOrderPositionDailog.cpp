	...
	m_fPriceTick(1.0f)
{
	...
	ZeroMemory(&m_tPLOrder, sizeof(m_tPLOrder));
...
void CEditOrderPositionDailog::ShowOrderReq()
{
    ...
    if(DEFAULT_SVR()&&DEFAULT_SVR()->GetQuotInfo(m_tPLOrder.InstrumentID,field)!=0) {
    }
...
//	m_poOrderInputPanel->LockForSetChange();
	strText=m_tPLOrder.InstrumentID;
	m_poOrderInputPanel->SetInstrumentId(strText, FALSE);
	m_poOrderInputPanel->SetDirection(m_tPLOrder.orderSrc.bIsBuy);
	m_poOrderInputPanel->SetOpenClose(0);
	m_poOrderInputPanel->SetPrice(m_tPLOrder.orderSrc.fPrice);
	m_poOrderInputPanel->SetQty(m_tPLOrder.orderSrc.nVolume);
...
void CEditOrderPositionDailog::ShowOrderPosition()
{
	...
	m_poOrderPositionPanel->SetIsRunInServer(m_tPLOrder.bIsRunInServer);
	m_poOrderPositionPanel->SetPriceTick(m_fPriceTick);
	m_poOrderPositionPanel->SetOffsetPriceObject(m_tPLOrder.plparam.nPriceOffsetType);
	m_poOrderPositionPanel->SetLoseOffsetPrice(
			m_tPLOrder.plparam.bDoStopLose, m_tPLOrder.plparam.fStopLosePriceOffset);
	m_poOrderPositionPanel->SetGainOffsetPrice(
			m_tPLOrder.plparam.bDoStopGain, m_tPLOrder.plparam.fStopGainPriceOffset);
	m_poOrderPositionPanel->SetPriceType(m_tPLOrder.plparam.nPriceType);
	m_poOrderPositionPanel->SetCloseMode(m_tPLOrder.plparam.nCloseMode);
	m_poOrderPositionPanel->SetCloseOffsetPrice(m_tPLOrder.plparam.fClosePriceOffset);
}
...
bool CEditOrderPositionDailog::Show( bool show )
{
	if(show) {
		...
		ShowOrderPosition();
		wxString strText;
		strText.Printf(LOADFORMATSTRING(EOPD_ECLSO,"%d"), m_tPLOrder.FTID);//文本必须包含%d
...
void CEditOrderPositionDailog::OnOk(wxCommandEvent& event)
{
	...
	m_tPLOrder.bIsRunInServer = m_poOrderPositionPanel->GetIsRunInServer();

	m_tPLOrder.plparam.nPriceOffsetType = m_poOrderPositionPanel->GetOffsetPriceObject();
	m_tPLOrder.plparam.bDoStopLose = m_poOrderPositionPanel->GetLoseOffsetPrice(
			m_tPLOrder.plparam.fStopLosePriceOffset);
	m_tPLOrder.plparam.bDoStopGain = m_poOrderPositionPanel->GetGainOffsetPrice(
			m_tPLOrder.plparam.fStopGainPriceOffset);
	m_tPLOrder.plparam.nPriceType = m_poOrderPositionPanel->GetPriceType();
	m_tPLOrder.plparam.nCloseMode = m_poOrderPositionPanel->GetCloseMode();
	m_tPLOrder.plparam.fClosePriceOffset = m_poOrderPositionPanel->GetCloseOffsetPrice();
	...
	//			+ fPriceOffset 
	//			- m_tPLOrder.plparam.fStopGainPriceOffset;	//止盈价
	if(!m_tPLOrder.plparam.bDoStopLose) {
		m_tPLOrder.plparam.fStopLosePriceOffset = 0.0f;
		//m_tPLOrder.fStopLosePrice = 0.0f;
	}
	if(!m_tPLOrder.plparam.bDoStopGain) {
		m_tPLOrder.plparam.fStopGainPriceOffset = 0.0f;
		//m_tPLOrder.fStopGainPrice = 0.0f;
	}

	USERLOG_INFO("用户编辑盈损单，对话框中单击[确认]确认\n%s%s", 
