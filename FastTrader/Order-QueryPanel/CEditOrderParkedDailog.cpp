	...
	m_fPriceTick(1.0f)
{
	...
	ZeroMemory(&m_tParkedOrder, sizeof(m_tParkedOrder));
...
void CEditOrderParkedDailog::FillParked(CLocalOrderService::PARKEDORDER& tParked)
{
...
wxString CEditOrderParkedDailog::GetParkedLog()
{
	wxString strText;
	wxRadioButton* pRadio = (wxRadioButton*)FindWindowById(ID_EDITPARKED_RADIO_AUTO, this);
	if(pRadio) {
		if(pRadio->GetValue())
			//strText = ((wxWindow*)pRadio)->GetLabel();
			strText = pRadio->GetLabel();
	}
	pRadio = (wxRadioButton*)FindWindowById(ID_EDITPARKED_RADIO_HANDY, this);
	if(pRadio) {
		if(pRadio->GetValue())
...
//	m_poOrderInputPanel->FillOrder(m_tParkedOrder.req);
	...
	USERLOG_INFO("用户编辑预埋单，对话框中单击[确认]确认\n%s\n%s", 
