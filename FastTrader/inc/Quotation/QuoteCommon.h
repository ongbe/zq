...
inline void GetFullStr(wxString& str, double fValue)
{
	wxString strTemp = str;
	if(isInvalidValue(fValue))
	{
		str.Printf(_T("%s - "), strTemp);
	}
	else
	{
		str.Printf(_T("%s %.2f "), strTemp, fValue);
	}
}
inline void GetFullStr(wxString& str, int nValue, int n)
{
	wxString strTemp = str;
	...
	{
		str.Printf(_T("%s %7d "), strTemp, nValue);
	}
}
inline void AppendLog_DepthMarketData(PlatformStru_DepthMarketData* pData)
{
	...
	wxString str;
	str.Printf(wxT("%s %s %s"), pData->TradingDay, 	pData->UpdateTime, 	pData->InstrumentID);
