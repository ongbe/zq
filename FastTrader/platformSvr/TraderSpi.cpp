...
m_RefPlatFormService(RefPlatFormService)
{
    ...
    //从productshortname.xml中读取产品简称不等于产品名称的产品列表
//    map<string,string>      m_mapExtProductShortName;
    CMyXML myxml("productshortname.xml");
    if(myxml.m_bvalid)
    {
        vector<pair<string,string>> SubNodes;
        myxml.GetSubNodes(SubNodes,"root","ProductShortName");
        for(int i=0;i<(int)SubNodes.size();i++)
            m_mapExtProductShortName[SubNodes[i].first]=SubNodes[i].second;
    }
...
{
    if(bIsLast) m_RefPlatFormService.m_bInQry=false;
...
void CTraderSpi::OnRspOrderAction(CThostFtdcInputOrderActionField *pInputOrderAction, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspOrderAction tmp(nRequestID,bIsLast,*pInputOrderAction,pRspInfo?(*pRspInfo):BlankRspInfo);
...
void CTraderSpi::OnRspQryOptionInstrCommRate(CThostFtdcOptionInstrCommRateField *pOptionInstrCommRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspQryOptionInstrCommRate tmp(nRequestID,bIsLast,InstrumentCommissionRateField,RspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspQryOptionInstrCommRate, 0, 0, 0, sizeof(tmp), &tmp);
...
void CTraderSpi::OnRspQryOptionInstrTradeCost(CThostFtdcOptionInstrTradeCostField *pOptionInstrTradeCost, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspQryOptionInstrTradeCost tmp(nRequestID,bIsLast,InstrumentMarginRateField,RspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspQryOptionInstrTradeCost, 0, 0, 0, sizeof(tmp), &tmp);
...
void CTraderSpi::OnRspQryProduct(CThostFtdcProductField *pProduct, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    if(!pProduct)
    {
        LOG_INFO("OnRspQryProduct(请求查询产品响应),pProduct is NULL");
        return;
    }

    //在6.3.0api中，ExchangeProductID为无效值。这里过滤掉其中ch<32 or ch>=128 or ch==' or ch==" or ch==< or ch==>的字符，转换为空格。避免后面存xml时出错
    if(pProduct->ExchangeProductID[sizeof(pProduct->ExchangeProductID)-1]!=0) pProduct->ExchangeProductID[sizeof(pProduct->ExchangeProductID)-1]=0;
    for(int i=0;pProduct->ExchangeProductID[i]!=0;i++)
    {
        unsigned char ch=(unsigned char)(pProduct->ExchangeProductID[i]);
        if(ch<32||ch>=128||ch=='\''||ch=='\"'||ch=='<'||ch=='>') pProduct->ExchangeProductID[i]=' ';
    }
    ...
    //ProductName太长则定制一下
    string ShortName;
    if(CTools_Ansi::GetValueInMap<string,string>(m_mapExtProductShortName,string(ProductField.ProductID),ShortName))
    {
        memset(ProductField.ProductShortName,0,sizeof(ProductField.ProductShortName));
        strncpy(ProductField.ProductShortName,ShortName.c_str(),sizeof(ProductField.ProductShortName)-1);
    }

    DataRspQryProduct tmp(nRequestID,bIsLast,ProductField,pRspInfo?(*pRspInfo):BlankRspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspQryProduct, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnRspQryInstrument(CThostFtdcInstrumentField *pInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    //确定组合保证金优惠模式，配置项的0-4有效：0-不优惠(缺省)，1-第一腿优惠，2-第二腿优惠，3-按套利组合中高腿收取，4-上期大额单边
    if(strcmp(InstrumentField.ExchangeID,"CZCE")==0) InstrumentField.CombMarginDiscountMode=GlobalConfigManager::m_DiscountCZCE;
    else if(strcmp(InstrumentField.ExchangeID,"DCE")==0) InstrumentField.CombMarginDiscountMode=GlobalConfigManager::m_DiscountDCE;
    else if(strcmp(InstrumentField.ExchangeID,"SHFE")==0) InstrumentField.CombMarginDiscountMode=GlobalConfigManager::m_DiscountSHFE;
    else if(strcmp(InstrumentField.ExchangeID,"CFFEX")==0) InstrumentField.CombMarginDiscountMode=GlobalConfigManager::m_DiscountCFFEX;
    else InstrumentField.CombMarginDiscountMode=0;
    if(InstrumentField.CombMarginDiscountMode<0||InstrumentField.CombMarginDiscountMode>4) InstrumentField.CombMarginDiscountMode=0;
    else if(InstrumentField.CombMarginDiscountMode==4&&InstrumentField.MaxMarginSideAlgorithm!=THOST_FTDC_MMSA_YES) InstrumentField.CombMarginDiscountMode=0;
    if(InstrumentField.MaxMarginSideAlgorithm==THOST_FTDC_MMSA_YES) InstrumentField.CombMarginDiscountMode=4;

    //确定平仓顺序: 0: 统一先开先平；1: 先单一后组合先开先平
    if(strcmp(InstrumentField.ExchangeID,"CZCE")==0) InstrumentField.UnwindPriorities=GlobalConfigManager::m_UnwindPrioritiesCZCE;
    else if(strcmp(InstrumentField.ExchangeID,"DCE")==0) InstrumentField.UnwindPriorities=GlobalConfigManager::m_UnwindPrioritiesDCE;
    else if(strcmp(InstrumentField.ExchangeID,"SHFE")==0) InstrumentField.UnwindPriorities=GlobalConfigManager::m_UnwindPrioritiesSHFE;
    else if(strcmp(InstrumentField.ExchangeID,"CFFEX")==0) InstrumentField.UnwindPriorities=GlobalConfigManager::m_UnwindPrioritiesCFFEX;
    else InstrumentField.UnwindPriorities=0;
    if(InstrumentField.UnwindPriorities<0||InstrumentField.UnwindPriorities>1) InstrumentField.UnwindPriorities=0;

    //确定今仓保证金使用的价格: 0: 按成交价计算(缺省)；1: 按昨结算价计算
    if(strcmp(InstrumentField.ExchangeID,"CZCE")==0) InstrumentField.PriceForMarginOfTodayPosition=GlobalConfigManager::m_PriceForMarginOfTodayPositionCZCE;
    else if(strcmp(InstrumentField.ExchangeID,"DCE")==0) InstrumentField.PriceForMarginOfTodayPosition=GlobalConfigManager::m_PriceForMarginOfTodayPositionDCE;
    else if(strcmp(InstrumentField.ExchangeID,"SHFE")==0) InstrumentField.PriceForMarginOfTodayPosition=GlobalConfigManager::m_PriceForMarginOfTodayPositionSHFE;
    else if(strcmp(InstrumentField.ExchangeID,"CFFEX")==0) InstrumentField.PriceForMarginOfTodayPosition=GlobalConfigManager::m_PriceForMarginOfTodayPositionCFFEX;
    else InstrumentField.PriceForMarginOfTodayPosition=0;
    if(InstrumentField.PriceForMarginOfTodayPosition<0||InstrumentField.PriceForMarginOfTodayPosition>1) InstrumentField.PriceForMarginOfTodayPosition=0;

    //对平今指令的支持，0:不支持(但ctp可能会转换成平仓)；1:支持
    if(strcmp(InstrumentField.ExchangeID,"CZCE")==0) InstrumentField.CloseTodayInstructionSupport=GlobalConfigManager::CloseTodaySupport_CZCE;
    else if(strcmp(InstrumentField.ExchangeID,"DCE")==0) InstrumentField.CloseTodayInstructionSupport=GlobalConfigManager::CloseTodaySupport_DCE;
    else if(strcmp(InstrumentField.ExchangeID,"SHFE")==0) InstrumentField.CloseTodayInstructionSupport=GlobalConfigManager::CloseTodaySupport_SHFE;
    else if(strcmp(InstrumentField.ExchangeID,"CFFEX")==0) InstrumentField.CloseTodayInstructionSupport=GlobalConfigManager::CloseTodaySupport_CFFEX;
    else InstrumentField.CloseTodayInstructionSupport=0;
    if(InstrumentField.CloseTodayInstructionSupport<0||InstrumentField.CloseTodayInstructionSupport>1) InstrumentField.CloseTodayInstructionSupport=0;

    //对平仓指令的支持，0:不支持；1:支持，可以平昨仓和今仓；2:支持，只能平昨仓
    if(strcmp(InstrumentField.ExchangeID,"CZCE")==0) InstrumentField.CloseInstructionSupport=GlobalConfigManager::CloseSupport_CZCE;
    else if(strcmp(InstrumentField.ExchangeID,"DCE")==0) InstrumentField.CloseInstructionSupport=GlobalConfigManager::CloseSupport_DCE;
    else if(strcmp(InstrumentField.ExchangeID,"SHFE")==0) InstrumentField.CloseInstructionSupport=GlobalConfigManager::CloseSupport_SHFE;
    else if(strcmp(InstrumentField.ExchangeID,"CFFEX")==0) InstrumentField.CloseInstructionSupport=GlobalConfigManager::CloseSupport_CFFEX;
    else InstrumentField.CloseInstructionSupport=0;
    if(InstrumentField.CloseInstructionSupport<0||InstrumentField.CloseInstructionSupport>1) InstrumentField.CloseInstructionSupport=0;

    //检查这个产品类型是否被支持
    if(!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_All)&&
        (pInstrument->ProductClass==THOST_FTDC_PC_Futures&&!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_Futures)||
         pInstrument->ProductClass==THOST_FTDC_PC_Options&&!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_Options)||
         pInstrument->ProductClass==THOST_FTDC_PC_Combination&&!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_Combination)||
         pInstrument->ProductClass==THOST_FTDC_PC_Spot&&!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_Spot)||
         pInstrument->ProductClass==THOST_FTDC_PC_EFP&&!GlobalConfigManager::IsSupportProductClassType(GlobalConfigManager::PC_EFP)))
...
void CTraderSpi::OnRspQryDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    if(pDepthMarketData)
    {
        ...
        DataRspQryDepthMarketData tmp(nRequestID,bIsLast,field,pRspInfo?(*pRspInfo):BlankRspInfo);
...
void CTraderSpi::OnRtnOrder(CThostFtdcOrderField *pOrder)
{
    ...
    PlatformStru_OrderInfo OrderField(*pOrder);
...
void CTraderSpi::OnRtnTrade(CThostFtdcTradeField *pTrade)
{
    ...
    PlatformStru_TradeInfo  TradeField(*pTrade);
...
void CTraderSpi::OnRspQryBrokerTradingParams(CThostFtdcBrokerTradingParamsField *pBrokerTradingParams, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {
        LOG_INFO("OnRspQryBrokerTradingParams(请求查询经纪公司交易参数响应),pBrokerTradingParams is NULL");
        return;
    }

    PlatformStru_BrokerTradingParams BrokerTradingParams(*pBrokerTradingParams);
    DataRspQryBrokerTradingParams tmp(nRequestID,bIsLast,BrokerTradingParams,pRspInfo?(*pRspInfo):BlankRspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspQryBrokerTradingParams, 0, 0, 0, sizeof(tmp), &tmp);
...
//                        pRspTransfer->CustType, pRspTransfer->BankAccount,pRspTransfer->AccountID,
...
void CTraderSpi::OnRspForQuoteInsert(CThostFtdcInputForQuoteField *pInputForQuote, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspForQuoteInsert tmp(nRequestID,bIsLast,PlatformStru_InputForQuoteField(InputForQuote),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspForQuoteInsert, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnRspExecOrderInsert(CThostFtdcInputExecOrderField *pInputExecOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspExecOrderInsert tmp(nRequestID,bIsLast,PlatformStru_InputExecOrderField(InputExecOrder),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspExecOrderInsert, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnRspExecOrderAction(CThostFtdcInputExecOrderActionField *pInputExecOrderAction, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspExecOrderAction tmp(nRequestID,bIsLast,PlatformStru_InputExecOrderActionField(InputExecOrderAction),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_RspExecOrderAction, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnRspQryExecOrder(CThostFtdcExecOrderField *pExecOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    DataRspQryExecOrder tmp(nRequestID,bIsLast,PlatformStru_ExecOrderField(ExecOrder),RspInfo);

    if(bIsLast)  m_RefPlatFormService.m_bInQry=false;

    m_RefPackageCache.SaveDataTo(BID_RspQryExecOrder, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnErrRtnForQuoteInsert(CThostFtdcInputForQuoteField *pInputForQuote, CThostFtdcRspInfoField *pRspInfo)
{
    ...
    DataErrRtnForQuoteInsert tmp(PlatformStru_InputForQuoteField(InputForQuote),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_ErrRtnForQuoteInsert, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnErrRtnExecOrderInsert(CThostFtdcInputExecOrderField *pInputExecOrder, CThostFtdcRspInfoField *pRspInfo)
{
    ...
    DataErrRtnExecOrderInsert tmp(PlatformStru_InputExecOrderField(InputExecOrder),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_ErrRtnExecOrderInsert, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnErrRtnExecOrderAction(CThostFtdcExecOrderActionField *pExecOrderAction, CThostFtdcRspInfoField *pRspInfo)
{
    ...
    DataErrRtnExecOrderAction tmp(PlatformStru_ExecOrderActionField(ExecOrderAction),RspInfo);

    m_RefPackageCache.SaveDataTo(BID_ErrRtnExecOrderAction, 0, 0, 0, sizeof(tmp), &tmp); 
...
void CTraderSpi::OnRtnExecOrder(CThostFtdcExecOrderField *pExecOrder)
{
    if(!pExecOrder)
    {
        LOG_INFO("OnRtnExecOrder(执行宣告通知),pExecOrder is NULL");
        return;
    }

    PlatformStru_ExecOrderField ExecOrderField(*pExecOrder);
    DataRtnExecOrder tmp(ExecOrderField);

    int OriSetOff=-1,NewSetOff=-1;
    bool bSave=m_RefPackageCache.SaveDataTo(BID_RtnExecOrder, 0, 0, 0, sizeof(tmp), &tmp);//,&OriSetOff,&NewSetOff);
    LOG_INFO("OnRtnExecOrder(执行宣告通知) : \n\t\t\t bSaveOK=%d OriSetOff=%d NewSetOff=%d SaveLen=%d\n%s",
                    bSave,OriSetOff,NewSetOff,sizeof(tmp),tmp.ExecOrderField.tostring().c_str());
}
