...
const wxString CFieldValueTool::TradeType2String(char TradeType,int language)
{
	...
	case THOST_FTDC_TRDT_SplitCombination:
		return LOADSTRING(CFieldValueTool_SplitCombination);
		break;
	default:
		return LOADSTRING(CFieldValueTool_UNKOWN);//__UNKOWN__;
		break;
...
const wxString CFieldValueTool::TradeType2CombState(char TradeType,int language)
{
	switch(TradeType)
	{
	    case THOST_FTDC_TRDT_CombinationDerived:
		    return LOADSTRING(CFieldValueTool_CombState_CombPosition);//language==LANGUAGE_CHINESE ?  CombinationDerived_Cn : CombinationDerived_En;
		    break;
	    default:
		    return LOADSTRING(CFieldValueTool_CombState_CommonPosition);//__UNKOWN__;
		    break;
    }
}
...
const wxString CFieldValueTool::ForceCloseReason2String(char param,int language)
{
	...
	case THOST_FTDC_FCC_ForceClose:
		return LOADSTRING(CFieldValueTool_ITTForceClose);//language==LANGUAGE_CHINESE ?  Other_Cn : Other_En;
		break;
	default:
		return LOADSTRING(CFieldValueTool_UNKOWN);//__UNKOWN__;
		break;
...
const char CFieldValueTool::String2ForceCloseReason(wxString Type,int* language)
{
	...
	COMPARELANGUAGE(Type, CFieldValueTool_ITTForceClose, THOST_FTDC_FCC_ForceClose)
...
const wxString CFieldValueTool::ProductType2String(char param,int language)
{
	...
	case THOST_FTDC_PC_Combination:
		return LOADSTRING(CFieldValueTool_CombinationInstrument);//language==LANGUAGE_CHINESE ?  Combination_Cn : Combination_En;
		break;
	...
	case THOST_FTDC_PC_SpotOption:
		return LOADSTRING(CFieldValueTool_SpotOption);//language==LANGUAGE_CHINESE ?  SpotOption_Cn : SpotOption_En;
		break;
	default:
		return LOADSTRING(CFieldValueTool_UNKOWN);//__UNKOWN__;
		break;
...
const char CFieldValueTool::String2ProductType(wxString Type,int* language)
{
	...
	COMPARELANGUAGE(Type, CFieldValueTool_CombinationInstrument, THOST_FTDC_PC_Combination)
...
///执行类型
const wxString CFieldValueTool::ActionType2String(char ActionType,int language)
{
	switch(ActionType) {
		case THOST_FTDC_ACTP_Exec:
			return LOADSTRING(执行);
			break;
		case THOST_FTDC_ACTP_Abandon:
			return LOADSTRING(放弃);
			break;
		default:
			return LOADSTRING(CFieldValueTool_UNKOWN);
			break;
	}
}

const char CFieldValueTool::String2ActionType(wxString Type,int* language)
{
	COMPARELANGUAGE(Type, 执行, THOST_FTDC_ACTP_Exec)
	COMPARELANGUAGE(Type, 放弃, THOST_FTDC_ACTP_Abandon)
	if(language) *language=0;
	return 0;
}

///期权行权后是否保留期货头寸的标记类型
const wxString CFieldValueTool::ReservePositionFlag2String(char ReservePositionFlag,int language)
{
	switch(ReservePositionFlag) {
		case THOST_FTDC_EOPF_Reserve:
			return LOADSTRING(保留);
			break;
		case THOST_FTDC_EOPF_UnReserve:
			return LOADSTRING(不保留);
			break;
		default:
			return LOADSTRING(CFieldValueTool_UNKOWN);
			break;
...
const char CFieldValueTool::String2ReservePositionFlag(wxString Type,int* language)
{
	COMPARELANGUAGE(Type, 保留, THOST_FTDC_EOPF_Reserve)
	COMPARELANGUAGE(Type, 不保留, THOST_FTDC_EOPF_UnReserve)
	if(language) *language=0;
	return 0;
}

///期权行权后生成的头寸是否自动平仓类型
const wxString CFieldValueTool::CloseFlag2String(char CloseFlag,int language)
{
	switch(CloseFlag) {
		case THOST_FTDC_EOCF_AutoClose:
			return LOADSTRING(自动平仓);
			break;
		case THOST_FTDC_EOCF_NotToClose:
			return LOADSTRING(免于自动平仓);
			break;
		default:
			return LOADSTRING(CFieldValueTool_UNKOWN);
			break;
...
const char CFieldValueTool::String2CloseFlag(wxString Type,int* language)
{
	COMPARELANGUAGE(Type, 自动平仓, THOST_FTDC_EOCF_AutoClose)
	COMPARELANGUAGE(Type, 免于自动平仓, THOST_FTDC_EOCF_NotToClose)
	if(language) *language=0;
	return 0;
}

///执行结果类型
const wxString CFieldValueTool::ExecResult2String(char ExecResult,int language)
{
	switch(ExecResult) {
		case THOST_FTDC_OER_NoExec:
			return LOADSTRING(没有执行);
			break;
		case THOST_FTDC_OER_Canceled:
			return LOADSTRING(已经取消);
			break;
		case THOST_FTDC_OER_OK:
			return LOADSTRING(执行成功);
			break;
		case THOST_FTDC_OER_NoPosition:
			return LOADSTRING(期权持仓不够);
			break;
		case THOST_FTDC_OER_NoDeposit:
			return LOADSTRING(资金不够);
			break;
		case THOST_FTDC_OER_NoParticipant:
			return LOADSTRING(会员不存在);
			break;
		case THOST_FTDC_OER_NoClient:
			return LOADSTRING(客户不存在);
			break;
		case THOST_FTDC_OER_NoInstrument:
			return LOADSTRING(合约不存在);
			break;
		case THOST_FTDC_OER_NoRight:
			return LOADSTRING(没有执行权限);
			break;
		case THOST_FTDC_OER_InvalidVolume:
			return LOADSTRING(不合理的数量);
			break;
		case THOST_FTDC_OER_NoEnoughHistoryTrade:
			return LOADSTRING(没有足够的历史成交);
			break;
		default:
			return LOADSTRING(CFieldValueTool_UNKOWN);
			break;
...
const char CFieldValueTool::String2ExecResult(wxString Type,int* language)
{
	COMPARELANGUAGE(Type, 没有执行, THOST_FTDC_OER_NoExec)
	COMPARELANGUAGE(Type, 已经取消, THOST_FTDC_OER_Canceled)
	COMPARELANGUAGE(Type, 执行成功, THOST_FTDC_OER_OK)
	COMPARELANGUAGE(Type, 期权持仓不够, THOST_FTDC_OER_NoPosition)
	COMPARELANGUAGE(Type, 资金不够, THOST_FTDC_OER_NoDeposit)
	COMPARELANGUAGE(Type, 会员不存在, THOST_FTDC_OER_NoParticipant)
	COMPARELANGUAGE(Type, 客户不存在, THOST_FTDC_OER_NoClient)
	COMPARELANGUAGE(Type, 合约不存在, THOST_FTDC_OER_NoInstrument)
	COMPARELANGUAGE(Type, 没有执行权限, THOST_FTDC_OER_NoRight)
	COMPARELANGUAGE(Type, 不合理的数量, THOST_FTDC_OER_InvalidVolume)
	COMPARELANGUAGE(Type, 没有足够的历史成交, THOST_FTDC_OER_NoEnoughHistoryTrade)
	COMPARELANGUAGE(Type, CFieldValueTool_UNKOWN, THOST_FTDC_OER_Unknown)
	if(language) *language=0;
	return 0;
}

//取小数点的位数
int CFieldValueTool::GetNumDigits(double dbTick)
{
	char buf[512]={0};
	sprintf_s(buf,sizeof(buf)-1, "%f", dbTick);
	char* p=strchr(buf,'.');
	if(!p) return 0;
	...
	//有小数点的话，把后面的0去掉
	int tail=0;
	while(tail<len-1&&p[len-tail-1]=='0') tail++;

	return len-tail-1;
}
const string CFieldValueTool::Price2String(double dbData, double dbTick)
{
	if(util::isInvalidValue(dbData)||util::isInvalidValue(dbTick)) 
		return string("-");
	char strPriceText[512]={0},strFormatText[512]={0};
	sprintf_s(strFormatText,sizeof(strFormatText)-1,"%%0.%df", GetNumDigits(dbTick));
	sprintf_s(strPriceText,sizeof(strPriceText)-1,strFormatText, dbData);
	return strPriceText;
}
const string CFieldValueTool::Int2String(int nData)
{
	if(util::isInvalidValue(nData)) return "-";
	else
	{
		char buf[512]={0};
		sprintf_s(buf,sizeof(buf)-1,"%d",nData);
		return buf;
	}
}
