...
bool CCryptoAPI::Encode3DES(std::string srcstr,UCHAR** pwd,int& pwdlen)
{
	...
	if(GetHCRYPTPROV(hCryptProv)&&GetHCRYPTKEY(hCryptProv,hKey))
	{
		...
		BOOL brlt=CryptEncrypt(hKey,NULL,TRUE,0,NULL,&dwPwdLength,srcstr.size());
		if(brlt)
		{
			*pwd =new UCHAR[dwPwdLength];
			memcpy(*pwd,srcstr.c_str(),srcstr.size());
			brlt=CryptEncrypt(hKey,NULL,TRUE,0,*pwd,&dwSrclenth,dwPwdLength);
            if(brlt)
			{
				pwdlen=dwPwdLength;
				return true;
...
BOOL CCryptoAPI::GetHCRYPTKEY(const HCRYPTPROV& hCryptProv,HCRYPTKEY& hKey)
{
	...
	BOOL brlt=CryptImportKey(hCryptProv, DesKeyBlob, sizeof(DesKeyBlob), 0, CRYPT_EXPORTABLE, &hKey);
	return brlt;
}
