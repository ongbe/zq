...
SystemParamMgr::SystemParamMgr(void)
{
	...
	m_SystemParamInfo.bHideAccount.xmlset(false);
...
bool SystemParamMgr::InitCfg(TiXmlElement *root, bool bFromUserCfg )
{
	...
	pElement = pElementParamInfo->FirstChildElement("HideAccount");
	if ( pElement != NULL && pElement->GetText() != NULL )
		m_SystemParamInfo.bHideAccount.xmlset(atoi(pElement->GetText())>0?true:false, bFromUserCfg);
...
bool SystemParamMgr::SaveCfg(TiXmlElement *root)
{
	...
	if ( m_SystemParamInfo.bHideAccount.needWriteToUserCfg())
	{
		pElement = new TiXmlElement("HideAccount");
		pContext = new TiXmlText(GlobalFunc::ConvertToString(m_SystemParamInfo.bHideAccount.operator bool()));
		pElement->LinkEndChild(pContext);
		pSystemParamInfo->LinkEndChild(pElement);
	}
