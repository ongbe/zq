...
///////////////////////////////////////////////////////////////////////////////
    ...
    XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_KLINEIMG_CLICKED;
    XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_DELETEIMG_CLICKED;
    ...
    extern UINT WM_XLISTCTRL_KLINEIMG_CLICKED;
    extern UINT WM_XLISTCTRL_DELETEIMG_CLICKED;
#endif

///////////////////////////////////////////////////////////////////////////////
// CXListCtrl data
#define ID_CBM_USERDEF  5000
...
struct XLISTCTRLDATA
{
	...
	XLISTCTRLDATA()
	{
        ...
        bNumSpin        = FALSE;
        strNumSpinMessage   = _T("");
        bShowNumSpinMessage = TRUE;

        bKLine             = FALSE;
		dwItemData           = 0;
	}

    struct NUM_EDIT_SPIN 
    {
        bool bNumEdit;
        bool bNumSpin;
    ...
    static int             nComboBoxID;            //动态创建的空间ID，初始值ID_CBM_USERDEF
    CComboBox*      pComboBox;
    ...
    // for numspin
    NUM_EDIT_SPIN   sNumEditSpin;       //xedit + spin


    BOOL		bNumSpin;				// true = show NumSpin control
    CString		strNumSpinMessage;			// custom message for NumSpin control text                                            
    BOOL		bShowNumSpinMessage;		// TRUE = display message, or custom message
...
///////////////////////////////////////////////////////////////////////////////
// CXListCtrl class
enum eXListStyle{eTPriceDialog, eQuickOrderDialog};
enum eXListImage{eKLineImage, eUpLightImage, eUpDarkImage, eDownLightImage, eDownDarkImage, eDeleteImage};
...
protected:
	void	DrawCheckbox(int nItem, 
					  ...
					  XLISTCTRLDATA *pCLD);
#ifndef DO_NOT_INCLUDE_XCOMBOLIST
	void	DrawComboBox(int nItem,int nSubItem);
#endif
    void DrawComboBoxTest(int nItem, int nSubItem);
    ...
    CXEdit *		m_pSpinEdit;             
    int				m_nSpinEditItem;        //+++
    int				m_nSpinEditSubItem;     //+++


    CXSpinCtrl *  m_pSpinCtrl;
...
#endif
    afx_msg void OnComboCloseUp(UINT);
    afx_msg void OnComboSetFocus(UINT);
    afx_msg void OnComboKillFocus(UINT);
    afx_msg void OnComboDropDown(UINT);
...
public:
    ...
    // mode 0 = Color_Light     1 = Color_Thick
    int m_ColorMode;
    void    SetBKColorMode(int mode);

    //for spinCtrl
    double m_dTickPrice,m_dLowerPrice,m_dUpperPrice;
    void SetInstrumentData(const double& dLowerPrice,const double& dUpperPrice,const double dPriceTick);

    //default means the edit on the spin ctrl's left
    inline CXEdit * GetSpinBuddyEdit(){return m_pSpinEdit;};

    eXListStyle m_eStyle;
    void SetListStyle(eXListStyle eStyle);
    void HideCombBoxCtrl();
//protected:
//    virtual BOOL OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult); //屏蔽列表拖动，但同时也屏蔽了滚动条

private:
        bool            m_bCombBoxFocus;
        CComboBox*      m_pCurCombBox;  //当前的CComboBox
        int             m_nItem,m_nSubItem;

        CString         m_sText;        //当前的CComboBox中文本
        COLORREF GetTextRGB(CString strName);
public:
        void     free(int nItme = -1);    //释放动态申请的CComboBox
};
