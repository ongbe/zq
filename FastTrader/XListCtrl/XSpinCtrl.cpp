...
CXSpinCtrl::CXSpinCtrl(CWnd *pParent/*=NULL*/):
m_pParent(pParent),
...
CXSpinCtrl::~CXSpinCtrl()
{
}


BEGIN_MESSAGE_MAP(CXSpinCtrl, CSpinButtonCtrl)
	//{{AFX_MSG_MAP(CXSpinCtrl)
...
/////////////////////////////////////////////////////////////////////////////
// CXSpinCtrl message handlers

void CXSpinCtrl::SetValueForBuddy (double val)
{
    CXListCtrl* pXListCtrl = (CXListCtrl*)m_pParent;
    CWnd* pEdit = (pXListCtrl!=NULL)?pXListCtrl->GetSpinBuddyEdit():GetBuddy();
	if (pEdit)
	{
		CString str;
...
void CXSpinCtrl::SetPos(double val)
{
	SetValueForBuddy (val);
	SetIntPos (val);
}

double CXSpinCtrl::GetPos()
{
    CXListCtrl* pXListCtrl = (CXListCtrl*)m_pParent;
    CWnd* pEdit = (pXListCtrl!=NULL)?pXListCtrl->GetSpinBuddyEdit():GetBuddy();
	if (pEdit)
	{
		...
		double val = atof ((LPSTR)(LPCTSTR)str);
		return val;
	}
	else
	{
...
void CXSpinCtrl::SetRangeAndDelta(double lower, double upper, double delta)
{
	m_MinVal = lower;
	m_MaxVal = upper;
	m_Delta = delta;
...
void CXSpinCtrl::SetIntPos (double pos)
{
	//// avoid division by zero
...
void CXSpinCtrl::GetRangeAndDelta(double& lower, double& upper, double& delta)
{
	lower = m_MinVal;
	upper = m_MaxVal;
	delta = m_Delta;
...
BOOL CXSpinCtrl::OnDeltapos(NMHDR* pNMHDR, LRESULT* pResult)
{
...
int CXSpinCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CXSpinCtrl::SetDecimalPlaces(int number)
{
	m_NumDecPlaces = number;
}

int CXSpinCtrl::GetDecimalPlaces()
{
	return m_NumDecPlaces;
}

void CXSpinCtrl::SetFormatString (LPCTSTR lpszFormatString /*= NULL*/)
{
