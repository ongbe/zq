...
int CXHeaderCtrl::DrawImage(CDC* pDC, CRect rect, LPHDITEM lphdi, BOOL bRight)
{
	...
	if (lphdi->iImage != XHEADERCTRL_NO_IMAGE)
	{
		...
		if (pImageList && (rect.Width() > 0))
		{
			...
			//pImageList->DrawIndirect(pDC, lphdi->iImage, point, size, CPoint(0, 0));//列表头不显示图
...
LRESULT CXHeaderCtrl::OnLayout(WPARAM, LPARAM lParam)
{
    ...
    m_nHeight = (int)(pwpos->cy * 1.5/*m_Height*/); //改变高度,m_Height为倍数
    pwpos->cy = m_nHeight;//表头高度
    prc->top = m_nHeight; //表头下面的表内容高度
    return lResult; 
}
