...
XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_KLINEIMG_CLICKED = ::RegisterWindowMessage(_T("WM_XLISTCTRL_KLINEIMG_CLICKED"));
XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_DELETEIMG_CLICKED = ::RegisterWindowMessage(_T("WM_XLISTCTRL_DELETEIMG_CLICKED"));



int XLISTCTRLDATA::nComboBoxID = ID_CBM_USERDEF;
#define CHECK_MARK_GAP  3   //checkbox 中的 mark 与 "口"四周的间隔
/////////////////////////////////////////////////////////////////////////////
// CXListCtrl
...
BEGIN_MESSAGE_MAP(CXListCtrl, CListCtrl)
    ...
    ON_WM_SIZE()
    ON_CONTROL_RANGE(CBN_CLOSEUP ,   ID_CBM_USERDEF , ID_CBM_USERDEF + 1000 ,OnComboCloseUp)
    ON_CONTROL_RANGE(CBN_SETFOCUS ,  ID_CBM_USERDEF , ID_CBM_USERDEF + 1000 ,OnComboSetFocus)
    ON_CONTROL_RANGE(CBN_KILLFOCUS ,  ID_CBM_USERDEF , ID_CBM_USERDEF + 1000 ,OnComboKillFocus)
END_MESSAGE_MAP()
...
void CXListCtrl::free(int nItem /*= -1*/)
{
    int nSubItemCount = GetColumns();
    if(nItem==-1)
    {
        ...
        for (int nItem=0; nItem<nItemCount; nItem++)
        {
            ...
            if (pXLCD)
            {
                for (int nSubItem=0;nSubItem<nSubItemCount; nSubItem++)
                {
                    if (pXLCD[nSubItem].bCombo)
                    {
                        CComboBox* pComboBox = pXLCD[nSubItem].pComboBox;
                        if(pComboBox)
                        {
                            delete pComboBox;
                            pComboBox = NULL;
...
///////////////////////////////////////////////////////////////////////////////
// DrawNumSpin
void CXListCtrl::DrawNumSpin(int nItem,int nSubItem)
{
    XLISTCTRL_TRACE(_T("in CXListCtrl::DrawNumSpin\n"));
    ...
    {
        XLISTCTRL_TRACE(_T("XListCtrl: num spin ctrl require LVS_EX_FULLROWSELECT style\n"));
        ASSERT(FALSE);
    }
#endif
    ...
    ASSERT(m_pSpinEdit == NULL);

    m_pSpinEdit = new CXEdit(this, m_strInitialString);

    if (m_pSpinEdit)
    {
        CRect cliRect;
        cliRect.CopyRect(&rect);
        cliRect.DeflateRect(0,0,rect.Width()/3,0);
        BOOL bSuccess = m_pSpinEdit->Create(dwStyle, cliRect, this, 100);

        m_nSpinEditItem = nItem;
        m_nSpinEditSubItem = nSubItem;

        if (bSuccess)
        {
            m_pSpinEdit->ShowWindow(SW_SHOW);
            m_pSpinEdit->SetFocus();
        }
    }

    //spin
    dwStyle = UDS_WRAP|WS_CHILD|WS_VISIBLE|UDS_ALIGNRIGHT|UDS_ARROWKEYS;

    ASSERT(m_pSpinCtrl == NULL);

    m_pSpinCtrl = new CXSpinCtrl(this);

    if (m_pSpinCtrl)
    {
        CRect cliRect;
        cliRect.CopyRect(&rect);
        cliRect.DeflateRect(rect.Width()*2/3,0,rect.Width()*2/3,0);
        cliRect.DeflateRect(2,1,2,1);
        BOOL bSuccess = m_pSpinCtrl->Create(dwStyle, cliRect, this, 101);
        if (bSuccess)
        {
            if(nSubItem == 4)//手数
                m_pSpinCtrl->SetRangeAndDelta(1,10000,1);
            else if(nSubItem == 7)//价格
                m_pSpinCtrl->SetRangeAndDelta(m_dLowerPrice,m_dUpperPrice,m_dTickPrice);
							  ...
							  XLISTCTRLDATA *pXLCD)
{
	...
	chkboxrect = rect;
	chkboxrect.bottom -= 1;
	chkboxrect.left += rect.Width()/2 - rect.Height()/2;		// line up checkbox with header checkbox
	chkboxrect.right = chkboxrect.left + chkboxrect.Height();	// width = height

    chkboxrect.DeflateRect(m_nRowHeight/5,m_nRowHeight/5);
	//CString str;
	...
	pDC->FillSolidRect(&chkboxrect, RGB(255,255,255)/*m_crWindow*/);
	...
	if (pXLCD[nSubItem].nCheckedState == 1)
	{
		...
		// draw the checkmark
        int markLeft    = chkboxrect.left  + CHECK_MARK_GAP;
        int markRight   = chkboxrect.right - CHECK_MARK_GAP;
        int markTop     = chkboxrect.top   + CHECK_MARK_GAP;//mark right's top
        int markMid     = chkboxrect.Width()/2 + chkboxrect.left - 2;
        int markHight = (chkboxrect.Height() - 2*CHECK_MARK_GAP)/3;//mark pixel hight
        int nDrawCount = markRight-markMid;
        
		int i;
		for (i = 0; i < nDrawCount; i++)
		{
			pDC->MoveTo(markRight, markTop);
			pDC->LineTo(markRight, markTop+markHight);
			markRight--;
			markTop++;
		}
		for (i = 0; i < nDrawCount-3; i++)
		{
			pDC->MoveTo(markRight, markTop);
			pDC->LineTo(markRight, markTop+markHight);
			markRight--;
...
BOOL GetBitmapFromImageList(CDC* pDC,SIZE size,CImageList *imglist,int nIndex,CBitmap &bmp)
{
	...
	if (pImageList)
	{
		...
		if (nImage >= 0)
		{
			if (rect.Width() > 0)
			{
                ...
                //CBitmap bmp;
                //if(GetBitmapFromImageList(pDC,size,pImageList,nImage,bmp))
                //{
                //     CDC ImageDC;
                //     ImageDC.CreateCompatibleDC(pDC);
                //     CBitmap *pOldImageBMP = ImageDC.SelectObject(&bmp);
                //     TransparentBlt(pDC->m_hDC, 0, 0, size.cx, size.cy, ImageDC.m_hDC, 0, 0, size.cx, size.cy, RGB(255,255,255));
						  ...
						  XLISTCTRLDATA *pXLCD)
{
	...
	if (!str.IsEmpty())
	{
        ...
        if(nSubItem==9 && m_eStyle==eQuickOrderDialog) 
            nFmt = nFmt & ~HDF_CENTER | (str.CompareNoCase("买")==0)?HDF_LEFT:HDF_RIGHT;
	...
	else
	{
		...
		else
		{
			...
			if (pXLCD[nSubItem].nCheckedState >= 0)
			{
                ...
                bSend = true;
                WM_COMMANDID = WM_XLISTCTRL_CHECKBOX_CLICKED;
            ...
            {       
                ...
                m_nItem = nItem; 
                m_nSubItem = nSubItem; 
                m_sText = GetItemText(nItem,nSubItem); 
                m_pCurCombBox = pXLCD[nSubItem].pComboBox; 
                m_pCurCombBox->SetFocus();
            ...
            else if (pXLCD[nSubItem].bNumSpin)
            {
                CListCtrl::OnLButtonDown(nFlags, point);
                DrawNumSpin(nItem, nSubItem);
            }
            ...
            else if (pXLCD[nSubItem].nImage==eKLineImage)
            {
                CListCtrl::OnLButtonDown(nFlags, point);
                bSend = true;
                WM_COMMANDID = WM_XLISTCTRL_KLINEIMG_CLICKED;                
            }
            else if (pXLCD[nSubItem].nImage==eDeleteImage)//“删”图标
            {
                CListCtrl::OnLButtonDown(nFlags, point);
                free(nItem);
                bSend = true;
                WM_COMMANDID = WM_XLISTCTRL_DELETEIMG_CLICKED;                
            }

            CWnd *pWnd = GetParent();
            if (bSend && pWnd && ::IsWindow(pWnd->m_hWnd))
                pWnd->SendMessage(WM_COMMANDID, nItem, nSubItem);
		}
	}


    if(m_pCurCombBox)
        m_pCurCombBox->ShowWindow(SW_HIDE);
						   ...
						   COLORREF crBackground)
{
	...
	pXLCD[nItem].crText       = crText;
	pXLCD[nItem].crBackground = crBackground;
	pXLCD[nItem].nImage       = -1;
...
//////////////////////////////////////////////////////////////////////////
void CXListCtrl::SetBKColorMode(int mode)
{
    m_ColorMode = mode;
    m_crWindow = (m_ColorMode==0)?RGB(246,245,245):RGB(9,58,117);
}
///////////////////////////////////////////////////////////////////////////////
// SetBold
							 ...
							 BOOL bSort /*= FALSE*/)
{
	...
	if (bEnableCombo)
	{
		...
		SetItemText(nItem, nSubItem, strValue);
	}

	UpdateSubItem(nItem, nSubItem);
    DrawComboBoxTest(nItem,nSubItem);
	return TRUE;
}
...
// This function creates a Button in the specified subitem.  
// If bShowButtonText is TRUE, either the default text
// of "button" or the custom text (bShowButtonText) will be
// displayed.  If bShowButtonText is FALSE, only the Button
// will be displayed, with no text.
BOOL CXListCtrl::SetNumSpin(    int nItem, 
                                int nSubItem)

{
    ...
    pXLCD[nSubItem].sNumEditSpin.bNumEdit = TRUE;
    pXLCD[nSubItem].sNumEditSpin.bNumSpin = TRUE;

    pXLCD[nSubItem].bNumSpin              = TRUE;
    //pXLCD[nSubItem].bShowNumSpinMessage   = bShowEditText;
    //pXLCD[nSubItem].strNumSpinMessage     = lpszEditText;
...
{
	if (nIDEvent == 1 || nIDEvent == 4)			// timer set when combo is created
	{
		KillTimer(nIDEvent);
		UnlockWindowUpdate();
	}
...
void CXListCtrl::DrawComboBox(int nItem, int nSubItem)
{
	...
	if (m_pCombo)
	{
		...
		if (bSuccess)
		{
			...
			m_pCombo->SetDefaultVisibleItems(pXLCD[nSubItem].nInitialComboSel);
...
void CXListCtrl::DrawComboBoxTest(int nItem, int nSubItem)
{
    ASSERT(nItem >= 0);
    ASSERT(nItem < GetItemCount());
    if ((nItem < 0) || nItem >= GetItemCount())
    ...
    rect.bottom += pXLCD[nSubItem].psa->GetCount()*rect.Height();

    CRect rectClient;
    GetClientRect(&rectClient);
    
    CComboBox* pComboTest = new CComboBox();
    ASSERT(pComboTest);

    if (pComboTest)
    {
        DWORD dwStyle = WS_CHILD /*| WS_VISIBLE*/| CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_VSCROLL;
        BOOL bSuccess = pComboTest->Create(dwStyle,rect, this, ++(pXLCD[nSubItem].nComboBoxID));
        if (bSuccess)
        {
            pComboTest->SetFont(GetFont());
            if (pXLCD[nSubItem].psa)
            {
                // add strings to combo
                CString s = _T("");
                for (int i = 0; i < pXLCD[nSubItem].psa->GetSize(); i++)
                {
                    s = pXLCD[nSubItem].psa->GetAt(i);
                    if (!s.IsEmpty())
                        pComboTest->AddString(s);
                }

                pComboTest->SetCurSel(0);
            }

            pComboTest->ShowWindow(SW_HIDE);
        }

        pXLCD[nSubItem].pComboBox = pComboTest;
    }
}
///////////////////////////////////////////////////////////////////////////////
...
COLORREF CXListCtrl::GetTextRGB(CString strName)
{
...
void CXListCtrl::OnComboCloseUp(UINT)
{
    // Find the longest string in the combo box.
    if(m_pCurCombBox)
    {
        CString str;
        m_pCurCombBox->GetWindowText(str);
        COLORREF crText, crBackground;
        GetItemColors(m_nItem, m_nSubItem, crText, crBackground);
        SetItemText(m_nItem,m_nSubItem,str,GetTextRGB(str),crBackground);
        m_pCurCombBox->ShowWindow(SW_HIDE);
    }
}
void CXListCtrl::OnComboSetFocus(UINT)
{
    // Find the longest string in the combo box.
    if(m_pCurCombBox)
    {
        int nIndex = m_pCurCombBox->FindString(-1,m_sText);
        m_pCurCombBox->SetCurSel(nIndex);
        CRect rect;
        GetSubItemRect(m_nItem,m_nSubItem,LVIR_BOUNDS,rect);
        rect.bottom = rect.Height()*12;
        m_pCurCombBox->MoveWindow(rect);
        m_pCurCombBox->ShowWindow(SW_SHOW);
    }
}
void CXListCtrl::OnComboKillFocus(UINT)
{
    // Find the longest string in the combo box.
    if(m_pCurCombBox)
        m_pCurCombBox->ShowWindow(SW_HIDE);
...
LRESULT CXListCtrl::OnXEditEscape(WPARAM, LPARAM)
{
    ...
    if (m_pSpinEdit && ::IsWindow(m_pSpinEdit->m_hWnd))
    {
        m_pSpinEdit->DestroyWindow();
        delete m_pSpinEdit;
        m_pSpinEdit = NULL;

        //delete spin ctrl the same time
        m_pSpinCtrl->DestroyWindow();
        delete m_pSpinCtrl;
        m_pSpinCtrl = NULL;

        // restore original string
        SetItemText(m_nSpinEditItem, m_nSpinEditSubItem, m_strInitialString);
        UpdateSubItem(m_nSpinEditItem, m_nSpinEditSubItem);
    }  
...
LRESULT CXListCtrl::OnXEditKillFocus(WPARAM, LPARAM)
{
    ...
    if (m_pSpinEdit && ::IsWindow(m_pSpinEdit->m_hWnd))
    {
        ...
        // set new string in subitem
        SetItemText(m_nSpinEditItem, m_nSpinEditSubItem, str);
        ...
        if (str != m_strInitialString)
        {
            ...
            XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(m_nSpinEditItem);
            if (pXLCD)
            {
                pXLCD[m_nSpinEditSubItem].bModified = TRUE;
                m_bListModified = TRUE;
            }
        }
        ...
        if (pWnd && ::IsWindow(pWnd->m_hWnd))
            pWnd->SendMessage(WM_XLISTCTRL_EDIT_END, 
            m_nSpinEditItem, m_nSpinEditSubItem);
    }
	return 0;
}
...
{  
    ...
    if(m_HeaderCtrl.GetSafeHwnd())
        m_HeaderCtrl.MoveWindow(0,0,cx,m_HeaderCtrl.GetHeight());
}
void CXListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    //pDC->SetBkMode(TRANSPARENT);
    //pDC->FillRect(rcBack,&CBrush((m_ColorMode==0)?RGB(90,162,0):RGB(9,58,117)));
    ...
    if(lpDrawItemStruct->itemAction & ODA_DRAWENTIRE)
    {
        ...
        for (int i = 0; i< GetHeaderCtrl()->GetItemCount();i++)
        {
            ...
            XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(nItem);//(XLISTCTRLDATA *) pLVCD->nmcd.lItemlParam;
            ASSERT(pXLCD);
            ...
            {
                crText  = pXLCD[nSubItem].crText; 
                crBkgnd = (bHighlight && (m_eStyle==eQuickOrderDialog))?(m_ColorMode==0)?RGB(90,162,0):RGB(120,0,1):pXLCD[nSubItem].crBackground;
            ...
            //CDC* pDC = CDC::FromHandle(lpDrawItemStruct->hDC);//(pLVCD->nmcd.hdc);
            CRect rect;
            GetSubItemRect(nItem, nSubItem, LVIR_BOUNDS, rect);
...
void CXListCtrl::SetRowHeight(int nHeight)
{
...
//}
void CXListCtrl::SetInstrumentData(const double& dLowerPrice,const double& dUpperPrice,const double dPriceTick)
{
    m_dTickPrice = dPriceTick;
    m_dLowerPrice = dLowerPrice;
    m_dUpperPrice = dUpperPrice;
}
void CXListCtrl::SetListStyle(eXListStyle eStyle)
{
    m_eStyle = eStyle;
}
void CXListCtrl::HideCombBoxCtrl()
{
    if(m_pCurCombBox)
        m_pCurCombBox->ShowWindow(SW_HIDE);
}