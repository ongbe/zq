...
void CTCPHistoryService::CallbackReceiveData(char *pData,int DataLen,Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	...
	//		OFFER_INFO("CMDID_HEARTBEATRSP \n");
		...
		if ( VerifyRequest(pkgHead))
		{
			...
			{//处理行情
				std::vector<TLineBase> vecTimeLineData;
			    if ( pkgHead.userdata1 == CF_ERROR_SUCCESS )
				{
					int nCount = pkgHead.len/sizeof(TLineBase);
					for(int i =0; i<nCount; i++)
					{
						TLineBase timeLineData;
						ZeroMemory(&timeLineData, sizeof(TLineBase));
						m_pInstance->m_pStreamCache->GetFrom(&timeLineData, sizeof(TLineBase));
						vecTimeLineData.push_back(timeLineData);
					
						QuationHistory_info("\n收到分时行情[时间:%d; 价格:%.4f; 成交量:%d; 成交金额:%.2f;持仓量:%d]\n", 
							(DWORD)timeLineData.dwTime,
							timeLineData.fLastPrice,
							timeLineData.dwVolume,
							timeLineData.fTurnover,
							timeLineData.dwOpenInterest);
					}	
				}
			...
			{//处理行情
				 std::vector<KLineBase> vecKLineData;
				 if ( pkgHead.userdata1 == CF_ERROR_SUCCESS )
				 {
					 int nCount = pkgHead.len/sizeof(KLineBase);
					for(int i =0; i<nCount; i++)
					{
						KLineBase timeLineData;
						ZeroMemory(&timeLineData, sizeof(KLineBase));
						m_pInstance->m_pStreamCache->GetFrom(&timeLineData, sizeof(KLineBase));
						vecKLineData.push_back(timeLineData);
						QuationHistory_info("\n收到k线行情[时间:%d; 开:%.2f; 高:%.2f;低:%.2f;收:%.2f;量:%d;成交额:%.2f;持仓:%d]\n", 
							(DWORD)timeLineData.dwTime,
							timeLineData.fOpenPrice,
							timeLineData.fHighPrice,
							timeLineData.fLowPrice,
							timeLineData.fClosePrice,							
							timeLineData.dwVolume,
							timeLineData.fTurnover,
							timeLineData.dwOpenInterest);
					}				
...
void CTCPHistoryService::CallbackConnectSuccess(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	...
	{
		std::string strIP = CTools_Ansi::ConvertdwIP(SvrAddr.IP);
		QueryQuation_info("连接历史行情服务器成功:IP[%s], Port[%d]\n", strIP.c_str(),  SvrAddr.Port);
...
void CTCPHistoryService::CallbackConnectFail(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	...
	{
		std::string strIP = CTools_Ansi::ConvertdwIP(SvrAddr.IP);
		QueryQuation_info("连接历史行情服务器失败:IP[%s], Port[%d]\n", strIP.c_str(),  SvrAddr.Port);
