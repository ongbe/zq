...
void CJ_JieSuanDlg::OnHisCal( wxCommandEvent& evt )
{
    ...
    if(pTmp)
    {
        ...
        else
        {
            ...
            {
                if(DEFAULT_SVR())
                    DEFAULT_SVR()->UnSubscribeBusinessData(BID_RspQrySettlementInfo, GID_SettlementInfo);
...
CJ_JieSuanDlg::~ CJ_JieSuanDlg()
{
	...
	if(DEFAULT_SVR()) DEFAULT_SVR()->UnSubscribeBusinessData(BID_RspQrySettlementInfo,GID_SettlementInfo);
	pCJ_JieSuanDlg = NULL;
}
...
void  CJ_JieSuanDlg::OnQryDayInfo(wxCommandEvent& event)
{
	...
	m_AllContent.Empty();
	if(DEFAULT_SVR()) DEFAULT_SVR()->SubscribeBusinessData( BID_RspQrySettlementInfo, GID_SettlementInfo, MyCallBack, true);
	...
	if(DEFAULT_SVR()) DEFAULT_SVR()->ReqQrySettlementInfo( temp ) ;
}
...
{ 
	...
	if(DEFAULT_SVR()) DEFAULT_SVR()->ReqQryCFMMCTradingAccountKey( temp ) ;
