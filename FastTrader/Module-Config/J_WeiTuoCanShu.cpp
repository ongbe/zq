...
{
	std::map<std::string, std::vector<SimpleInstrumentInfo>> mapExchToInstrumentID;
	...
	for ( ; it != outData.end(); ++it)
	{
		...
		if(DEFAULT_SVR()) DEFAULT_SVR()->GetInstrumentInfo( *it, info );
		std::string strExchange = info.ExchangeID;
        strExchange = DEFAULT_SVR()?DEFAULT_SVR()->ExchangeID2String( strExchange.c_str()):"";
		SimpleInstrumentInfo instrument;
		instrument.id = info.InstrumentID;
		instrument.name = info.InstrumentName;
		std::map<std::string, std::vector<SimpleInstrumentInfo>>::iterator it_find 
	...
	std::map<std::string, std::vector<SimpleInstrumentInfo>>::iterator it_map 
		= mapExchToInstrumentID.begin();
	for ( ; it_map != mapExchToInstrumentID.end(); ++it_map )
	{
		InstrumentGroupInfo groupInfo;
