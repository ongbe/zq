               ...
               m_pSvr(NULL)
{
    ...
    if(!IS_MULTIACCOUNT_VERSION) m_pSvr=DEFAULT_SVR();
...
void CQuotPanel::InitGridUI()
{
    ...
    if(m_GroupButtonInToolBar)
        tbar=((FrameBase*)GETTOPWINDOW())->CreateToolbar_Begin();
	for(int i=0;i<groupCnt;++i)
	{
		...
		if(p->IsVisibility == 1)
		{
			OneGroupInfo tmp;
            if(m_GroupButtonInToolBar)
			    tmp.pWxButton=((FrameBase*)GETTOPWINDOW())->CreateToolbar_Item(tbar, wxID_ANY,p->GroupName, wxDefaultPosition, wxSize(72,-1), 0);			
            else
			...
			allInstrumentIDs.clear();
				...
				for(vector<string>::iterator it=allInstrumentIDs.begin(); it!=allInstrumentIDs.end(); ++it)
				{
					...
					memset(&rawQuot,0,sizeof(rawQuot));
					if(!m_pSvr||m_pSvr&&m_pSvr->GetQuotInfo(strInstrumentID, rawQuot)<0)
					{
    ...
    if(m_GroupButtonInToolBar)
        ((FrameBase*)GETTOPWINDOW())->CreateToolbar_End(tbar);
		...
		if(lastallgroupcount>0)
		{
			...
			{	
				if(gg==oldIndex&&gg!=GetIndexFromGroupId(LastAllGroups[gg].GroupId))
				{
                    ...
                    if(m_GroupButtonInToolBar)
                        ((FrameBase*)GETTOPWINDOW())->ProcessEvent(cmdevt);
                    else
                        ProcessEvent(cmdevt);
				}
			...
			for (std::set<std::string>::iterator it = InstrumentIDs.begin(); it != InstrumentIDs.end(); ++it) 
			{
                if(GlobalConfigManager::m_bUseComb)
                {
				    ...
				    if(bIsComboCode) 
					{
					    ...
					    m_pSvr->SetSubscribeStatus( strCmbSubCode1,GID_QUOT_TABLE,eUnsubscribeMarketData);
					    m_pSvr->SetSubscribeStatus( strCmbSubCode2,GID_QUOT_TABLE,eUnsubscribeMarketData);
				    }
                }
				m_pSvr->SetSubscribeStatus(*it,GID_QUOT_TABLE,eUnsubscribeMarketData);
			}
		}
		if(table)
		{
			for(int row=0;row<table->GetNumberRows();row++)
			{
                ...
                if(GlobalConfigManager::m_bUseComb)
                {
				    ...
				    if(bIsComboCode) 
					{
					    ...
					    m_pSvr->SetSubscribeStatus( strCmbSubCode1,GID_QUOT_TABLE);
					    m_pSvr->SetSubscribeStatus( strCmbSubCode2,GID_QUOT_TABLE);
				    }
                }
				m_pSvr->SetSubscribeStatus(strid,GID_QUOT_TABLE);
			}
		}
	}
	return 0;
...
void CQuotPanel::Release()
{
    ...
    if(m_GroupButtonInToolBar)
    {
        wxToolBar* tbar=((FrameBase*)GETTOPWINDOW())->CreateToolbar_Begin();
        ((FrameBase*)GETTOPWINDOW())->CreateToolbar_End(tbar);
    }
...
void CQuotPanel::OnRangeSelected( wxGridRangeSelectEvent& evt )
{
	...
	if(evt.Selecting()&&row1==row2)
	{
		...
		if(pTable)
		{
            ...
            DWORD EvtParamID;
            string EvtParamStr(pTable->GetInstrumentIDFromRow(row1).c_str());
            CFTEventParam::CreateEventParam(EvtParamID,NULL,&EvtParamStr,NULL,0,0);
            sendevt.SetInt((int)EvtParamID);
            sendevt.SetExtraLong((long)GID_QUOT_TABLE);
...
void CQuotPanel::OnCellLeftClick(wxGridEvent& evt)
{
	...
	if(pTable&&pTable->GetDirectionFromColumn(evt.GetCol()))
	{
        ...
        DWORD EvtParamID;
        string EvtParamStr(pTable->GetInstrumentIDFromRow(evt.GetRow()).c_str());
        CFTEventParam::CreateEventParam(EvtParamID,NULL,&EvtParamStr,NULL,0,0);
        sendchangeevt.SetInt((int)EvtParamID);
        sendchangeevt.SetExtraLong((long)GID_QUOT_TABLE);
        ...
        CFTEventParam::CreateEventParam(EvtParamID,NULL,&EvtParamStr,NULL,0,1);
        sendevt.SetInt((int)EvtParamID);
...
void CQuotPanel::OnCellLeftDoubleClick(wxGridEvent& evt)
{
	...
	if(parentevt.GetSkipped())
	{
		...
		if(pTable&&pTable->GetDirectionFromColumn(evt.GetCol()))
		{
			...
			wxCommandEvent sendevt(wxEVT_ORDERINSERT_MOUSEORDER);
			sendevt.SetExtraLong(pTable->GetDirectionFromColumn(evt.GetCol()));
            CFTEventParam::CreateEventParam(EvtParamID,NULL,&EvtParamStr,NULL,0,2);
            sendevt.SetInt((int)EvtParamID);
        ...
        else if(pTable&&pTable->IsInstrumentIDColumn(evt.GetCol())&&
            m_pSvr&&m_pSvr->SupportPlugins()&&
            (GlobalConfigManager::m_RunMode==2||GlobalConfigManager::m_RunMode==3))
        {
            //双击合约名称列，在期权版本，打开期权策略窗口
	        vector<wxString> vAppID;
	        g_Plusin.GetPlusinMenuInfo(MENU_POS_QUOTE,vAppID);

            //期权策略，对于APPID==OptionStrategy的情况进行特殊处理，只针对期权标的物期货合约弹出期权策略窗口
            for(int i=(int)vAppID.size()-1;i>=0;i--)
            {
                if(vAppID[i]==wxString("OptionStrategy"))
                {
                    string curInstrumentID=string(pTable->GetInstrumentIDFromRow(evt.GetRow()).c_str());
                    if(m_pSvr->IsUnderlyingInstrumentID(curInstrumentID))
                    {
				        g_Plusin.ShowPlusin(wxString("OptionStrategy"));
                    }
                    break;
                }
            }
...
void CQuotPanel::OnRightClick(wxGridEvent& evt)
{
	...
	if(m_pSvr&&m_pSvr->SupportPlugins())
	{
        ...
        for(int i=(int)vAppID.size()-1;i>=0;i--)
        {
            ...
            {
	            QuotGridTable* pTable=(QuotGridTable*)m_grid->GetTable();
                if(GlobalConfigManager::m_RunMode==1)
                {
                    vAppID.erase(vAppID.begin()+i);
                }
                else if(pTable)
                {
                    string curInstrumentID=string(pTable->GetInstrumentIDFromRow(evt.GetRow()).c_str());
...
{
    if(!m_bNeedLoadAllQuotData) return;
