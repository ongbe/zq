...
void wxAuiManager::ShowHint(const wxRect& rect,wxAuiManagerDock dir)
{
    if (m_hint_wnd)
    {
...
//        }
        ...
        if(0)
        {
            static wxSize mainFrameSize(0,0);
            wxSize curSize=event.GetSize();
            if(curSize.x<mainFrameSize.x||curSize.y<mainFrameSize.y)
            {
                int i, pane_count;
...
/*----------ouyang modified 2010.09--------------------------------------------------------------------*/
            if(dock.dock_direction == wxAUI_DOCK_CENTER)
            {
                used_height +=dock.min_size;
                used_width += dock.size;
            }
/*----------ouyang modified 2010.09--------------------------------------------------------------------*/
            if (dock.resizable)
