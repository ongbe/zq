...
// -- bitmaps --
 ...
 static unsigned char close_bits[]={
       0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f, 0xff, 0x3f, 0xfe, 
       0x1f, 0xfc,0x0f, 0xf8,0xff, 0xff,  0x0f,  0xf8, 0xff, 0xff, 0xff, 0xff,
       0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
#endif
...
wxAuiDefaultTabArt::wxAuiDefaultTabArt()
{
    ...
    m_base_colour =wxColor(253,238,208);//base_colour;
...
{
    // draw background
   wxColor top_color       =m_base_colour;//wxAuiStepColour(m_base_colour, 90);
   wxColor bottom_color   =*wxWHITE; //wxAuiStepColour(m_base_colour, 170);
   wxRect r;
...
void wxAuiTabContainer::Render(wxDC* raw_dc, wxWindow* wnd)
{
    ...
    for (i = 0; i < page_count; ++i)
    {
        ...
        if (page.window->GetId()==GID_ORDER_INSERT_PANEL||(m_flags & wxAUI_NB_CLOSE_ON_ALL_TABS) != 0 ||
...
void wxAuiNotebook::OnTabDragMotion(wxCommandEvent& evt)
{
    ...
    if (m_flags & wxAUI_NB_TAB_EXTERNAL_MOVE)
    {
        ...
        {
            wxWindow* src_tab_ctrl=tab_ctrl;
           while (tab_ctrl)
            {
            ...
            tab_ctrl=src_tab_ctrl;
            while (tab_ctrl)
            {
                if (tab_ctrl->IsKindOf(CLASSINFO(wxAuiNotebook)))
                    break;
                tab_ctrl = tab_ctrl->GetParent();
            }
            ...
            if (tab_ctrl)
            {
                wxAuiNotebook* nb = (wxAuiNotebook*)tab_ctrl;

                if (nb!=this||src_tabs->GetPageCount()>1)
                {
					wxAuiManagerDock dockdir=wxAUI_DOCK_NONE;
                    wxRect hint_rect = tab_ctrl->GetClientRect();
                    tab_ctrl->ClientToScreen(&hint_rect.x, &hint_rect.y);
					hint_rect.y+=nb->GetTabCtrlHeight();
					hint_rect.height-=nb->GetTabCtrlHeight();
					double x_delta=(screen_pt.x-hint_rect.x)*1.0/hint_rect.width;
					double y_delta=(screen_pt.y-hint_rect.y)*1.0/hint_rect.height;
					if(x_delta>0&&x_delta<0.25&&y_delta>0.25&&y_delta<0.75)//左
					{
						hint_rect.width/=2;
						dockdir=wxAUI_DOCK_LEFT;
					}
					else if(x_delta>0.25&&x_delta<0.75&&y_delta>0&&y_delta<0.25)//上
					{
						hint_rect.height/=2;
						dockdir=wxAUI_DOCK_TOP;
					}
					else if(x_delta>0.75&&x_delta<1&&y_delta>0.25&&y_delta<0.75)//右
					{
						hint_rect.width/=2;
						hint_rect.x+=hint_rect.width;
						dockdir=wxAUI_DOCK_RIGHT;
					}
					else if(x_delta>0.25&&x_delta<0.75&&y_delta>0.75&&y_delta<1)//下
					{
						hint_rect.height/=2;
						hint_rect.y+=hint_rect.height;
						dockdir=wxAUI_DOCK_BOTTOM;
					}
					if(dockdir!=wxAUI_DOCK_NONE)
						m_mgr.ShowHint(hint_rect,dockdir);
                    return;
                }
            }
        }
...
void wxAuiNotebook::OnTabEndDrag(wxCommandEvent& command_evt)
{
    ...
    if (m_flags & wxAUI_NB_TAB_EXTERNAL_MOVE)
    {
		...
		if (tab_ctrl)
		{
			...
			if (nb!=this||src_tabs->GetPageCount()>1)
			{
				if(m_mgr.GetDockDirection()==wxAUI_DOCK_NONE)
				{
                    m_mgr.HideHint();
                    return;
				}
				...
				src_page->Reparent(nb);
				//创建分割视图
				wxSplitterWindow* pParent=wxDynamicCast(nb->GetParent(),wxSplitterWindow);
				if(pParent)
				{
					wxWindow* new_pParent=pParent;
					wxSplitterWindow* Splitter=pParent;
					if(pParent->IsSplit())
					{
						Splitter=new MySplitterWindow(pParent);
						new_pParent=Splitter;	
						pParent->ReplaceWindow(nb,Splitter);
						nb->Reparent(Splitter);
					}
					wxAuiNotebook* pNewTabCtrl=new wxAuiNotebook(new_pParent,wxID_ANY,wxDefaultPosition,wxDefaultSize,TAB_STYLE);
					switch(m_mgr.GetDockDirection())
					{
					case wxAUI_DOCK_TOP:
						Splitter->SplitHorizontally(pNewTabCtrl,nb);
						break;
					case wxAUI_DOCK_RIGHT:
						Splitter->SplitVertically(nb,pNewTabCtrl);
						break;
					case wxAUI_DOCK_BOTTOM:
						Splitter->SplitHorizontally(nb,pNewTabCtrl);
						break;
					case wxAUI_DOCK_LEFT:
						Splitter->SplitVertically(pNewTabCtrl,nb);
						break;
					default:
						wxLogDebug("m_mgr.GetDockDirection()=%d",m_mgr.GetDockDirection());
						break;
					}
					pNewTabCtrl->AddPage(page_info.window, page_info.caption);
    ...
    //            // found out the insert idx
				//wxAuiTabCtrl* dest_tabs =pNewTabCtrl->GetActiveTabCtrl();
    //            int insert_idx =dest_tabs->GetPageCount();
    //            dest_tabs->InsertPage(page_info.window, page_info, insert_idx);
    //            pNewTabCtrl->m_tabs.AddPage(page_info.window, page_info);

    //            pNewTabCtrl->DoSizing();
    //            dest_tabs->DoShowHide();
    //            dest_tabs->Refresh();

    //            // set the selection in the destination tab control
    //            pNewTabCtrl->SetSelection(nb->m_tabs.GetIdxFromWindow(page_info.window));
    ...
    if ((m_flags & wxAUI_NB_TAB_SPLIT) && m_tabs.GetPageCount() >= 2)
    {
        ...
        if (tab_frame)
        {
            ...
            if (dest_tabs == src_tabs)
                return;
            wxAuiPaneInfo& paneinfo = m_mgr.GetPane(tab_frame);
            if(paneinfo.dock_direction ==wxAUI_DOCK_TOP||paneinfo.dock_direction == wxAUI_DOCK_CENTER)
                    return;
