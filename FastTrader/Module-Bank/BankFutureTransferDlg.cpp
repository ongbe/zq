...
static int rspOnTransferCallBackFunc(const GUIModuleID GID,const AbstractBusinessData& data)
{
    ...
    CFileOpr::getObj().writelocallog("银期转账日志","rspOnTransferCallBackFunc(2): BID=%d",data.BID);
    switch (data.BID)
    {
...
EVT_COMBOBOX(ID_WXCOMBOBOX1,CBankFutureTransferDlg::OnSelected)
END_EVENT_TABLE()
...
void CBankFutureTransferDlg::CreateGUIControls()
{
	...
	if(DEFAULT_SVR())
		DEFAULT_SVR()->GetContractBank(ContractBankInfo);
	wxButton* pBtn=NULL;
	...
	//左上
	{
		...
		if(m_cmbBankName->GetCount()==1)
        {
            ...
            OnSelected(wxCommandEvent());
        }

		//vector<string> setcur = DEFAULT_SVR()->GetCurrencys("");
		//vector<string>::iterator itr = setcur.begin();
		//while (itr!=setcur.end())
		//{
		//	m_cmbCurrency->Append((*itr).c_str());
		//	itr++;
		//}
	...
	//右边部分
	{
		...
		m_DetailList->InsertColumn(iIndex++,LOADSTRING(BankTransfer_CurrencyID),wxLISTEXT_FORMAT_LEFT,100);
						  ...
						  double TradeAmount,const char* CurrencyID,const char* TradeTime,const char* ErrorMsg,const char* BankID)
{
	if(m_TransferDetailIDSet.insert(FutureSerial).second)
	{
		...
		m_DetailList->SetItem(RowId,5,wxString(CurrencyID));
		m_DetailList->SetItem(RowId,6,wxString(ErrorMsg));
	}
}
...
wxString CBankFutureTransferDlg::GetUIValue(const wxString flag)
{
	...
	if(flag=="BankName")
		...
		str=m_cmbCurrency->GetValue();
	return str;
}

void CBankFutureTransferDlg::OnSelected(wxCommandEvent& event)
{
    int sel=m_cmbBankName->GetCurrentSelection();
    m_cmbCurrency->Clear();
    string strBankName(m_cmbBankName->GetValue().c_str());
