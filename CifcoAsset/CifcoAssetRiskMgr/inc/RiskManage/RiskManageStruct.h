...
struct SProfitCurve
{
	int nIDType;//0, 理财产品下面的交易账号; 1 资管组织下面的交易账号; 2某具体账户
	int nID;	//资管组织ID 或 理财产品ID
	DateType DateBegin;//开始时间格式：20130606
	DateType DateEnd;  //结束时间格式：20130608
};

struct SProfitCurveReturn
{
	int nIDType;//0, 理财产品下面的交易账号; 1 资管组织下面的交易账号; 2某具体账户
	int nID;	//资管组织ID 或 理财产品ID
	DateType Date;//开始时间格式：20130606	

	double dbSumProfit;		//累计收益金额（历史总权益 – 账户初始资金）
	double dbOffsetBalance;//今日权益变化（结算后总权益 – 今日初始权益）
};
...
struct SProfit
{
	std::string		strTradeAccount; //账号
	double			dbProfit;		 //账户初始金额
	double			dbBalance;       //总权益
	double			dbOffsetProfit;  //当日权益变化
