...
#pragma endregion
	...
	public: System::Void UpdateData(RiskAllAccountField * nTta){
				if(nTta == NULL)
					return;
				//有初始权益
				//CSQY = nTta->pre.PreBalance;
				//盈亏比
				Double lnFuYinKui = TotalStatiscData::SafeDivMulti100((nTta->cur.Balance - CSQY),CSQY);
				this->label2->Text = lnFuYinKui.ToString("f2") + L"%";          
				this->simpleFlagControl1->Value = (Single)lnFuYinKui;
				//浮动盈亏
				Double ldbYinKui =(nTta->cur.Balance - CSQY)/10000.00;
				...
				//总权益
	            this->label5->Text = (Int32(TotalStatiscData::SafeDivMulti100(nTta->cur.Balance,CSQY))).ToString() + L"%";     
				 //风险度
				this->label6->Text = (Int32(TotalStatiscData::SafeDivMulti100(nTta->cur.CurrMargin,nTta->cur.Balance))).ToString() + L"%";  
				return;
