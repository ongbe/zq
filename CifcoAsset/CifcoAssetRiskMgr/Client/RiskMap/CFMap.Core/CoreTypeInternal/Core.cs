   ...
   internal class Core : IDisposable
   {
      ...
      public BackgroundWorker OnMapOpen()
      {
         if(!IsStarted)
         {
            ...
            if(x == 1)
            {
               CFMaps.Instance.noMapInstances = false;
            }
            ...
            try
            {
               while(tileLoadQueue.Count == 0)
               {
                  ...
                  if(++loadWaitCount >= GThreadPoolSize)
                  {
                     ...
                     if(IsStarted)
                     {
                        CFMaps.Instance.MemoryCache.RemoveOverload();
                     ...
                     Debug.WriteLine(ctid + " - OnTileLoadComplete: " + lastTileLoadTimeMs + "ms, MemoryCacheSize: " + CFMaps.Instance.MemoryCache.Size + "MB");
                        ...
                        do
                        {
                           ...
                           if(!provider.MaxZoom.HasValue || task.Value.Zoom <= provider.MaxZoom)
                           {
                              if(skipOverZoom == 0 || task.Value.Zoom <= skipOverZoom)
                              {
                                 ...
                                 if(tl.InvertedAxisY)
                                 {
                                    img = CFMaps.Instance.GetImageFrom(tl, new GPoint(task.Value.Pos.X, maxOfTiles.Height - task.Value.Pos.Y), task.Value.Zoom, out ex);
                                 }
                                 else // ok
                                 {
                                    img = CFMaps.Instance.GetImageFrom(tl, task.Value.Pos, task.Value.Zoom, out ex);
                                 }
                              }
                           }
                           ...
                           if(img == null && okZoom > 0 && fillEmptyTiles && Provider.Projection is MercatorProjection)
                           {
                              ...
                              while(img == null && zoomOffset < task.Value.Zoom)
                              {
                                 ...
                                 img = CFMaps.Instance.GetImageFrom(tl, parentTile, task.Value.Zoom - zoomOffset++, out ex);
                              }
      ...
      void UpdateBounds()
      {
         ...
         try
         {
            ...
            if(CFMaps.Instance.ShuffleTilesOnLoad)
            {
