...
namespace CFMap.CacheProviders
{
   ...
   internal class SQLitePureImageCache : PureImageCache
   {
      ...
      static System.Reflection.Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
      {
         ...
         {
            string rootDir = System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData) + Path.DirectorySeparatorChar + "CFMap" + Path.DirectorySeparatorChar;
      ...
      public string CacheLocation
      {
         ...
         set
         {
            ...
            dir = gtileCache;
           // dir = gtileCache + IMapProvider.LanguageStr + Path.DirectorySeparatorChar;
      ...
      bool PureImageCache.PutImageToCache(byte[] tile, int nMapId, GPoint pos, int zoom)
      {
         bool ret = true;
         if(Created)
         {
                     ...
                     using(DbTransaction tr = cn.BeginTransaction())
                     {
                        try
                        {
                           using(DbCommand cmd = cn.CreateCommand())
                           {
                               ...
                               cmd.Parameters.Add(new SQLiteParameter("@p4", nMapId));
      ...
      PureImage PureImageCache.GetImageFromCache(int nMapId, GPoint pos, int zoom)
      {
         PureImage ret = null;
         try
         {
            using(SQLiteConnection cn = new SQLiteConnection())
            {
               ...
               cn.Open();
               {
                  ...
                  using(DbCommand com = cn.CreateCommand())
                  {
                     com.CommandText = string.Format(finnalSqlSelect, pos.X, pos.Y, zoom,nMapId);
                     ...
                     using(DbDataReader rd = com.ExecuteReader(System.Data.CommandBehavior.SequentialAccess))
                     {
                        if(rd.Read())
                        {
                           ...
                           rd.GetBytes(0, 0, tile, 0, tile.Length);
                           {
                              ...
                              {
                                  ret = IMapProvider.TileImageProxy.FromArray(tile);
                              }
                           }
                           tile = null;
                        }
