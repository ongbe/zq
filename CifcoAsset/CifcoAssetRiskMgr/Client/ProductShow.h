...
#pragma endregion
	private: System::Void OnLoad(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 std::set<int> setOrgAccount,setFinicalAccount;			
				 m_pdataInfo->GetTrustTradeAccountByOrgID(mnCurrentOrgID, true, setOrgAccount);
				 m_pdataInfo->GetTrustTradeAccountByFinancialID(mnCurrentProduceID, setFinicalAccount);
				 ...
				 { 
					 ...
					 m_pdataInfo->GetTrustTradeAccount(*lIte,lTraderInfo);
					 lItem->AccountName = Tools::string2String(lTraderInfo.szName);
...
private: System::Void SubscribeStatiacsData()
		 {
			 ...
			 re.nFinancialProductID=mnCurrentProduceID;
...
private: System::Void SubscribeAccountStatiacsData()
		 {
			 ...
			 {				
				 if( !CTcpLayer::SendData(Cmd_RM_SubscribeFund_Req, &mRiskMapData->mStaticsAccountVector[0],sizeof(int)*mRiskMapData->mStaticsAccountVector.size(), mnSeqID))
				 {				
					 // Windows::Forms::MessageBox::Show("订阅汇总数据失败", "提示", Windows::Forms::MessageBoxIcon::Error);
					 MessageBox::Show("订阅汇总数据失败", "提示",MessageBoxButtons::OK);
				 }
...
private: System::Void UnsubscribeAccountStatiacsData()
		 {
			 ...
			 if(mRiskMapData->mStaticsAccountVector.size() > 0)
			 {				
				 if( !CTcpLayer::SendData(Cmd_RM_SubscribeFund_Req, &mRiskMapData->mStaticsAccountVector[0],sizeof(int)*mRiskMapData->mStaticsAccountVector.size(), 0))
				 {					
					 MessageBox::Show("退订汇总数据失败", "提示",MessageBoxButtons::OK);
				 }
			 }
				  ...
				  case Cmd_RM_SubscribeFund_Rsp:				
					  {
						  if(head.userdata1 == OPERATOR_SUCCESS ) 
						  {
							  ...
							  for(int i=0; i<nItemCount; i++)
							  {
								  ...
								  m_pdataInfo->GetTrustTradeAccount(pFundInfo->cur.InvestorID, acct); 
