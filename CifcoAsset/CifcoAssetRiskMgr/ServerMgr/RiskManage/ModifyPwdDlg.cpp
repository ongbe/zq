...
void CModifyPwdDlg::OnBnClickedOk()
{
	...
	modifyPwd.nRiskMgmtUserID = ConfigManager::GetInstance()->GetAccountID();
	...
	if ( !CTCPDataService::GetInstance()->SendData(Cmd_RM_ModifyPwd_Req, &modifyPwd, sizeof(modifyPwd)))
	{
...
LRESULT CModifyPwdDlg::OnRecvMessage( WPARAM wParam, LPARAM lParam )
{
	...
	if ( pRecvData->head.cmdid == Cmd_RM_ModifyPwd_Rsp )
	{
