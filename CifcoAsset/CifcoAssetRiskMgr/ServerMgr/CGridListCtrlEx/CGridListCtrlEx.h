...
struct tgCellPos
{
	...
	tgCellPos( int row, int col)
	{
		nRow = row;
		nCol = col;
	}

	bool operator < ( const tgCellPos& rhs) const
	{
		if ( this->nRow != rhs.nRow )
		{
			return this->nRow < rhs.nRow;
		}
		else
		{
			return this->nCol < rhs.nCol;
		}
	}
};
...
//------------------------------------------------------------------------
class AFX_EXT_CLASS CGridListCtrlEx : public CListCtrl
{
public:
	CGridListCtrlEx();
	~CGridListCtrlEx();
	...
	// Cell Color
	void SetCellTextColor(int nRow, int nCol, COLORREF clr);
	COLORREF GetCellTextColor(int nRow, int nCol);
	void ResetCellTextColor();
...
protected:
	...
	std::map<tgCellPos, COLORREF> m_mapCellClr;
