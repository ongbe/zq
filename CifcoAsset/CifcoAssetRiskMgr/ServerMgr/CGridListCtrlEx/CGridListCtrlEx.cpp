...
BOOL CGridListCtrlEx::OnToolNeedText(UINT id, NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	{
		int n = MultiByteToWideChar(CP_ACP, 0, tooltip, -1, pTTTW->szText,
...
{
	textColor = GetCellTextColor(nRow, nCol);
	return true;
}
...
void CGridListCtrlEx::SetCellTextColor(int nRow, int nCol, COLORREF clr)
{
	tgCellPos cellPos(nRow, nCol);
	m_mapCellClr[cellPos] = clr;
	CListCtrl::Update(nRow);
}

COLORREF CGridListCtrlEx::GetCellTextColor(int nRow, int nCol)
{
	tgCellPos cellPos(nRow, nCol);
	std::map<tgCellPos, COLORREF>::iterator it = m_mapCellClr.find(cellPos);
	if ( it != m_mapCellClr.end() )
	{
		return it->second;
	}
	else
...
void CGridListCtrlEx::ResetCellTextColor()
{
	m_mapCellClr.clear();
}
