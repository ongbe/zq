...
CReal_SvrTradeData::CReal_SvrTradeData()
{
	...
	mbNeedCache = myxml.GetNodeAttrValuebool2(false,"NeedCache","root","Redis","TraderData");
	mCacheHostIP = myxml.GetNodeAttrValue2("127.0.0.1","IP","root","Redis","TraderData","CacheHost");
	mCacheHostPort = myxml.GetNodeAttrValueInt2(6379,"Port","root","Redis","TraderData","CacheHost");
...
{
	return mpTradingDayManager.IsTradingDay(nsDay);
}
...
{
	return mpTradingDayManager.IsSetttemnted(nsDay);
}
...
{
	return mpTradingDayManager.GetLastTradingData(nsDay,nsLastDay);
}
//设置当前交易日
void CReal_SvrTradeData::SetCurrentTradingDay(const std::string & nsCurrentTradingDay,bool nbSetByMySelf) 
{
	...
	std::string lsTempTradingDay = nsCurrentTradingDay.substr(0,4) + nsCurrentTradingDay.substr(4,2) + nsCurrentTradingDay.substr(6,2);
...
//报盘可以工作了
void CReal_SvrTradeData::TradeDataInit(bool nbForce ) 
{
	if(mpDBOpr == NULL)
		return;
	mbForceInit = nbForce;
	...
	PUSH_LOG_ERROR(Logger,false,true,"开始初始化服务器...");

	//mpDBOpr->CreateMyTableAndPartitions("");

	//设置模块的状态为初始化状态
	SetTraderDataStatus(TradeDataStatusInitialing);
	//meUserTradedDataWithDBReady = DataReday_NoReady;
	//meUserTradedDataWithCTPReady = DataReday_NoReady;
	...
	//清理错误信息
	CInterface_SvrNotifyAndAsk::getObj().ClearAllError();
	...
	Stru_NotifyEvent lDataEventLogin(EventTypeNeedAllLoginTrade,lTime < lNigthTime ? 0 : 1);
	CInterface_SvrNotifyAndAsk::getObj().NotifyEvent(lDataEventLogin);
	PUSH_LOG_ERROR(Logger,false,true,"发送登陆所有委托交易帐号指令...");


	//通知模块线程不停地check 用户的CTP状态，检测是否能否登陆上
	PostThreadMessage(g_idThread,TMESSAGE_TRADEINIT_CHECK_USER_CTP_STATUS,0,0);



	//发送开始初始化命令，通知其他模块开始初始化
	Stru_NotifyEvent lDataEventInit(EventTypeStartInit,0);
...
///设置指定合约信息
void CReal_SvrTradeData::SetInstrumentInfo(PlatformStru_InstrumentInfo& nInstrumentInfo,bool bLast)
{
//	CSafeLock lLock(m_mutex);
	if(	mpPlatformDataInstrument == NULL)
	{
	...
	mpPlatformDataInstrument->SetInstrumentInfo(nInstrumentInfo.InstrumentID,nInstrumentInfo,false,bLast);
}
...
///获取合约品种代码
int CReal_SvrTradeData::GetProductID_InstrumentIDsByExchangeID(std::map<std::string, std::set<std::string> >& outData,
									 std::string ExchangeID)
{
//	CSafeLock lLock(m_mutex);
	if(	mpPlatformDataInstrument == NULL)
	...
	mpPlatformDataInstrument->GetProductID_InstrumentIDsByExchangeID(outData,ExchangeID);
	return outData.size();
}
...
{	
	if(!CInterface_SvrLogin::getObj().IsSlaver())
	{
		...
		for(unsigned int i = 0; i < lVecSocket.size() && bSettlementManul == false; i++)
		{
			//TRADE_LOG("Master push a Quotto SLaver %d",lVecSocket[i]);
    ...
    //TRADE_LOG(Logger,false,true,"从服务接收从主服务发送的深度行情数据inst=%s",inData.InstrumentID);

	if(mbNeedCache && bSettlementManul == false)
	{
		//缓存修改的持仓，和资金
		mCache->PushQuot(inData);
	}


	//如果有结算价的情况
	//如果是结算模块调用bsettment=true
	//则不进行计算，不保存到内存中，但需设置到行情对象中
	//如果是报盘调用，则需要分情况，如果是交易状态则需要保存计算，如果是结算状态，则只需要保存到内存中
	bool lbCalu = false;
	if(inData.SettlementPrice != util::GetDoubleInvalidValue())
	{
		if(bSettlementManul != false)
		{
			TRADE_LOG("Manual set SettlementPrice price quot come ，intstrument id is %s，price is %6.3f ,presettprice is %6.3f",inData.InstrumentID,inData.SettlementPrice,inData.PreSettlementPrice);
			//结算模块调用
           lbCalu = false;
		   mpPlatformDataQuots->SetQuotInfo(inData);
		}
		else
		{
			TRADE_LOG("CTP SettlementPrice price quot come ，intstrument id is %s，price is %6.3f ,presettprice is %6.3f",inData.InstrumentID,inData.SettlementPrice,inData.PreSettlementPrice);
		
			mMutextSettlementQuot.lock();
			//把最新价复制为今天的计算价
			inData.LastPrice = inData.SettlementPrice;
			//插入到结算行情map中
			mmapSettlementQuot[inData.InstrumentID] = inData;
			mMutextSettlementQuot.unlock();

			//存结算价
			bool lbUsingQutoDay = true;
			std::string lsTradingDay = inData.TradingDay;
			if(lsTradingDay.empty())
			{
				lbUsingQutoDay = false;
			...
			std::string lsTempTradingDay;
			if(lbUsingQutoDay)
				lsTempTradingDay = lsTradingDay.substr(0,4) + "-" + lsTradingDay.substr(4,2) + "-" + lsTradingDay.substr(6,2);
			else
				lsTempTradingDay = lsTradingDay;
			TRADE_LOG("add price to db with %s",lsTempTradingDay.c_str());

			//if(!lsTempTradingDay.empty())
			mpDBOpr->AddQutoInfo("TradeData_Quto",lsTempTradingDay,inData);
		...
		/*	if(GetTraderDataStatus() == TradeDataStatusSettlemtIniting ||
				GetTraderDataStatus() == TradeDataStatusSettlemting ||
				GetTraderDataStatus() == TradeDataStatusSettlemtEnd)
			{
				lbCalu = false;
			}*/
		}
	...
	{
		//发送行情事件	
		Stru_NotifyEvent ls(EventTypeQuto,0);
		ls.AddBufer((void*)&inData,sizeof(inData));	
		...
		while(lIterUser != mMapUser2TradingData.end())
		{
			if(lIterUser->second->CalulateByNewQuto(inData,bSettlementManul))
			{
				...
				ls.AddBufer((void*)lIterUser->first.c_str(),lIterUser->first.length() );	
				CInterface_SvrNotifyAndAsk::getObj().NotifyEvent(ls);

				Stru_NotifyEvent lsFund(EventTypeFund,0);
				lsFund.AddBufer((void*)lIterUser->first.c_str(),lIterUser->first.length() );	
				...
				if(mbNeedCache)
				{
					...
					for(int i = 0 ; i < lvscPos.size(); i++)
						mCache->CachePosition(lvscPos[i]);
		...
		{
			if(lIterAccount->second->CalulateByNewQuto(inData,bSettlementManul))
			{
				////产生持仓变化和资金变化事件		
				//Stru_NotifyEvent ls(EventTypePosition,0);
				//ls.AddBufer((void*)lIterUser->first.c_str(),lIterUser->first.length() );	
...
//报盘调用，当某一个主帐号资金查询完成
void CReal_SvrTradeData::QryFundReach(const PlatformStru_TradingAccountInfo& inData)
{
	...
	if(mMapAccountData.end() != lIter)
	{
		if(GetTraderDataStatus() == TradeDataStatusTrading && CInterface_SvrLogin::getObj().IsSlaver())
		{
			...
			//设置费率对象到对应的用户对象
			SetRatesObjectToAccountRelationUserObject(inData.BrokerID,inData.AccountID);

			//装载日初的数据
			DispathInitialDataToUserWithAccount(inData.BrokerID,inData.AccountID,mMapUser2TradingData);
...
void CReal_SvrTradeData::QryPositionReach( PlatformStru_Position& PositionInfo, int ErrID, bool bIsLast)
{
	...
	if(mMapAccountData.end() != lIter)
	{
		...
		if(CF_ERROR_SUCCESS == lpTemp->QryPositionReach(PositionInfo,ErrID,bIsLast) && ErrID == 0)
		{
			...
			if(mbNeedCache)
				mCache->CachePosition(PositionInfo);
		}	
	}
	return;
}
...
void CReal_SvrTradeData::QryPositionDetailReach( PlatformStru_PositionDetail& PositionDetailInfo, int ErrID, bool bIsLast)
{
	...
	if(mMapAccountData.end() != lIter)
	{
		...
		if(CF_ERROR_SUCCESS == lpTemp->QryPositionDetailReach(PositionDetailInfo,ErrID,bIsLast) && ErrID == 0)
		{
			...
			//mBufferDataStoreOrder.Push(OrderInfo,BufferStoreCallback);
			if(mbNeedCache)
				mCache->CachePositionDetail(PositionDetailInfo);
		}	
	}
...
CF_ERROR CReal_SvrTradeData::PushOrder(PlatformStru_OrderInfo & nOrderInfo,bool nbNeedRiseEvent)
{
	...
	if(mMapUser2TradingData.end() != lIter)
	{
		...
		if(lRet != CF_ERROR_NOINITED && nbNeedRiseEvent)
		{
			...
			if(mbNeedCache)
			{
				mCache->CacheOrder(nOrderInfo);
				mCache->PushOrder(nOrderInfo);
			}
...
CF_ERROR CReal_SvrTradeData::PushTrade(PlatformStru_TradeInfo& nTradeInfo)
{
	...
	TRADE_LOG("PushTrade BrokerID:%s UserID %s IID %s %d %c",nTradeInfo.BrokerID,nTradeInfo.InvestorID,nTradeInfo.InstrumentID,nTradeInfo.Direction);
	...
	std::vector<PlatformStru_InstrumentInfo> & outData) 
{
	...
	return mpDBOpr->LoadTodayInstrument(lsTradingDay,outData);
																  ...
																  const std::string & nsDay) 
{
	...
	if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayInstrument(nsDay,lVecInstrument))
	{
		...
		for(int i=0;i<(int)lVecInstrument.size();i++)
		{
			SetInstrumentInfo(lVecInstrument[i],i==(int)lVecInstrument.size()-1?true:false);
		}
...
{
	return mpDBOpr->LoadQryFundInOut(nQryFund,nvecFund);
}
									  ...
									  _MapUserTradeData & nmapUserTradeData)
{
	...
	//if(lpRate != NULL)
	{
		...
		for(;lIter != lVecUser.end();lIter++)
		{
			DispathInitialDataToUser(lIter->szAccount,lsTempTradingDay,nmapUserTradeData);	
		}	
...
//装载某一天所有用户的的日初数据
CF_ERROR CReal_SvrTradeData::LoadInitData(const std::string & nsTime)
{
	if(GetTraderDataStatus() != TradeDataStatusInitialing &&
		GetTraderDataStatus() != TradeDataStatusSettlemtIniting)
	{
	...
	for(;lIter != lVecUser.end();lIter++)
	{
		if(lIter->nUserType == USER_TYPE_TRADER)
		{
			...
			strcpy(lFundInfo.AccountID,lIter->szAccount);
			mUserIniFundInfo.insert(std::make_pair(lIter->szAccount,lFundInfo));
			mUserInitPosition.insert(std::make_pair(lIter->szAccount,lVecPosition));
			mUserInitPositionDeatil.insert(std::make_pair(lIter->szAccount,lVecPosionDeatil));
			mUserInitInitFund[lIter->szAccount] = ldbVal;
		}
	}
	if(lbLoad )
		return CF_ERROR_SUCCESS;
...
//初始化用户对象
CF_ERROR CReal_SvrTradeData::InitUserMapWithEmpty(_MapUserTradeData & nmapUserData)
{

	{
		...
		for(;lIter != lVecAccount.end();lIter++)
		{
			...
			CInterface_SvrBrokerInfo::getObj().GetBrokerInfoByID(lIter->nBrokerID,lBroker);
			BrokerAccountKey lKey(lBroker.szBrokerCode,lIter->szTradeAccount);
			//mMapAccountRateReady[lKey] = CommissionMargin_NoReady;
	...
	}
	{
		...
		{
			if(lIter->nUserType == USER_TYPE_TRADER && 
				nmapUserData.find(lIter->szAccount) == nmapUserData.end())
				CreateEmptyAccountOpAndAddMap(lIter->szAccount,nmapUserData);
		}
	}
	return CF_ERROR_SUCCESS;
...
//从服务器初始化
bool CReal_SvrTradeData::SlaverInit()  //从服务器的初始化入口函数
{
	TRADE_LOG("TradeDataInit with in Slaver Mode");
	...
	////写入所有的用户在线状态到
	//vector<sUserOnlineInfo> lvecInfo;
	//CInterface_SvrLogin::getObj().GetAllOnlineUserStatus(lvecInfo);
	//for(int i = 0 ; i < lvecInfo.size();i++)
	//{
	//	UserInfo lInfo;
	//	if(lvecInfo[i].mUserID != SLAVE_USER_ID)
	//	{
	//		CInterface_SvrUserOrg::getObj().GetUserByID(lvecInfo[i].mUserID,lInfo);
	//		//PushTraderUseroffLine(lInfo.szAccount,lvecInfo[i].mUserOnlineCount <= 0 ? false : true);
	//	}
	//}

	////发送开始初始化命令，通知其他模块开始初始化 //add： SvrLogin会关闭所有的CloseAllTraderSocket()，没有必要
	//Stru_NotifyEvent lDataEventInit(EventTypeStartInit,0);
	//CInterface_SvrNotifyAndAsk::getObj().NotifyEvent(lDataEventInit);
...
bool  CReal_SvrTradeData::SlaverInitAfterGetTradingDay()
{
	//转载日初数据
	std::string lsTempTradingDay;
	std::string lsLastTradingDay;
	...
	//设置合约和费率的日期
	msInstrumentDay = lsTempTradingDay;
	//std::string lsTempShortTradingDay = lsLastTradingDay.substr();
	//从数据库转载合约
	//处理合约
	//如果数据库有今天的合约，则读取数据库
	//如果数据库里没有今天的合约，则向报盘发送查询合约的事件
	vector<PlatformStru_InstrumentInfo> lVecInstrument;	
	std::set<std::string> lSetInstrumentId;
	if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayInstrument(lsTempTradingDay,lVecInstrument))
	{
		TRADE_LOG("LoadTodayInstrument From DataBase Success Send Get Instrument %s",lsTempTradingDay.c_str());
		for(int i=0;i<(int)lVecInstrument.size();i++)
		{
			SetInstrumentInfo(lVecInstrument[i],i==(int)lVecInstrument.size()-1?true:false);
		}
	...
	for(;lIter != lVecAccount.end();lIter++)
	{
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号的费率");
			continue;
		}

		//装载主帐号的资金
		PlatformStru_TradingAccountInfo lAccountInfo;	
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountFundInfo("TRADEDATA_A_FUNDINIT",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lAccountInfo))
		{
			//设置状态		
			if(mMapAccountData.end() != lIterReady)
			{
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号%s的资金",lIter->szTradeAccount);
			continue;
		}

		//装载主帐号的持仓
		vector<PlatformStru_Position> lvecPosition;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountPositions("TRADEDATA_A_P_INIT",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lvecPosition))
		{
			//设置状态
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号%s的持仓",lIter->szTradeAccount);
			if(mMapAccountData.end() != lIterReady)
			{			
				PlatformStru_Position lPostion;
				lIterReady->second->QryPositionReach(lPostion,-100,true);
		...
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountPositionDetails("TRADEDATA_A_PD_INIT",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lvecPositionDetail))
		{
			//设置状态
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号%s的持仓明细",lIter->szTradeAccount);
			if(mMapAccountData.end() != lIterReady)
			{			
				PlatformStru_PositionDetail lPostionDetail;
				lIterReady->second->QryPositionDetailReach(lPostionDetail,-100,true);
		...
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountTrades("TRADEDATA_A_T_INIT",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lvecTrader))
		{
			//设置状态
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号%s的成交",lIter->szTradeAccount);
			if(mMapAccountData.end() != lIterReady)
			{			
				PlatformStru_TradeInfo lTrader;
				lIterReady->second->QryTradeReach(lTrader,-100,true);	
		...
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountOrders("TRADEDATA_A_O_INIT",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lvecOrder))
		{
			//设置状态
		...
		{
			PUSH_LOG_ERROR(Logger,false,true,"数据库中没有该主帐号%s的报单",lIter->szTradeAccount);
			if(mMapAccountData.end() != lIterReady)
			{			
				PlatformStru_OrderInfo lOrder;
				lIterReady->second->QryOrderReach(lOrder,-100,true);
		...
		//装载最近前一次主服务落地的数据
		//装载主帐号的成交
		vector<PlatformStru_TradeInfo> lvecSecondTrader;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayAccountTrades("CTP_TRADES",lBroker.szBrokerCode,lIter->szTradeAccount,lsTempTradingDay,lvecSecondTrader))
		{
			//设置状态
			
			if(mMapAccountData.end() != lIterReady)
			{
				vector<PlatformStru_TradeInfo>::iterator lIte = lvecSecondTrader.begin();
				for(;lIte != lvecSecondTrader.end();lIte++)
				{	
					//如果已有的成交则不用再次计算
					TradeKey key(*lIte);
				
					if(lIterReady->second->TestTradeInfoExist(key) == false)
					{
						lIterReady->second->PushTrader(*lIte);	
						lIterReady->second->SetLoadTraderUpdateSeq(lIte->UpdateSeq);
					}
				}				
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for(;lIterUser != mMapUser2TradingData.end();lIterUser++)
	{
		...
		//装载子帐号的资金
		PlatformStru_TradingAccountInfo lAccountInfo;	
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayUserFundInfo("SETTLEMENT_FUNDINIT",lIterUser->first,lsLastTradingDay,lAccountInfo))
		{
			//设置状态
			lIterUser->second->SetQryFundInfo(lAccountInfo);
			if(mbNeedCache)
				mCache->CacheFund(lAccountInfo);
		}
		else
		{
		...
		//装载主帐号的持仓
		vector<PlatformStru_Position> lvecPosition;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayUserPositions("SETTLEMENT_POSITIONINIT",lIterUser->first,lsLastTradingDay,lvecPosition))
		{
			//设置状态
			vector<PlatformStru_Position>::iterator lIte = lvecPosition.begin();
			for(;lIte != lvecPosition.end();lIte++)
		...
		//装载主帐号的持仓明细
		vector<PlatformStru_PositionDetail> lvecPositionDetail;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayUserPositionDetails("SETTLEMENT_POSITIONDETAILINIT",lIterUser->first,lsLastTradingDay,lvecPositionDetail))
		{
			//设置状态
			vector<PlatformStru_PositionDetail>::iterator lIte = lvecPositionDetail.begin();
			for(;lIte != lvecPositionDetail.end();lIte++)
			{				
				...
				if(mbNeedCache)
					mCache->CachePositionDetail(*lIte);
			}	
			PlatformStru_PositionDetail lPostionDetail;		
		...
		//装载子帐号的成交
		vector<PlatformStru_TradeInfo> lvecTrader;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayUserTrades("TRADEDATA_TRADES",lIterUser->first,lsTempTradingDay,lvecTrader))
		{
			//设置状态
			vector<PlatformStru_TradeInfo>::iterator lIte = lvecTrader.begin();
			for(;lIte != lvecTrader.end();lIte++)
		...
		//装载主帐号的报单
		vector<PlatformStru_OrderInfo> lvecOrder;
		if(CF_ERROR_SUCCESS == mpDBOpr->LoadTodayUserOrders("TRADEDATA_ORDERS",lIterUser->first,lsTempTradingDay,lvecOrder))
		{
			//设置状态
			vector<PlatformStru_OrderInfo>::iterator lIte = lvecOrder.begin();
			for(;lIte != lvecOrder.end();lIte++)
	...
	////发送同步最新的主服务内存数据的请求
	lIterUser =  mMapUser2TradingData.begin();
	for(;lIterUser != mMapUser2TradingData.end() && mMasterSocket;lIterUser++)
	{
		int lastOrderSeq = 1;
		int lastTraderSeq = 1;
		lIterUser->second->GetLoadOrderUpdateSeq(lastOrderSeq);
		lIterUser->second->GetLoadTraderUpdateSeq(lastTraderSeq);
		TRADE_LOG("Send Syc Request user %s OrderSeq %d TraderSeq %d",lIterUser->first.c_str(),lastOrderSeq,lastTraderSeq);
		//获取数据库读取最大的updateseq
		CInterface_SvrTcp::getObj().SendPkgData(mMasterSocket,
			Cmd_TradeDataManager_SlaverQryOrderAndTraderBiggerFTID_Req,
			(void*)lIterUser->first.c_str(),
			lIterUser->first.size()+1,
			0,
			0,
			lastOrderSeq,
			lastTraderSeq,
	...
	for(;lIterAccount != mMapAccountData.end();lIterAccount++)
	{
		...
		int lastOrderSeq = 1;
		int lastTraderSeq = 1;
		lIterAccount->second->GetLoadOrderUpdateSeq(lastOrderSeq);
		lIterAccount->second->GetLoadTraderUpdateSeq(lastTraderSeq);
		TRADE_LOG("Send Syc Request account %s OrderSeq %d TraderSeq %d",lIterAccount->first.AccountID,lastOrderSeq,lastTraderSeq);
		CInterface_SvrTcp::getObj().SendPkgData(mMasterSocket,
			Cmd_TradeDataManager_SlaverQryOrderAndTraderBiggerFTID_Req,
			(void*)&lIterAccount->first,
			sizeof(BrokerAccountKey),
			0,
			0,
			lastOrderSeq,
			lastTraderSeq,
	...
	//
	//PUSH_LOG_ERROR(Logger,false,true,"---服务器从数据库初始化成功---");

	//发送定时器，定时检查是否都返回成功
	PostThreadMessage(g_idThread,TMESSAGE_TRADEINIT_CHECK_USER_ACCOUNT_SYNC,0,0);
...
// 	return lbRet;		
// }
bool CReal_SvrTradeData::ConnectRedisServer()
{
    if(mbNeedCache)
    {
        if(mCache==NULL) mCache = new CMemCache;
        if(mSubCache==NULL) mSubCache = new CMemCache;

        if(false == mCache->connect(mCacheHostIP,mCacheHostPort) || 
            false == mSubCache->connect(mCacheHostIP,mCacheHostPort))
        {
            PUSH_LOG_ERROR(Logger,false,true,"不能连接到Cache服务器%s:%d，初始化失败...",mCacheHostIP.c_str(),mCacheHostPort);
            return false;
        }
        else
        {
            PUSH_LOG_ERROR(Logger,false,true,"成功连接到Cache服务器");
...
bool CReal_SvrTradeData::DisConnectRedisServer()
{
    //用一个释放所有的Redis客户端连接就可以了
    if(mCache/* && mSubCache*/)
        //if(mCache->disconnect()&&mSubCache->disconnect())
        //{
            return mCache->shutdownRedisServer();
        //}
