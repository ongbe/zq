...
public: //交易所状态通知	
	...
	//交易状态初始化1
	virtual void TradeDataInit(bool nbForce = false) ;
...
public: //合约操作
	...
	///设置指定合约信息
	virtual void SetInstrumentInfo(PlatformStru_InstrumentInfo& outData,bool bLast) ;
...
public: //资金
	...
	//报盘调用，当某一个主帐号资金查询完成
	virtual void QryFundReach(const PlatformStru_TradingAccountInfo& inData) ;
...
public://内部接口
	...
	//装载某一天所有用户的的日初数据
	virtual CF_ERROR LoadInitData(const std::string & nsDate);
	...
	//初始化用户对象
	virtual CF_ERROR InitUserMapWithEmpty(_MapUserTradeData & nmapUserData);
	//全部数据重置
...
public:
	...
	bool SlaverInit();  //从服务器的初始化入口函数
	bool SlaverInitAfterGetTradingDay();
...
public:
	...
	bool CacheErrOrderInsert(const PlatformStru_OrderInfo & lOrder) {return mCache->PushOrderInputError(lOrder);}
	bool CacheErrOrderAction(const CThostFtdcOrderActionField & lOrderAction) {return mCache->PushOrderActionError(lOrderAction);}
...
public:	
	bool PushStrategyStopOrRun(const std::string & nsName,bool nsStop) {return mCache->PushStrategyStopOrRun(nsName,nsStop);};

	//bool PushTraderUseroffLine(const std::string & nUserName,bool bOffline) {return mCache->PushTraderUseroffLine(nUserName,bOffline);} ;

	bool PushStrategyInstanceStopOrRun(const std::string & nsUserName,
		const std::string & nsStategyName,
		bool nsStop) 
	{
			return mCache->PushStrategyInstanceStopOrRun(nsUserName,nsStategyName,nsStop);
	}

    bool PushStrategyTimer(const std::string & nsName,bool nsStop) {return mCache->PushStrategyTimer(nsName,nsStop);} ;

// 	virtual bool GetStategyUserInstanceUpdateTime(const std::string & nsUserName,
// 		const std::string & nsStategyName,
// 		time_t & nsUpdateTime) ;

    virtual bool ConnectRedisServer();
    virtual bool DisConnectRedisServer();
