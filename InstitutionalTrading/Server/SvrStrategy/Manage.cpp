...
void CManage::StartAllInstance()
{
	...
	while(it != mapStrategyInstance.end())
	{
		...
		SStrategy  strategy;
		bool bGetS = m_DataCenter.GetStrategy(instance.strategyName, strategy);
		if(bGetS && strategy.nUse == 1)//该策略处于启用状态
...
void CManage::HandleAllInstanceTimer(bool bStop)
{
    ...
    while(it != mapStrategyInstance.end())
    {
        ...
        {           
            ...
            CInterface_SvrTradeData::getObj().PushStrategyTimer(instance.strategyName, bStop);
        }
        it++;
    }
}
