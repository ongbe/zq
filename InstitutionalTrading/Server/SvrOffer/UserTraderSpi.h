...
{
public:
	CUserTraderSpi(CThostFtdcTraderApi *pUserApi, SLogin& sLogin) ;
	~CUserTraderSpi(void);
...
private:
	...
	CCTPAccount* m_pCTPAccount;

	//COfferMain*  m_pOfferMain; 


public:
	void SetAccount(CCTPAccount* pCTPAccount);
	int GetRequestID()//主要用于请求用的
	{
		int n = 0;
	...
	bool GetInputOrderKeyByAccountID(BrokerAccountKey BAKey, STransfer& keyCtp, STransfer& keyClient);
	bool GetInputOrderKeyByAccountID_ByOrderRef(BrokerAccountKey BAKey, SOrderRef& nCtpOrderRef, STransfer& nClientOrderRef);
