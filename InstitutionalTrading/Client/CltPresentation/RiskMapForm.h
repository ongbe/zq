...
#pragma endregion
		...
		{				
			 ...
			 const int lnBigOrgID = ORGNIZATION_ROOT;
			 sLoginRsp loginRsp;
			 ...
			 while(lnOrgID != 0)
			 {
				 ...
				 Organization lAssertOrg;
				 memset(&lAssertOrg, 0, sizeof(lAssertOrg));
				 if(!m_pdataInfo->GetOrganizationByID(
					 lnOrgID, lAssertOrg))
					 break;
		...
		*/
		...
		/*	int nWnd = this->Handle.ToInt32();
			...
			SubscribeFinicalProductStatiacsData(-1);
        }
	    ...
	    protected:virtual void WndProc( Message% m ) override
		{
					...
					case Cmd_RM_SubscribeStatistics_Rsp:
						{
							if(head.userdata3 == CF_ERROR_SUCCESS && head.seq == mnSeqID) 
							{
								...
								if(pStatic->mnFinancialProductID == -1) //汇总数据
								{								
								...
								for(int i = 0; i< count; i++)
								{
									...
									{																
										mRiskMapData->PushFinacialProductStatics(pStatic->mnFinancialProductID,pStatic);										
									}
								}
					...
					case Cmd_RM_Statistics_Push:
						{
							if(head.userdata3 == CF_ERROR_SUCCESS ) 
							{
								...
								{	
									mRiskMapData->mmapFinacialProducctStatics.erase(pStatic->mnFinancialProductID);															
								}
								bool lbStatic = false;
		 ...
		 //marker 鼠标点击事件
		  ...
		  void OnMapZoomChanged()
		  {
			  ...
			  {
				  mRiskMakerOverlay->IsChangedShowMarker(mnOlderZoom,this->cfMapControl1->Zoom);
				  mnOlderZoom = this->cfMapControl1->Zoom;
				  mbChickToZoom = false;
				  return;
			  }
     ...
     //             //再往上追溯一级 看看那个级别那个组织机构离中心点近
		  ...
		  void UpdateGridView(Int32 nParentID,bool nbClear)
		  {
			  ...
			  while(ltempOrg != nullptr)
			  {
				  ...
				  {
					  lDepartTotalData->Add(lIteratorStart->second.mnStatisticsType,lIteratorStart->second.mdblValue);				
					  lIteratorStart++;
				  } 
				  mRiskMapData->UnlockObject();
		  ...
		  void UpdatePie(Int32 nOrgID)
		  {
			...
			/*  if(nOrgID == -1)
			  ...
			  {
				  if( lIteratorStart->second.mnStatisticsType == ST_Balance) //总权益
				  {
					  ldbValue = lIteratorStart->second.mdblValue; 
					  
					  std::string lsName = m_pdataInfo->GetFinancialProductNameByID(lIteratorStart->first);

					  System::Diagnostics::Debug ::WriteLine(Tools::string2String(lsName.c_str()) + "" + ldbValue);
					  chart1->Series["Default"]->Points->AddXY(Tools::string2String(lsName.c_str()),ldbValue);		
					  chart1->Series["Default"]->Points[i]->Label = "#PERCENT{P1}";
					  chart1->Series["Default"]->Points[i]->LegendText = Tools::string2String(lsName.c_str());
					  chart1->Series["Default"]->Points[i++]->Tag = lIteratorStart->first;
...
private: System::Void SubscribeStatiacsData()
		 {
			 ...
			 if(lOrgMap != nullptr)
			 {
				 ...
				 for (;lIte != lOrgMap->end();lIte++)
				 {
					 ...
					 re.mnAssetMgmtOrgID=lTempOrg->ID;
...
private: System::Void SubscribeFinicalProductStatiacsData(Int32 lnOrgID)
		 {
			 ...
			 MAPFinancialProduct mapProductID;
			 m_pdataInfo->GetFinancialProduct( mapProductID);
			 if(mapProductID.size()>0)
			 {				
				 MAPFinancialProduct::iterator itInfo;
				 for(itInfo = mapProductID.begin(); itInfo != mapProductID.end(); itInfo++) 
				 {				
						 AssetOrgProRelation re;
						 re.mnAssetMgmtOrgID = -1;
						 re.mnFinancialProductID = itInfo->first;
