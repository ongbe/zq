...
#pragma endregion
	private: void LoadOrganAccount(TreeNode^ node, int nOrganID) {
				...
				if(m_pdataInfo->GetDealerByOrganID(	nOrganID, setFPID) == false)
					return;

				char strTemp[1024];
				for(it = setFPID.begin(); it != setFPID.end(); it++) {
					UserInfo lUserInfo;
					m_pdataInfo->GetUserInfoByUserID(*it,lUserInfo);

					memset(strTemp, 0, sizeof(strTemp));
					sprintf(strTemp, "A%d", lUserInfo.nUserID);
					String^ strText = Tools::string2String(strTemp);
					memset(strTemp, 0, sizeof(strTemp));
					sprintf(strTemp, "%s@%s", lUserInfo.szAccount, lUserInfo.szAlias);
...
public: virtual Void ProcessData() override
		{
			 ...
			 if(treeOrganAndAccount->Nodes->Count>0) {
				 ...
				 if(!bNoChecked)
				 { 
					 if(treeOrganAndAccount->Nodes[0]->Checked) {
