...
map<SOCKET, CTcpConnectSpi*> CTcpDataConnect::m_mapSpi;
			...
			if(Msg.lParam==scktOnTime) {
				...
				if(it != m_mapConn.end()) {
					...
					if(Msg.lParam==scktDisConnect) {
						...
						m_mapSpi.erase((SOCKET)Msg.wParam);
					}
				}
				else {
...
CTcpDataConnect* CTcpDataConnect::CreateConnect(string& SvrIP, WORD SvrPort, CTcpConnectSpi* pSpi)
{
	CTcpDataConnect* pConn = new CTcpDataConnect(SvrIP, SvrPort, pSpi);
...
CTcpDataConnect::CTcpDataConnect(string& SvrIP, WORD SvrPort, CTcpConnectSpi* pSpi)
{
	m_bStop = false;
	m_strSvrIP = SvrIP;
	m_wSvrPort = SvrPort;
...
void CTcpDataConnect::ConnSuccCBFunc(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket, int UserData)
{
	...
	if(pConn != NULL && !pConn->m_bStop) {
		...
		m_mapSpi.insert(make_pair(hSocket, pConn->GetConnectSpi()));
		int nCount = 0;
...
void CTcpDataConnect::ParseRecvPackage(int nMode, SOCKET hConnect)
{
	...
	case scktConnect:
		m_hConnect = hConnect;
		if(m_pConnectSpi != NULL) {
			m_pConnectSpi->OnFrontConnected();
			CFileOpr::getObj().writelocallog("TcpLayer","ParseRecvPackage，mode==connect, m_pConnectSpi==%x, SOCKET==%x",
				m_pConnectSpi, hConnect);
		}
		else {
			CFileOpr::getObj().writelocallog("TcpLayer","ParseRecvPackage，mode==connect, m_pConnectSpi==NULL, SOCKET==%x",
				hConnect);
		}
		break;
	case scktDisConnect:
		if(m_pConnectSpi != NULL) {
			m_pConnectSpi->OnFrontDisconnected();
		}
		m_hConnect = -1;
		break;
	case scktDataRecv:
		if(m_pRcvBuf)
		{
			...
			while(Stru_UniPkgHead::IsValidPkg(m_pRcvBuf+off,m_RcvBufLen-off,&LengthUsedByPkg))
			{
				...
				if(m_pConnectSpi != NULL) {
					m_pConnectSpi->OnRecvData(m_pRcvBuf+off+sizeof(Stru_UniPkgHead), 
							PkgHead.len, PkgHead.cmdid, PkgHead.moduleid, 
							PkgHead.seq, PkgHead.subseq, PkgHead.userdata1, PkgHead.userdata2, 
							PkgHead.userdata3, PkgHead.userdata4, m_pRcvBuf+off);
				}
