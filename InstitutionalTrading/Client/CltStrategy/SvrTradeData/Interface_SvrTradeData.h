...
public:
    ...
    void InitFunc(string szUser, string szBrokerCode, string szTradeAccount, CDataEntity* pDataEntity, CMemCache* pCmdCache, double dFund);
...
public: //交易所状态通知	
	...
	//交易数据管理模块初始化
	virtual void TradeDataInit(CMemCache* pCmdCache, string szUser,string szBrokerCode,string szTradeAccount,bool nbForce = false) = 0;
...
public: //合约操作
	...
	///设置指定合约信息
	virtual void SetInstrumentInfo(PlatformStru_InstrumentInfo& outData) = 0;
...
public: //资金
	...
	//报盘调用，当某一个主帐号资金查询完成
	virtual void QryFundReach(const PlatformStru_TradingAccountInfo& inData,const std::string & nUserID) = 0;
...
public:
    ...
    //设置用户初始资金 策略本地调试 
    virtual CF_ERROR SetUserInitFund(const double dInitFund) = 0;
