...
namespace CFMap.CoreType
{
   ...
   public struct PointLatLng
   {
      ...
      public double LengthTwoPoint(PointLatLng npt)
      {
          double ldb1 = Math.Abs(npt.Lat - this.Lat);
          double ldb2 = Math.Abs(npt.Lng - this.Lng);
          return Math.Sqrt(ldb1 * ldb1 + ldb2 * ldb2);
      }
