...
bool CTcpLayer::m_bConnected=false;
bool CTcpLayer::m_bDisConnected=false;
...
{
	m_mutex.lock();
	CFileOpr::getObj().writelocallog("TcpLayer","Release，m_pTcpClient=%x ",
							m_pTcpClient);
...
void CTcpLayer::RecvDataCBFunc(char *pData,int DataLen,Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	if(m_bStop) return;
	...
	{
		PostMessage((HWND)m_hMainWnd,WM_USER_Win,WndUserCmd_DataArrival,0L);
	}

}

//断开tcp连接后，调用此回调函数进行处理
void CTcpLayer::DisconnCBFunc(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	CFileOpr::getObj().writelocallog("TcpLayer","DisconnCBFunc，localaddr=(%u):%u svraddr=(%u):%u, hSocket=%x",
	...
	if(!m_bStop&&m_hMainWnd!=NULL&&IsWindow((HWND)m_hMainWnd))
	{
		PostMessage((HWND)m_hMainWnd,WM_USER_Win,WndUserCmd_ConnStatus,0L);
	}
}

//开始调用connect()后，调用此函数通知应用层
void CTcpLayer::StartConnCBFunc(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	CFileOpr::getObj().writelocallog("TcpLayer","StartConnCBFunc，localaddr=(%u):%u svraddr=(%u):%u, hSocket=%x",
							LocalAddr.IP,LocalAddr.Port,SvrAddr.IP,SvrAddr.Port,hSocket);

	//通知主窗口连接开始(lParam=1)
	if(!m_bStop&&m_hMainWnd!=NULL&&IsWindow((HWND)m_hMainWnd))
	{
		PostMessage((HWND)m_hMainWnd,WM_USER_Win,WndUserCmd_ConnStatus,1L);
	}
}

//连接成功后，调用此回调函数进行处理
void CTcpLayer::ConnSuccCBFunc(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	CFileOpr::getObj().writelocallog("TcpLayer","StartConnCBFunc，localaddr=(%u):%u svraddr=(%u):%u, hSocket=%x",
							LocalAddr.IP,LocalAddr.Port,SvrAddr.IP,SvrAddr.Port,hSocket);
	m_bConnected=true;
	...
	if(!m_bStop&&m_hMainWnd!=NULL&&IsWindow((HWND)m_hMainWnd))
	{
		PostMessage((HWND)m_hMainWnd,WM_USER_Win,WndUserCmd_ConnStatus,2L);
	}
}

//连接失败后，调用此回调函数进行处理
void CTcpLayer::ConnFailCBFunc(Stru_IPAddress& LocalAddr,Stru_IPAddress& SvrAddr,SOCKET hSocket,int UserData)
{
	...
	if(!m_bStop&&m_hMainWnd!=NULL&&IsWindow((HWND)m_hMainWnd))
	{
		PostMessage((HWND)m_hMainWnd,WM_USER_Win,WndUserCmd_ConnStatus,3L);
	}
}

//定期调用的回调函数，一般每秒会调用一次，可以用于检查心跳超时等
void CTcpLayer::OnTimeCBFunc(int UserData)
{
	m_mutex.lock();
...
//判断是否处于连接状态
bool CTcpLayer::IsConnected()
{
	return m_bConnected;
}

bool CTcpLayer::SendData(UINT nCMDID, void* pData, int nLen, UINT seq, unsigned int userdata1, unsigned int userdata2, unsigned int userdata3, unsigned int userdata4)
{
		...
		while(Stru_UniPkgHead::IsValidPkg(m_pRcvBuf+off,m_RcvBufLen-off,&LengthUsedByPkg))
		{
			...
			while(itlower!=itupper)
			{
				...
				if(IsWindow((HWND)(itlower->second)))
					SendMessage((HWND)(itlower->second),WM_USER_Win,WndUserCmd_YourPkgArrival,(LPARAM)(m_pRcvBuf+off));
