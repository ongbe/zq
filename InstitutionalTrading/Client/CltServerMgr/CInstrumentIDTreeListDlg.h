	...
	public:
		void LoadInstrumentTree() {
			...
			for(it = setProducts.begin(); it != setProducts.end(); it++) {
				...
				vector<PlatformStru_InstrumentInfo> vecInstrument;
				m_pdataInfo->GetInstrumentFieldByProductID(*it, vecInstrument);
				for(int k=0; k<(int)vecInstrument.size(); k++) {
					strText = Tools::string2String(vecInstrument[k].InstrumentID);
					node->Nodes->Add(strText, strText);
				}
			}
